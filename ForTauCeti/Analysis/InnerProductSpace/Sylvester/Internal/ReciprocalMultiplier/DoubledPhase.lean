...
-/
import ForTauCeti.Analysis.InnerProductSpace.Sylvester.Internal.ReciprocalMultiplier.Fourier
...
theorem finiteUnitaryOrbitCertificate_orthogonalBlockSum_of_doubledInterpolation
    ...
    RectangularUnitarilyInvariantNorm.HasFiniteUnitaryOrbitCertificate
      mass
      (((δ : ℝ) : 𝕜) • RectangularUnitarilyInvariantNorm.orthogonalBlockSum X X)
  ...
  have hunit (i : Fin (Module.finrank 𝕜 F))
      (j : Fin (Module.finrank 𝕜 E)) :
      ((δ : ℝ) : 𝕜) • RectangularUnitarilyInvariantNorm.orthogonalBlockSum
          (basisMatrixUnit eF eE i j) (basisMatrixUnit eF eE i j) =
        (((α i - β j : ℝ) : 𝕜)) •
...
theorem kyFan_reciprocalMultiplier_le_of_approximateFourierInterpolation
  ...
  have hscale :
      RectangularUnitarilyInvariantNorm.rectangularKyFanSum (2 * k)
          (((δ : ℝ) : 𝕜) •
            RectangularUnitarilyInvariantNorm.orthogonalBlockSum X X) =
        δ * RectangularUnitarilyInvariantNorm.rectangularKyFanSum (2 * k)
          (RectangularUnitarilyInvariantNorm.orthogonalBlockSum X X) :=
    RectangularUnitarilyInvariantNorm.rectangularKyFanSum_real_smul
      (2 * k) (RectangularUnitarilyInvariantNorm.orthogonalBlockSum X X) hδ.le
  rw [hscale,
