...
For a compact, positive, self-adjoint `A` on a complex Hilbert space the whole
...
`S`.  Eigenvalues are indexed by *reals* rather than complex numbers because for
a self-adjoint operator that is where they live, and because the bands used
below (`Set.Ici μ`, `Set.Ioi μ`) are intervals of `ℝ`. -/
def eigenSpan (A : E →L[ℂ] E) (S : Set ℝ) : Submodule ℂ E :=
  ⨆ s : S, eigenspace A.toLinearMap ((s : ℝ) : ℂ)

/-- Each eigenspace named by the band sits inside the band's span. -/
theorem eigenspace_le_eigenSpan (A : E →L[ℂ] E) {S : Set ℝ} {s : ℝ} (hs : s ∈ S) :
    eigenspace A.toLinearMap (s : ℂ) ≤ eigenSpan A S :=
  le_iSup (fun s : S => eigenspace A.toLinearMap ((s : ℝ) : ℂ)) ⟨s, hs⟩

/-- The band span is determined by the eigenspaces it names, so any submodule
containing all of them contains it. -/
theorem eigenSpan_le (A : E →L[ℂ] E) {S : Set ℝ} {W : Submodule ℂ E}
    (h : ∀ s ∈ S, eigenspace A.toLinearMap (s : ℂ) ≤ W) : eigenSpan A S ≤ W :=
  iSup_le fun s => h s s.2

/-- A larger band spans a larger subspace. -/
theorem eigenSpan_mono (A : E →L[ℂ] E) {S T : Set ℝ} (h : S ⊆ T) :
...
theorem eigenSpan_union (A : E →L[ℂ] E) (S T : Set ℝ) :
...
theorem eigenSpan_singleton (A : E →L[ℂ] E) (μ : ℝ) :
    eigenSpan A {μ} = eigenspace A.toLinearMap (μ : ℂ) := by
...
theorem eigenSpan_invariant (A : E →L[ℂ] E) (S : Set ℝ) :
  ...
  have h : eigenSpan A S ≤ Submodule.comap A.toLinearMap (eigenSpan A S) := by
    ...
    have hw' : A w = ((s : ℝ) : ℂ) • w := Module.End.mem_eigenspace_iff.mp hw
    have hmem : A w ∈ eigenspace A.toLinearMap ((s : ℝ) : ℂ) := by
      rw [hw']
...
variable {A : E →L[ℂ] E} [CompleteSpace E]

/-- The eigenspaces of a self-adjoint operator, indexed by their real eigenvalue,
form an orthogonal family.  This is Mathlib's complex-indexed family composed
with the injection `ℝ → ℂ`. -/
theorem orthogonalFamily_eigenspace_real (hAs : IsSelfAdjoint A) :
    OrthogonalFamily ℂ (fun s : ℝ => (eigenspace A.toLinearMap (s : ℂ) : Submodule ℂ E))
      (fun s => (eigenspace A.toLinearMap (s : ℂ)).subtypeₗᵢ) :=
  ((ContinuousLinearMap.isSelfAdjoint_iff_isSymmetric.mp
    hAs).orthogonalFamily_eigenspaces).comp Complex.ofReal_injective
...
theorem eigenSpan_isOrtho_eigenspace (hAs : IsSelfAdjoint A) {S : Set ℝ} {μ : ℝ}
    (hμ : μ ∉ S) : eigenSpan A S ⟂ eigenspace A.toLinearMap (μ : ℂ) := by
...
theorem eq_ofReal_re_of_eigenspace_ne_bot (hAs : IsSelfAdjoint A)
    (hApos : ∀ x, 0 ≤ RCLike.re ⟪A x, x⟫_ℂ) {μ : ℂ}
    (h : eigenspace A.toLinearMap μ ≠ ⊥) : μ = ((μ.re : ℝ) : ℂ) ∧ 0 ≤ μ.re := by
  ...
  have hreal : μ = ((μ.re : ℝ) : ℂ) := (Complex.conj_eq_iff_re.mp hconj).symm
  refine ⟨hreal, ?_⟩
  have hev' : Module.End.HasEigenvalue A.toLinearMap ((μ.re : ℝ) : ℂ) := by
    rw [← hreal]; exact hev
...
theorem le_norm_apply_of_mem_eigenSpan (hAs : IsSelfAdjoint A) {S : Set ℝ} {t : ℝ}
  ...
  have hfam :
      OrthogonalFamily ℂ
        (fun s : S => (eigenspace A.toLinearMap ((s : ℝ) : ℂ) : Submodule ℂ E))
        (fun s => (eigenspace A.toLinearMap ((s : ℝ) : ℂ)).subtypeₗᵢ) :=
  ...
  have hAsum : A x = ∑ s ∈ u, ((((s : ℝ) : ℂ) • f s : _) : E) := by
    rw [hxsum, map_sum]
    refine Finset.sum_congr rfl fun s _ => ?_
    have hfs := Module.End.mem_eigenspace_iff.mp (f s).2
    simpa using hfs
  have hAnorm : ‖A x‖ ^ 2 = ∑ s ∈ u, ‖(((s : ℝ) : ℂ) • f s : _)‖ ^ 2 := by
    rw [hAsum]
    simpa using hfam.norm_sum (fun s => (((s : ℝ) : ℂ) • f s : _)) u
  have hkey : t ^ 2 * ‖x‖ ^ 2 ≤ ‖A x‖ ^ 2 := by
    rw [hxnorm, hAnorm, Finset.mul_sum]
    refine Finset.sum_le_sum fun s _ => ?_
    have hts : t ≤ (s : ℝ) := hS s s.2
    have hnorm : ‖(((s : ℝ) : ℂ) • f s : _)‖ = |(s : ℝ)| * ‖f s‖ := by
      rw [norm_smul]
      simp
...
theorem le_approximationNumber_of_lt_rank_eigenSpan (hAs : IsSelfAdjoint A)
    ...
    (hn : (n : Cardinal) < Module.rank ℂ (eigenSpan A S)) :
...
theorem finiteDimensional_eigenSpan (hAc : IsCompactOperator A) (hAs : IsSelfAdjoint A)
    ...
    FiniteDimensional ℂ (eigenSpan A S) := by
  ...
  have hrank : Module.rank ℂ (eigenSpan A S) ≤ (n : Cardinal) := by
    by_contra hcon
...
theorem finrank_eigenSpan_le (hAc : IsCompactOperator A) (hAs : IsSelfAdjoint A)
  ...
  refine absurd (le_approximationNumber_of_lt_rank_eigenSpan hAs ht.le hS ?_)
    (not_le.mpr hn)
  rw [← Module.finrank_eq_rank' ℂ (eigenSpan A S)]
...
is invariant. -/
theorem orthogonal_invariant_of_invariant (hAs : IsSelfAdjoint A) {W : Submodule ℂ E}
  ...
  have hAw : ⟪A w, v⟫_ℂ = 0 := (Submodule.mem_orthogonal _ _).mp hv _ (hW w hw)
  have h := hsymm w v
...
theorem norm_comp_subtypeL_orthogonal_le (hAc : IsCompactOperator A)
    (hAs : IsSelfAdjoint A) {W : Submodule ℂ E} (hW : ∀ v ∈ W, A v ∈ W) {c : ℝ}
    (hc : 0 ≤ c)
  ...
  set S : (Wᗮ : Submodule ℂ E) →L[ℂ] (Wᗮ : Submodule ℂ E) := A.restrict hinvL with hSdef
  ...
  have hnorm : ‖S‖ ≤ (cn : ℝ) := by
    have hsr : spectralRadius ℂ S = ‖S‖₊ := S.spectralRadius_eq_nnnorm hSsa
    have hle : (‖S‖₊ : ℝ≥0∞) ≤ (cn : ℝ≥0∞) := by
      ...
      · simp
      · have hev : Module.End.HasEigenvalue (S : Module.End ℂ (Wᗮ : Submodule ℂ E)) k :=
        ...
        have hyeq : A (y : E) = k • (y : E) := by
          ...
          simpa [hSdef] using congrArg (fun z : (Wᗮ : Submodule ℂ E) => (z : E)) hme
        have hkc : ‖k‖ ≤ (cn : ℝ) := by
...
theorem approximationNumber_le_of_eigenSpan_cover (hAc : IsCompactOperator A)
    (hAs : IsSelfAdjoint A) (hApos : ∀ x, 0 ≤ RCLike.re ⟪A x, x⟫_ℂ) {S : Set ℝ} {c : ℝ}
    (hc : 0 ≤ c) (hcover : ∀ s : ℝ, c < s → s ∈ S)
    [FiniteDimensional ℂ (eigenSpan A S)] {n : ℕ} (hn : finrank ℂ (eigenSpan A S) ≤ n) :
  ...
  have hbd : ∀ (ν : ℂ) (v : E), v ∈ Wᗮ → v ≠ 0 → A v = ν • v → ‖ν‖ ≤ c := by
    ...
    have hvE' : v ∈ eigenspace A.toLinearMap ((ν.re : ℝ) : ℂ) := by rwa [← hreal]
    have hle : ν.re ≤ c := by
      by_contra hcon
      have hmem : ν.re ∈ S := hcover _ (lt_of_not_ge hcon)
...
theorem finite_eigenvalues_ge (hAc : IsCompactOperator A) (hAs : IsSelfAdjoint A)
  ...
  have hmem : (⟨v, hvW⟩ : W) ∈ eigenspace B ((s : ℝ) : ℂ) := by
...
theorem exists_eigenSpan_Ioi_eq_Ici (hAc : IsCompactOperator A) (hAs : IsSelfAdjoint A)
  ...
  by_cases hbot : eigenspace A.toLinearMap (s : ℂ) = ⊥
...
theorem le_approximationNumber_iff_lt_finrank_eigenSpan_Ici (hAc : IsCompactOperator A)
  ...
  · intro hlt
    refine le_approximationNumber_of_lt_rank_eigenSpan hAs hμ.le (S := Set.Ici μ)
      (fun s hs => hs) ?_
    rw [← Module.finrank_eq_rank' ℂ (eigenSpan A (Set.Ici μ))]
    exact_mod_cast hlt
...
theorem finrank_eigenSpan_Ici (hAc : IsCompactOperator A) (hAs : IsSelfAdjoint A)
  ...
  have hsplit : eigenSpan A (Set.Ici μ) =
      eigenSpan A (Set.Ioi μ) ⊔ eigenspace A.toLinearMap (μ : ℂ) := by
    rw [← eigenSpan_singleton A μ, ← eigenSpan_union, Set.Ioi_union_left]
  have hdisj : Disjoint (eigenSpan A (Set.Ioi μ)) (eigenspace A.toLinearMap (μ : ℂ)) :=
...
theorem finrank_eigenspace_eq_card_approximationNumber_eq (hAc : IsCompactOperator A)
    (hAs : IsSelfAdjoint A) (hApos : ∀ x, 0 ≤ RCLike.re ⟪A x, x⟫_ℂ) {μ : ℝ} (hμ : 0 < μ) :
    finrank ℂ (eigenspace A.toLinearMap (μ : ℂ)) =
      Nat.card {n : ℕ // A.approximationNumber n = μ} := by
  classical
  set N : ℕ := finrank ℂ (eigenSpan A (Set.Ici μ)) with hN
...
theorem eigenspace_eq_bot_of_not_pos (hAs : IsSelfAdjoint A)
    (hApos : ∀ x, 0 ≤ RCLike.re ⟪A x, x⟫_ℂ) (hA0 : eigenspace A.toLinearMap 0 = ⊥)
    {μ : ℂ} (hμ : ¬ ∃ r : ℝ, 0 < r ∧ μ = (r : ℂ)) :
  ...
  · refine hne ?_
    have hzero : μ = 0 := by rw [hreal, ← hz]; simp
    rw [hzero]
    exact hA0
  · exact hμ ⟨μ.re, hlt, hreal⟩
...
theorem finrank_eigenspace_congr_of_approximationNumber_eq {B : F →L[ℂ] F}
    ...
    (hAB : ∀ n, A.approximationNumber n = B.approximationNumber n) (μ : ℂ) :
    finrank ℂ (eigenspace A.toLinearMap μ) = finrank ℂ (eigenspace B.toLinearMap μ) := by
  by_cases hpos : ∃ r : ℝ, 0 < r ∧ μ = (r : ℂ)
...
theorem exists_linearIsometryEquiv_intertwining_of_approximationNumber_eq {B : F →L[ℂ] F}
    (hAc : IsCompactOperator A) (hAs : IsSelfAdjoint A)
    (hApos : ∀ x, 0 ≤ RCLike.re ⟪A x, x⟫_ℂ)
    (hA0 : eigenspace A.toLinearMap 0 = ⊥)
    (hBc : IsCompactOperator B) (hBs : IsSelfAdjoint B)
