...
theorem integrable_secondPrimitiveKernel_mul {w : ℝ → ℂ}
    ...
    Integrable (fun s => (secondPrimitiveKernel t s : ℂ) * w s) unitIocMeasure := by
...
density. -/
theorem norm_secondPrimitive_sub_le {w : ℝ → ℂ} (hw : Integrable w unitIocMeasure)
    (t t' : ℝ) :
    ‖secondPrimitive w t - secondPrimitive w t'‖
      ≤ |t - t'| * ∫ s, ‖w s‖ ∂unitIocMeasure := by
  have hdiff : secondPrimitive w t - secondPrimitive w t'
      = ∫ s, ((secondPrimitiveKernel t s : ℂ) - (secondPrimitiveKernel t' s : ℂ)) * w s
          ∂unitIocMeasure := by
...
theorem continuous_secondPrimitive {w : ℝ → ℂ} (hw : Integrable w unitIocMeasure) :
...
theorem ae_norm_secondPrimitive_le {w : ℝ → ℂ} (hw : Integrable w unitIocMeasure) :
    ∀ᵐ t ∂unitIocMeasure,
...
interval. -/
theorem memLp_secondPrimitive {w : ℝ → ℂ} (hw : Integrable w unitIocMeasure) :
...
theorem integral_secondPrimitive_mul_intervalBumpD2 {w : ℝ → ℂ}
    (hw : Integrable w unitIocMeasure) (k : ℕ) :
    ∫ t, secondPrimitive w t * (intervalBumpD2 k t : ℂ) ∂unitIocMeasure
      = ∫ s, w s * (intervalBump k s : ℂ) ∂unitIocMeasure := by
  ...
  have hFmeas : AEStronglyMeasurable
      (fun p : ℝ × ℝ =>
        (secondPrimitiveKernel p.1 p.2 : ℂ) * w p.2 * (intervalBumpD2 k p.1 : ℂ))
    ...
    · filter_upwards [haeprod] with p hp
      ...
      calc secondPrimitiveKernel p.1 p.2 * ‖w p.2‖ * |intervalBumpD2 k p.1|
          ≤ 1 * ‖w p.2‖ * C := by
            ...
            positivity
        _ = C * ‖w p.2‖ := by ring
  have houter : ∀ t, secondPrimitive w t * (intervalBumpD2 k t : ℂ)
      = ∫ s, (secondPrimitiveKernel t s : ℂ) * w s * (intervalBumpD2 k t : ℂ)
          ∂unitIocMeasure := by
    intro t
    rw [secondPrimitive, ← MeasureTheory.integral_mul_const]
  have hinner : ∀ᵐ s ∂unitIocMeasure,
      (∫ t, (secondPrimitiveKernel t s : ℂ) * w s * (intervalBumpD2 k t : ℂ)
          ∂unitIocMeasure)
        = w s * (intervalBump k s : ℂ) := by
    filter_upwards [ae_mem_unitIocMeasure] with s hs
    have hpt : ∀ t : ℝ,
        (secondPrimitiveKernel t s : ℂ) * w s * (intervalBumpD2 k t : ℂ)
          = w s * ((secondPrimitiveKernel t s * intervalBumpD2 k t : ℝ) : ℂ) := by
      intro t
      push_cast
      ring
    calc ∫ t, (secondPrimitiveKernel t s : ℂ) * w s * (intervalBumpD2 k t : ℂ)
            ∂unitIocMeasure
        = ∫ t, w s * ((secondPrimitiveKernel t s * intervalBumpD2 k t : ℝ) : ℂ)
            ∂unitIocMeasure := by
          exact integral_congr_ae (Filter.Eventually.of_forall hpt)
      _ = w s * ∫ t, ((secondPrimitiveKernel t s * intervalBumpD2 k t : ℝ) : ℂ)
            ∂unitIocMeasure := MeasureTheory.integral_const_mul _ _
      _ = w s * ((∫ t, secondPrimitiveKernel t s * intervalBumpD2 k t
            ∂unitIocMeasure : ℝ) : ℂ) := by rw [integral_complex_ofReal]
      _ = w s * (intervalBump k s : ℂ) := by
          rw [integral_secondPrimitiveKernel_mul_intervalBumpD2 hs k]
  calc ∫ t, secondPrimitive w t * (intervalBumpD2 k t : ℂ) ∂unitIocMeasure
      = ∫ t, ∫ s, (secondPrimitiveKernel t s : ℂ) * w s * (intervalBumpD2 k t : ℂ)
          ∂unitIocMeasure ∂unitIocMeasure :=
        integral_congr_ae (Filter.Eventually.of_forall houter)
    _ = ∫ s, ∫ t, (secondPrimitiveKernel t s : ℂ) * w s * (intervalBumpD2 k t : ℂ)
          ∂unitIocMeasure ∂unitIocMeasure := integral_integral_swap hFint
    _ = ∫ s, w s * (intervalBump k s : ℂ) ∂unitIocMeasure := integral_congr_ae hinner

/-! ## Vanishing moments force vanishing -/

/-- Multiplying an integrable function on `(0,1]` by a monomial keeps it integrable. -/
theorem integrable_mul_pow {h : ℝ → ℂ} (hh : Integrable h unitIocMeasure) (m : ℕ) :
    Integrable (fun t => h t * (t : ℂ) ^ m) unitIocMeasure := by
  refine Integrable.mono' hh.norm
    (hh.aestronglyMeasurable.mul
      ((Complex.continuous_ofReal.pow m).aestronglyMeasurable)) ?_
  filter_upwards [ae_mem_unitIocMeasure] with t ht
  rw [norm_mul, norm_pow, Complex.norm_real, Real.norm_eq_abs, abs_of_pos ht.1]
  calc ‖h t‖ * t ^ m ≤ ‖h t‖ * 1 :=
...
integrable. -/
theorem integrable_mul_intervalBumpD2 {h : ℝ → ℂ} (hh : Integrable h unitIocMeasure)
    (k : ℕ) :
    Integrable (fun t => h t * (intervalBumpD2 k t : ℂ)) unitIocMeasure := by
  obtain ⟨C, hC⟩ : ∃ C, ∀ x ∈ Set.Icc (0 : ℝ) 1, ‖intervalBumpD2 k x‖ ≤ C :=
    isCompact_Icc.exists_bound_of_continuousOn (continuous_intervalBumpD2 k).continuousOn
  refine Integrable.mono' (hh.norm.const_mul C)
    (hh.aestronglyMeasurable.mul
      ((Complex.continuous_ofReal.comp (continuous_intervalBumpD2 k)).aestronglyMeasurable))
    ?_
  filter_upwards [ae_mem_unitIocMeasure] with t ht
...
theorem integral_pow_eq_zero_of_forall_integral_bumpD2 {h : ℝ → ℂ}
    (hh : Integrable h unitIocMeasure)
    (hbump : ∀ k : ℕ, ∫ t, h t * (intervalBumpD2 k t : ℂ) ∂unitIocMeasure = 0)
    (h0 : ∫ t, h t ∂unitIocMeasure = 0)
    (h1 : ∫ t, h t * (t : ℂ) ∂unitIocMeasure = 0) :
    ∀ m : ℕ, ∫ t, h t * (t : ℂ) ^ m ∂unitIocMeasure = 0 := by
  intro m
  ...
  | _ m ih =>
    ...
    | 1, _ => simpa using h1
    | (k + 2), ih =>
      have hexp : ∀ t : ℝ, h t * (intervalBumpD2 k t : ℂ)
          = ((k : ℂ) + 2) * ((k : ℂ) + 1) * (h t * (t : ℂ) ^ k)
            - 2 * ((k : ℂ) + 3) * ((k : ℂ) + 2) * (h t * (t : ℂ) ^ (k + 1))
            + ((k : ℂ) + 4) * ((k : ℂ) + 3) * (h t * (t : ℂ) ^ (k + 2)) := by
        intro t
        rw [intervalBumpD2_eq_monomials]
        push_cast
        ring
      have hsplit : ∫ t, h t * (intervalBumpD2 k t : ℂ) ∂unitIocMeasure
          = ((k : ℂ) + 2) * ((k : ℂ) + 1)
              * ∫ t, h t * (t : ℂ) ^ k ∂unitIocMeasure
            - 2 * ((k : ℂ) + 3) * ((k : ℂ) + 2)
              * ∫ t, h t * (t : ℂ) ^ (k + 1) ∂unitIocMeasure
            + ((k : ℂ) + 4) * ((k : ℂ) + 3)
              * ∫ t, h t * (t : ℂ) ^ (k + 2) ∂unitIocMeasure := by
        have hint0 : Integrable
            (fun t : ℝ => ((k : ℂ) + 2) * ((k : ℂ) + 1) * (h t * (t : ℂ) ^ k))
        ...
        have hint2 : Integrable
            (fun t : ℝ => ((k : ℂ) + 4) * ((k : ℂ) + 3) * (h t * (t : ℂ) ^ (k + 2)))
            unitIocMeasure := (integrable_mul_pow hh (k + 2)).const_mul _
        have hB : ∫ t, (((k : ℂ) + 2) * ((k : ℂ) + 1) * (h t * (t : ℂ) ^ k)
              - 2 * ((k : ℂ) + 3) * ((k : ℂ) + 2) * (h t * (t : ℂ) ^ (k + 1))
              + ((k : ℂ) + 4) * ((k : ℂ) + 3) * (h t * (t : ℂ) ^ (k + 2)))
                ∂unitIocMeasure
            = (∫ t, (((k : ℂ) + 2) * ((k : ℂ) + 1) * (h t * (t : ℂ) ^ k)
              - 2 * ((k : ℂ) + 3) * ((k : ℂ) + 2) * (h t * (t : ℂ) ^ (k + 1)))
                ∂unitIocMeasure)
              + ∫ t, ((k : ℂ) + 4) * ((k : ℂ) + 3) * (h t * (t : ℂ) ^ (k + 2))
                ∂unitIocMeasure := integral_add (hint0.sub hint1) hint2
        have hA : ∫ t, (((k : ℂ) + 2) * ((k : ℂ) + 1) * (h t * (t : ℂ) ^ k)
              - 2 * ((k : ℂ) + 3) * ((k : ℂ) + 2) * (h t * (t : ℂ) ^ (k + 1)))
                ∂unitIocMeasure
            = (∫ t, ((k : ℂ) + 2) * ((k : ℂ) + 1) * (h t * (t : ℂ) ^ k)
                ∂unitIocMeasure)
              - ∫ t, 2 * ((k : ℂ) + 3) * ((k : ℂ) + 2) * (h t * (t : ℂ) ^ (k + 1))
      ...
      have hc2 : ((k : ℂ) + 4) * ((k : ℂ) + 3) ≠ 0 := by
        have h4 : ((k : ℂ) + 4) ≠ 0 := by
          have : ((k + 4 : ℕ) : ℂ) ≠ 0 := Nat.cast_ne_zero.mpr (by omega)
          push_cast at this
          exact this
        have h3 : ((k : ℂ) + 3) ≠ 0 := by
          have : ((k + 3 : ℕ) : ℂ) ≠ 0 := Nat.cast_ne_zero.mpr (by omega)
          push_cast at this
          exact this
...
theorem integrable_unitIocMeasure_of_continuous {f : ℝ → ℂ} (hf : Continuous f) :
...
integrable. -/
theorem integrable_mul_of_continuous {h g : ℝ → ℂ} (hh : Integrable h unitIocMeasure)
...
theorem ae_eq_zero_of_forall_integral_pow_eq_zero {h : ℝ → ℂ}
    (hh : MemLp h 2 unitIocMeasure)
    (hmom : ∀ m : ℕ, ∫ t, h t * (t : ℂ) ^ m ∂unitIocMeasure = 0) :
    h =ᵐ[unitIocMeasure] 0 := by
  have hhInt : Integrable h unitIocMeasure := hh.integrable one_le_two
  -- Every complex polynomial function integrates to zero against `h`.
  have hpoly : ∀ p : Polynomial ℂ,
      ∫ t, h t * Polynomial.eval (t : ℂ) p ∂unitIocMeasure = 0 := by
    intro p
    have hexp : ∀ t : ℝ, h t * Polynomial.eval (t : ℂ) p
        = ∑ m ∈ Finset.range (p.natDegree + 1),
            p.coeff m * (h t * (t : ℂ) ^ m) := by
      intro t
  ...
  have hcont : ∀ g : ℝ → ℂ, Continuous g →
    ...
    obtain ⟨pre, hpre⟩ := exists_polynomial_near_of_continuousOn 0 1
      (fun t => (g t).re) (Complex.continuous_re.comp hg).continuousOn δ hδ
    obtain ⟨pim, hpim⟩ := exists_polynomial_near_of_continuousOn 0 1
      (fun t => (g t).im) (Complex.continuous_im.comp hg).continuousOn δ hδ
    set p : Polynomial ℂ := pre.map (algebraMap ℝ ℂ)
      + Polynomial.C Complex.I * pim.map (algebraMap ℝ ℂ) with hpdef
    have hpeval : ∀ t : ℝ, Polynomial.eval (t : ℂ) p
        = ((pre.eval t : ℝ) : ℂ) + Complex.I * ((pim.eval t : ℝ) : ℂ) := by
      intro t
      have h1 : (pre.map (algebraMap ℝ ℂ)).eval ((t : ℝ) : ℂ) = ((pre.eval t : ℝ) : ℂ) := by
        rw [← Complex.coe_algebraMap, Polynomial.eval_map, Polynomial.eval₂_hom]
      have h2 : (pim.map (algebraMap ℝ ℂ)).eval ((t : ℝ) : ℂ) = ((pim.eval t : ℝ) : ℂ) := by
        rw [← Complex.coe_algebraMap, Polynomial.eval_map, Polynomial.eval₂_hom]
      rw [hpdef]
      rw [Polynomial.eval_add, Polynomial.eval_mul, Polynomial.eval_C, h1, h2]
    have hpc : Continuous fun t : ℝ => Polynomial.eval (t : ℂ) p :=
      p.continuous.comp Complex.continuous_ofReal
    have hnear : ∀ t ∈ Set.Icc (0 : ℝ) 1,
        ‖g t - Polynomial.eval (t : ℂ) p‖ ≤ 2 * δ := by
      intro t ht
      rw [hpeval]
      have hre : |(g t).re - pre.eval t| ≤ δ := by
        rw [abs_sub_comm]
        exact (hpre t ht).le
      have him : |(g t).im - pim.eval t| ≤ δ := by
        rw [abs_sub_comm]
        exact (hpim t ht).le
      calc ‖g t - (((pre.eval t : ℝ) : ℂ) + Complex.I * ((pim.eval t : ℝ) : ℂ))‖
          ≤ |(g t - (((pre.eval t : ℝ) : ℂ) + Complex.I * ((pim.eval t : ℝ) : ℂ))).re|
            + |(g t - (((pre.eval t : ℝ) : ℂ) + Complex.I * ((pim.eval t : ℝ) : ℂ))).im| :=
            Complex.norm_le_abs_re_add_abs_im _
        _ = |(g t).re - pre.eval t| + |(g t).im - pim.eval t| := by simp
        _ ≤ δ + δ := add_le_add hre him
        _ = 2 * δ := by ring
    have hsplitInt : ∫ t, h t * g t ∂unitIocMeasure
        = ∫ t, h t * (g t - Polynomial.eval (t : ℂ) p) ∂unitIocMeasure := by
      have hpt : ∀ t : ℝ, h t * g t
          = h t * (g t - Polynomial.eval (t : ℂ) p)
            + h t * Polynomial.eval (t : ℂ) p := by
        intro t
        ring
      have hgpInt : Integrable
          (fun t : ℝ => h t * (g t - Polynomial.eval ((t : ℝ) : ℂ) p)) unitIocMeasure :=
        integrable_mul_of_continuous hhInt (hg.sub hpc)
      have hppInt : Integrable
          (fun t : ℝ => h t * Polynomial.eval ((t : ℝ) : ℂ) p) unitIocMeasure :=
        integrable_mul_of_continuous hhInt hpc
      rw [integral_congr_ae (Filter.Eventually.of_forall hpt),
        integral_add hgpInt hppInt, hpoly p, add_zero]
    rw [hsplitInt, zero_add]
    calc ‖∫ t, h t * (g t - Polynomial.eval (t : ℂ) p) ∂unitIocMeasure‖
        ≤ ∫ t, ‖h t * (g t - Polynomial.eval (t : ℂ) p)‖ ∂unitIocMeasure :=
  ...
  suffices hzero : H = 0 by
    ...
    exact h1.trans h2
  have hSsub : (Lp.boundedContinuousFunction ℂ 2 unitIocMeasure : Set (Lp ℂ 2 unitIocMeasure))
      ⊆ {G : Lp ℂ 2 unitIocMeasure | ⟪G, H⟫_ℂ = 0} := by
    intro G hG
    ...
    have hcongr : ∀ᵐ t ∂unitIocMeasure, ⟪G t, H t⟫_ℂ = h t * (starRingEnd ℂ) (g t) := by
      filter_upwards [hGae, hHae] with t hGt hHt
      rw [RCLike.inner_apply, hGt, hHt]
    rw [integral_congr_ae hcongr]
    exact hcont (fun t => (starRingEnd ℂ) (g t)) (Complex.continuous_conj.comp g.continuous)
  have hdense : Dense
      (Lp.boundedContinuousFunction ℂ 2 unitIocMeasure : Set (Lp ℂ 2 unitIocMeasure)) :=
    Lp.boundedContinuousFunction_dense ℂ unitIocMeasure (by norm_num)
  have hclosed : IsClosed {G : Lp ℂ 2 unitIocMeasure | ⟪G, H⟫_ℂ = 0} :=
    isClosed_eq (continuous_id.inner continuous_const) continuous_const
  have hHself : ⟪H, H⟫_ℂ = 0 := by
    have : H ∈ closure
        (Lp.boundedContinuousFunction ℂ 2 unitIocMeasure : Set (Lp ℂ 2 unitIocMeasure)) :=
      hdense H
...
theorem eq_affine_add_secondPrimitive_of_forall_integral_bumpD2
    {u w : ℝ → ℂ} (hu : MemLp u 2 unitIocMeasure) (hw : MemLp w 2 unitIocMeasure)
    (hweak : ∀ k : ℕ,
      ∫ t, u t * (intervalBumpD2 k t : ℂ) ∂unitIocMeasure
        = ∫ t, w t * (intervalBump k t : ℂ) ∂unitIocMeasure) :
    ∃ a b : ℂ, u =ᵐ[unitIocMeasure]
      fun t => a + b * (t : ℂ) + secondPrimitive w t := by
  ...
  set h : ℝ → ℂ := fun t => u t - secondPrimitive w t with hhdef
  ...
  have hbump0 : ∀ k : ℕ, ∫ t, h t * (intervalBumpD2 k t : ℂ) ∂unitIocMeasure = 0 := by
    intro k
    have hψc : Continuous fun t : ℝ => (intervalBumpD2 k t : ℂ) :=
      Complex.continuous_ofReal.comp (continuous_intervalBumpD2 k)
    have hpt : ∀ t : ℝ, h t * (intervalBumpD2 k t : ℂ)
        = u t * (intervalBumpD2 k t : ℂ)
          - secondPrimitive w t * (intervalBumpD2 k t : ℂ) := by
      intro t
      simp only [hhdef]
      ring
    rw [integral_congr_ae (Filter.Eventually.of_forall hpt),
  ...
  have hI0 : ∫ _ : ℝ, (1 : ℂ) ∂unitIocMeasure = 1 := by
  ...
  have hI1 : ∫ t : ℝ, ((t : ℝ) : ℂ) ∂unitIocMeasure = (1 : ℂ) / 2 := by
    rw [integral_complex_ofReal, integral_unitIocMeasure_eq_intervalIntegral,
      integral_id]
    norm_num
  have hI2 : ∫ t : ℝ, ((t : ℝ) : ℂ) * ((t : ℝ) : ℂ) ∂unitIocMeasure = (1 : ℂ) / 3 := by
    have hpt : ∀ t : ℝ, ((t : ℝ) : ℂ) * ((t : ℝ) : ℂ) = ((t ^ 2 : ℝ) : ℂ) := by
      intro t
      push_cast
      ring
    ...
    rw [integral_pow]
    norm_num
  set A : ℂ := ∫ t, h t ∂unitIocMeasure with hAdef
  set B : ℂ := ∫ t, h t * (t : ℂ) ∂unitIocMeasure with hBdef
  set a : ℂ := 4 * A - 6 * B with hadef
  set b : ℂ := 12 * B - 6 * A with hbdef
  set h₀ : ℝ → ℂ := fun t => h t - (a + b * (t : ℂ)) with hh₀def
  have haffc : Continuous fun t : ℝ => a + b * ((t : ℝ) : ℂ) := by
    fun_prop
  have haffInt : Integrable (fun t : ℝ => a + b * ((t : ℝ) : ℂ)) unitIocMeasure :=
  ...
  have hh₀Mem : MemLp h₀ 2 unitIocMeasure := by
    ...
    filter_upwards [ae_mem_unitIocMeasure] with t ht
    calc ‖a + b * ((t : ℝ) : ℂ)‖ ≤ ‖a‖ + ‖b * ((t : ℝ) : ℂ)‖ := norm_add_le _ _
      _ ≤ ‖a‖ + ‖b‖ * 1 := by
          ...
          exact ht.2
      _ = ‖a‖ + ‖b‖ := by ring
  have hIc : ∀ c : ℂ, ∫ _ : ℝ, c ∂unitIocMeasure = c := by
    intro c
  ...
  have hbtInt : Integrable (fun t : ℝ => b * ((t : ℝ) : ℂ)) unitIocMeasure :=
    integrable_unitIocMeasure_of_continuous (by fun_prop)
  -- the affine moments of `h₀` vanish by the choice of `a` and `b`
  have haff0 : ∫ t, (a + b * ((t : ℝ) : ℂ)) ∂unitIocMeasure = a + b / 2 := by
    rw [integral_add (integrable_const a) hbtInt, hIc,
      MeasureTheory.integral_const_mul, hI1]
    ring
  have haff1 : ∫ t, (a + b * ((t : ℝ) : ℂ)) * ((t : ℝ) : ℂ) ∂unitIocMeasure
      = a / 2 + b / 3 := by
    have hpt : ∀ t : ℝ, (a + b * ((t : ℝ) : ℂ)) * ((t : ℝ) : ℂ)
        = a * ((t : ℝ) : ℂ) + b * (((t : ℝ) : ℂ) * ((t : ℝ) : ℂ)) := by
      intro t
      ring
    have hatInt : Integrable (fun t : ℝ => a * ((t : ℝ) : ℂ)) unitIocMeasure :=
      integrable_unitIocMeasure_of_continuous (by fun_prop)
    have hbt2Int : Integrable (fun t : ℝ => b * (((t : ℝ) : ℂ) * ((t : ℝ) : ℂ)))
        unitIocMeasure :=
      integrable_unitIocMeasure_of_continuous (by fun_prop)
    rw [integral_congr_ae (Filter.Eventually.of_forall hpt),
  ...
  have h₀0 : ∫ t, h₀ t ∂unitIocMeasure = 0 := by
    ...
    ring
  have h₀1 : ∫ t, h₀ t * ((t : ℝ) : ℂ) ∂unitIocMeasure = 0 := by
    have hpt : ∀ t : ℝ, h₀ t * ((t : ℝ) : ℂ)
        = h t * ((t : ℝ) : ℂ) - (a + b * ((t : ℝ) : ℂ)) * ((t : ℝ) : ℂ) := by
      intro t
      simp only [hh₀def]
      ring
    have htmulInt : Integrable (fun t : ℝ => h t * ((t : ℝ) : ℂ)) unitIocMeasure :=
      integrable_mul_of_continuous hhInt (by fun_prop)
    have haffmulInt : Integrable
        (fun t : ℝ => (a + b * ((t : ℝ) : ℂ)) * ((t : ℝ) : ℂ)) unitIocMeasure :=
      integrable_unitIocMeasure_of_continuous (by fun_prop)
    rw [integral_congr_ae (Filter.Eventually.of_forall hpt),
      integral_sub htmulInt haffmulInt, haff1, ← hBdef, hadef, hbdef]
    ring
  have h₀bump : ∀ k : ℕ, ∫ t, h₀ t * (intervalBumpD2 k t : ℂ) ∂unitIocMeasure = 0 := by
    intro k
    have hψ0 : ∫ t, ((intervalBumpD2 k t : ℝ) : ℂ) ∂unitIocMeasure = 0 := by
      rw [integral_complex_ofReal, integral_unitIocMeasure_eq_intervalIntegral,
        integral_intervalBumpD2]
      norm_num
    have hψ1 : ∫ t, ((t : ℝ) : ℂ) * ((intervalBumpD2 k t : ℝ) : ℂ) ∂unitIocMeasure
        = 0 := by
      have hpt : ∀ t : ℝ, ((t : ℝ) : ℂ) * ((intervalBumpD2 k t : ℝ) : ℂ)
    ...
    have hψc : Continuous fun t : ℝ => (intervalBumpD2 k t : ℂ) :=
      Complex.continuous_ofReal.comp (continuous_intervalBumpD2 k)
    have hpt : ∀ t : ℝ, h₀ t * (intervalBumpD2 k t : ℂ)
        = h t * (intervalBumpD2 k t : ℂ)
          - (a * (intervalBumpD2 k t : ℂ)
            + b * (((t : ℝ) : ℂ) * (intervalBumpD2 k t : ℂ))) := by
      intro t
      simp only [hh₀def]
      ring
    have h1Int : Integrable (fun t : ℝ => a * (intervalBumpD2 k t : ℂ)) unitIocMeasure :=
      integrable_unitIocMeasure_of_continuous (by fun_prop)
    have h2Int : Integrable
        (fun t : ℝ => b * (((t : ℝ) : ℂ) * (intervalBumpD2 k t : ℂ))) unitIocMeasure :=
      integrable_unitIocMeasure_of_continuous (by fun_prop)
    have h12Int : Integrable
        (fun t : ℝ => a * (intervalBumpD2 k t : ℂ)
          + b * (((t : ℝ) : ℂ) * (intervalBumpD2 k t : ℂ))) unitIocMeasure :=
      h1Int.add h2Int
    rw [integral_congr_ae (Filter.Eventually.of_forall hpt),
