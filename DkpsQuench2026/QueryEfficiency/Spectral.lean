...
2. Define the CMDS entry rate exactly as in the theorem statement and obtain the
   ...
   theorem in `DkpsQuench2026.Response.Means`) using `hpopulationNorm`, `hηNonneg`, and
...
spectral/response theorem needed by both final raw-response Quench capstones.
