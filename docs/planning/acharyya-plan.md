...
> `AGENTS.md`, under which Mathlib is not the near-term target. See
> `remaining-work.md`** (tracker) and
> `tauceti-adaptation-and-spectra-extraction.md` (the current roadmap). The
> superseded readiness audit is kept at
> `topurge/docs/planning/prep_mathlib_review_and_readiness.md`. The "optional
...
| 3 | `Acharyya2024/Consistency.lean:134` `growing_models_growing_queries_perStage_consistency_of_uniqueProfile` | triangular-array regime | **FALSE as stated** (same disease). WP8 |
...
| 7 | `Acharyya2025/SpectralPipeline.lean` `operatorNormClose_of_entrywiseClose` | entrywise ε → operator nε | ✅ **PROVED** (WP1, this session) |
| 8 | `Acharyya2025/SpectralPipeline.lean` `exists_config_gramRealizesCMDS_of_spectralAssumptions` | CMDS Gram realization | Unprovable as stated (vacuous `CMDSpectralAssumptions`); true core = WP3 |
| 9 | `Acharyya2025/SpectralPipeline.lean` `cited_cmds_spectral_to_config_perturbation` | Davis–Kahan/Weyl/Gram-rigidity | The boss fight. Decomposed into WP4/WP5/WP7 |
...
**Hard bridge layers first.** WP4 (Gram rigidity) → WP5 (Weyl) → WP7
...
`fixed_models_growing_queries_consistency_of_uniqueProfile` from it.
...
`exists_config_gramRealizesCMDS_of_spectralAssumptions` from it.

### WP4 — Gram rigidity (exact case)
...
Then sin-Θ → Gram rigidity for the scaled embedding (needs WP4 + eigenvalue
...
* Matrix-world corollary of `exists_isometry_configError_spectralConfig_le`:
  ...
  ψ is ANY Gram realization of B (via WP4 Gram rigidity to move from
...
- [x] WP6 committed (statement-repair pass: growing_queries +
      exists_config_gramRealizesCMDS_of_spectralAssumptions repaired+proved; matrix-world capstone
...
- 2026-06-11: WP0/WP1 committed; 5 false statements marked TODO(false-statement)
  ...
  (Gram rigidity via Finsupp.linearCombination factorization + LinearIsometry.extend,
  file Acharyya2025/GramRigidity.lean). WP5 agent launched (discrete
...
- 2026-06-11 (later): WP2 ✅ COMMITTED (Probability.lean; WP2/WP3 agents were
  ...
  graveyard). WP4 ✅ COMMITTED (GramRigidity.lean, agent-clean). WP5 ✅ COMMITTED
...
- 2026-06-11 (evening): WP7(c4) ConfigPerturbation.lean ✅ COMMITTED — THE
  ...
  (growing_queries_dissimilarity_converges, exists_config_gramRealizesCMDS_of_spectralAssumptions;
...
- 2026-06-11 (mathlib-prep session): MATHLIB BUMP + STAGING LIBRARY.
  ...
  `ForMathlib` namespace. Staged: candidate #1 Gram rigidity
  ...
  upstream). Paper libs rewired as thin consumers: GramRigidity.lean,
