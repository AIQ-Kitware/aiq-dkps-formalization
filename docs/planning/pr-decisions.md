...
> `Challenge/MathlibCandidate/` + `Challenge/` (see
...
> historical; the live homes are `Challenge/MathlibCandidate/` for the original
> candidates, neutral `Challenge/<Name>/` packages for the other regressions, and
> `comparator/{candidate-*,challenge-*}.json`.
