...
gaps at the ends of the block `r, …, r + d - 1` are exactly the boundary gap of
the sorted spectrum of `A⋆A`. -/
theorem orderedBlockBoundaryGap_rightGram {A : E →ₗ[𝕜] F} {n d r : ℕ}
    (hn : finrank 𝕜 E = n) {Δ : ℝ}
    (hlo : ∀ q p : Fin n, (q : ℕ) + 1 = r → (p : ℕ) = r →
      Δ ≤ A.singularValues (q : ℕ) ^ 2 - A.singularValues (p : ℕ) ^ 2)
    (hhi : ∀ q p : Fin n, (q : ℕ) + 1 = r + d → (p : ℕ) = r + d →
      Δ ≤ A.singularValues (q : ℕ) ^ 2 - A.singularValues (p : ℕ) ^ 2) :
    OrderedBlockBoundaryGap (isSymmetric_rightGram A) hn r d Δ := by
...
theorem orderedBlockBoundaryGap_leftGram {A : E →ₗ[𝕜] F} {m d r : ℕ}
    (hm : finrank 𝕜 F = m) {Δ : ℝ}
    (hlo : ∀ q p : Fin m, (q : ℕ) + 1 = r → (p : ℕ) = r →
      Δ ≤ A.singularValues (q : ℕ) ^ 2 - A.singularValues (p : ℕ) ^ 2)
    (hhi : ∀ q p : Fin m, (q : ℕ) + 1 = r + d → (p : ℕ) = r + d →
      Δ ≤ A.singularValues (q : ℕ) ^ 2 - A.singularValues (p : ℕ) ^ 2) :
    OrderedBlockBoundaryGap (isSymmetric_leftGram A) hm r d Δ := by
  refine orderedBlockBoundaryGap_iff.mpr ⟨fun q p hq hp => ?_, fun q p hq hp => ?_⟩ <;>
...
theorem singularBoundaryGap_of_rank_le {A : E →ₗ[𝕜] F} {n d r : ℕ} {Δ : ℝ}
    (hrank : finrank 𝕜 (LinearMap.range A) ≤ r + d)
    (hs : ∀ q : Fin n, (q : ℕ) + 1 = r + d → Δ ≤ A.singularValues (q : ℕ) ^ 2) :
...
The block is the consecutive singular indices `r, …, r + d - 1`; `V`, `V̂` are
arbitrary orthonormal right singular frames of `A`, `Â` at those indices, with no
separation assumed among the sample singular values; and the hypothesis is the
squared boundary gap `Δ ≤ min(σ_{r-1}² − σ_r², σ_s² − σ_{s+1}²)` read at the
*ambient* dimension, with `σ_j = 0` past the rank.
...
theorem yuWangSamworth_rightSingularSubspace_block_le
    {A Â : E →ₗ[𝕜] F} [Nontrivial E] {n d r : ℕ} {hn : finrank 𝕜 E = n}
    (hrd : r + d ≤ n) {v vHat : Fin d → E}
    (hv : IsOrderedRightSingularFrame A hn (consecutiveEmb hrd) v)
    (hvHat : IsOrderedRightSingularFrame Â hn (consecutiveEmb hrd) vHat)
    {Δ : ℝ} (hΔ : 0 < Δ)
    (hlo : ∀ q p : Fin n, (q : ℕ) + 1 = r → (p : ℕ) = r →
      Δ ≤ A.singularValues (q : ℕ) ^ 2 - A.singularValues (p : ℕ) ^ 2)
    (hhi : ∀ q p : Fin n, (q : ℕ) + 1 = r + d → (p : ℕ) = r + d →
  ...
  yuWangSamworth_rightSingularSubspace_frame_le hv hvHat hΔ
    ((orderedBlockBoundaryGap_rightGram hn hlo hhi).indexGap hrd)

/-- **Theorem 3, left singular subspaces, in singular-value notation.** -/
theorem yuWangSamworth_leftSingularSubspace_block_le
    {A Â : E →ₗ[𝕜] F} [Nontrivial E] {m d r : ℕ} {hm : finrank 𝕜 F = m}
    (hrd : r + d ≤ m) {u û : Fin d → F}
    (hu : IsOrderedLeftSingularFrame A hm (consecutiveEmb hrd) u)
    (hû : IsOrderedLeftSingularFrame Â hm (consecutiveEmb hrd) û)
    {Δ : ℝ} (hΔ : 0 < Δ)
    (hlo : ∀ q p : Fin m, (q : ℕ) + 1 = r → (p : ℕ) = r →
      Δ ≤ A.singularValues (q : ℕ) ^ 2 - A.singularValues (p : ℕ) ^ 2)
    (hhi : ∀ q p : Fin m, (q : ℕ) + 1 = r + d → (p : ℕ) = r + d →
  ...
  yuWangSamworth_leftSingularSubspace_frame_le hu hû hΔ
    ((orderedBlockBoundaryGap_leftGram hm hlo hhi).indexGap hrd)
