#!/usr/bin/env python3
"""Static preflight for Palomar Registry submission surfaces.
...
permanent, which is why it is worth having a tripwire at all; the authorization for
this script is recorded as D3 in `dev/palomar-readiness.md`.
...
What it checks:
  ...
  metadata     formalization.yaml is v0.4, has the required fields, its licence
               matches the root LICENSE, and carries no leftover template text
...
Usage:
    ...
    python3 scripts/check_palomar_readiness.py --entry yws-2015
...
def check_metadata(rep: Report, path: pathlib.Path, *, wrapper: bool) -> None:
    try:
        import yaml
    except ImportError:
    ...
    sub = repo.get("substantive_formalization")
    if wrapper:
        if not sub:
            rep.fail(f"{where}: a wrapper must declare "
                     f"repository.substantive_formalization")
        else:
            rev = str(sub.get("revision", ""))
            if not re.fullmatch(r"[0-9a-f]{40}", rev):
                rep.warn(f"{where}: substantive_formalization.revision is {rev!r}; "
                         f"a real 40-character SHA is a maintainer step at submission")
    elif sub:
        rep.fail(f"{where}: the root metadata declares substantive_formalization, "
                 f"which marks it a thin wrapper; this repository is the "
                 f"substantive development")

    hits = TEMPLATE_RE.findall(path.read_text())
    if hits and not wrapper:
        rep.fail(f"{where}: leftover template/placeholder text ({len(hits)} occurrence(s))")
    elif hits:
        rep.warn(f"{where}: {len(hits)} placeholder(s) still to be filled before submitting")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if root_meta.exists():
        check_metadata(rep, root_meta, wrapper=False)
    else:
        rep.fail("no root formalization.yaml")

    configs = sorted(ENTRY_DIR.glob("*/comparator.json"))
    if args.entry:
        configs = [c for c in configs if c.parent.name == args.entry]
        if not configs:
