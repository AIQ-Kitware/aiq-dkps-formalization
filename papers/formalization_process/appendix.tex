...
accounting record.  Retained model turns and token quantities are incomplete
...
\centering
\scriptsize
\caption{Observed lower bounds and derived estimates. Not all LLM usage is tracked, thus it is important that these are read as lower bounds, and not project totals. Similarly we emphasize that energy use is modeled, and only gives an order of magnitude estimate.}
...
\centering
\scriptsize
\caption{Recovered model-resolved token measurements.  These are measured lower bounds for the five labels with
