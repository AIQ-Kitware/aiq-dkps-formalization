...
def main() -> None:
    ...
    exact_history_rows = [r for r in rows if r["accounting_state"] == "exact_measured"]
    total_lean_line_churn = sum(int(r["lean_additions"]) + int(r["lean_deletions"]) for r in rows)
    exact_lean_line_churn = sum(
        int(r["lean_additions"]) + int(r["lean_deletions"]) for r in exact_history_rows
    )
    exact_commit_share = len(exact_history_rows) / len(rows) if rows else 0.0
    exact_lean_line_churn_share = (
        exact_lean_line_churn / total_lean_line_churn if total_lean_line_churn else 0.0
    )

    # Keep the directly measured Git-churn proxy, and also provide an adjusted
    # formalization-text proxy that removes explicitly identified bulk vendor
    # lifecycle commits.  Intervening edits to vendored code remain in the
    # ordinary history and therefore remain counted.
    row_by_sha = {r["commit"]: r for r in rows}
    lean_churn_adjustment_rows = []
    adjusted_total_lean_line_churn = total_lean_line_churn
    adjusted_exact_lean_line_churn = exact_lean_line_churn
    for item in LEAN_CHURN_COVERAGE_ADJUSTMENTS:
        sha = str(item.get("commit") or "").strip()
        if not sha or sha not in row_by_sha:
            raise ValueError(
                f"Lean-churn coverage adjustment commit is absent from included history: {sha!r}"
            )
        row = row_by_sha[sha]
        churn = int(row["lean_additions"]) + int(row["lean_deletions"])
        exact_churn = churn if row["accounting_state"] == "exact_measured" else 0
        adjusted_total_lean_line_churn -= churn
        adjusted_exact_lean_line_churn -= exact_churn
        lean_churn_adjustment_rows.append(
            {
                "commit": sha,
                "label": str(item.get("label") or sha),
                "reason": str(item.get("reason") or ""),
                "lean_line_churn": churn,
                "exact_measured_lean_line_churn": exact_churn,
                "accounting_state": row["accounting_state"],
            }
        )
    if adjusted_total_lean_line_churn <= 0 or adjusted_exact_lean_line_churn < 0:
        raise ValueError("invalid adjusted Lean-line churn coverage denominator")
    adjusted_exact_lean_line_churn_share = (
        adjusted_exact_lean_line_churn / adjusted_total_lean_line_churn
    )
    lean_churn_adjustment_total = total_lean_line_churn - adjusted_total_lean_line_churn
    ...
    summary = {
        ...
        "coverage_proxies": {
            "exact_commit_share": exact_commit_share,
            "total_lean_line_churn": total_lean_line_churn,
            "exact_lean_line_churn": exact_lean_line_churn,
            "exact_lean_line_churn_share": exact_lean_line_churn_share,
            "lean_line_churn_definition": "sum of Lean-line insertions and deletions across included primary-repository commits",
            "lean_churn_adjustments": lean_churn_adjustment_rows,
            "lean_churn_adjustment_total": lean_churn_adjustment_total,
            "adjusted_total_lean_line_churn": adjusted_total_lean_line_churn,
            "adjusted_exact_lean_line_churn": adjusted_exact_lean_line_churn,
            "adjusted_exact_lean_line_churn_share": adjusted_exact_lean_line_churn_share,
            "adjusted_lean_line_churn_definition": "raw Lean-line churn less explicitly configured bulk vendor-lifecycle commits; intervening project edits remain counted",
        },
    ...
    def truncated_scientific(value: int | float) -> str:
    ...
    adjustment_table = [
        ...
        r"\toprule",
        r"Coverage proxy & Total Lean-line changes & Exact subset & Coverage \\",
        r"\midrule",
        fr"Raw Git churn & {format_int(total_lean_line_churn)} & {format_int(exact_lean_line_churn)} & {format_pct(exact_lean_line_churn_share)} \\",
        fr"Less: known non-LLM changes & $-{format_int(lean_churn_adjustment_total)}$ & -- & -- \\",
        r"\midrule",
        fr"Adjusted formalization-text proxy & {format_int(adjusted_total_lean_line_churn)} & {format_int(adjusted_exact_lean_line_churn)} & {format_pct(adjusted_exact_lean_line_churn_share)} \\",
        r"\bottomrule",
        r"\end{tabular}",
    ]
    (SNAPSHOTS / "lean_churn_coverage_adjustment_table.tex").write_text(
        "\n".join(adjustment_table) + "\n"
    )
    ...
    macros = [
        ...
        f"\\newcommand{{\\HistoryCommitCountScientific}}{{{truncated_scientific(len(rows))}}}",
        f"\\newcommand{{\\ExactMeasuredCommitCount}}{{{format_int(state_n('exact_measured'))}}}",
        f"\\newcommand{{\\ExactMeasuredCommitShare}}{{{format_pct(exact_commit_share)}}}",
        f"\\newcommand{{\\TotalLeanLineChurn}}{{{format_int(total_lean_line_churn)}}}",
        f"\\newcommand{{\\ExactMeasuredLeanLineChurn}}{{{format_int(exact_lean_line_churn)}}}",
        f"\\newcommand{{\\ExactMeasuredLeanLineChurnShare}}{{{format_pct(exact_lean_line_churn_share)}}}",
        f"\\newcommand{{\\LeanLineChurnAdjustmentTotal}}{{{format_int(lean_churn_adjustment_total)}}}",
        f"\\newcommand{{\\AdjustedTotalLeanLineChurn}}{{{format_int(adjusted_total_lean_line_churn)}}}",
        f"\\newcommand{{\\AdjustedExactMeasuredLeanLineChurn}}{{{format_int(adjusted_exact_lean_line_churn)}}}",
        f"\\newcommand{{\\AdjustedExactMeasuredLeanLineChurnShare}}{{{format_pct(adjusted_exact_lean_line_churn_share)}}}",
        f"\\newcommand{{\\ExactPrimaryBillableInputTokens}}{{{format_int(exact_on_history['billable_input_tokens'])}}}",
        f"\\newcommand{{\\ExactPrimaryOutputTokens}}{{{format_int(exact_on_history['output_tokens'])}}}",
        f"\\newcommand{{\\CommitsWithoutExactAccounting}}{{{format_int(summary['commits_without_exact_accounting'])}}}",
        f"\\newcommand{{\\CommitsWithoutExactAccountingScientific}}{{{truncated_scientific(summary['commits_without_exact_accounting'])}}}",
        ...
        f"\\newcommand{{\\LedgerTurnsScientific}}{{{truncated_scientific(measured_all['turns'])}}}",
