...
def generate() -> None:
    ...
    # Small tracked snapshots are the only theory-analysis outputs consumed by paper.tex.
    # They are regenerated from the checked-in compiler facts plus checked-in human taxonomy.
    coverage_pretty = {
        "focused_reusable_slice": "Focused reusable",
        "substantial_focused_subsystem": "Substantial focused",
        "substantial_reusable_subsystem": "Substantial reusable",
        "broad_partial_development": "Broad partial",
    }
    table = [
        "% Generated by scripts/build_formalization_theory_graph.py; do not edit by hand.",
        r"\begin{tabularx}{\textwidth}{@{}X l@{}}",
        r"\toprule",
        r"Theory area & Coverage in this work \\",
        r"\midrule",
    ]
    for r in basic:
        table.append(
            f"{latex_escape(r['theory'])} & "
            f"{latex_escape(coverage_pretty[r['coverage_level']])}" + r" \\"
        )
    table += [r"\bottomrule", r"\end{tabularx}"]
    (SNAPSHOTS / "formalization_basic_theory_table.tex").write_text("\n".join(table) + "\n", encoding="utf-8")

    audit_table = [
        "% Generated by scripts/build_formalization_theory_graph.py; do not edit by hand.",
        r"\begin{tabularx}{\textwidth}{@{}X l r r r r@{}}",
        r"\toprule",
        r"Theory area & Scope & YWS & DK & Decls. & Mods. \\",
        r"\midrule",
    ]
    for r in basic:
        audit_table.append(
            f"{latex_escape(r['theory'])} & "
            f"{latex_escape(coverage_pretty[r['coverage_level']])} & "
            f"{r['yws_units']} & {r['dk_results']} & "
            f"{r['reached_declarations']} & {r['reached_modules']}" + r" \\"
        )
    audit_table += [r"\bottomrule", r"\end{tabularx}"]
    (SNAPSHOTS / "formalization_basic_theory_audit_table.tex").write_text("\n".join(audit_table) + "\n", encoding="utf-8")

    cov_by_corpus = {r["corpus"]: r for r in coverage_rows}
    yws_cov = cov_by_corpus["YWS2015"]
    dk_cov = cov_by_corpus["DK1970"]
    root_decl_count = len({r["root_declaration"] for r in roots})
    yws_theory_count = sum(int(r["yws_units"]) > 0 for r in basic)
    dk_theory_count = sum(int(r["dk_results"]) > 0 for r in basic)
    snapshot_macros = [
        "% Generated by scripts/build_formalization_theory_graph.py; do not edit by hand.",
        f"\\newcommand{{\\FormalizationBasicTheoryCount}}{{{len(basic)}}}",
        f"\\newcommand{{\\FormalizationTraceDeclarationCount}}{{{len(nodes)}}}",
        f"\\newcommand{{\\FormalizationTraceEdgeCount}}{{{len(edges)}}}",
        f"\\newcommand{{\\FormalizationTraceModuleCount}}{{{len({r['module'] for r in nodes})}}}",
        f"\\newcommand{{\\FormalizationRootDeclarationCount}}{{{root_decl_count}}}",
        f"\\newcommand{{\\FormalizationYWSUnitCount}}{{{yws_cov['source_units']}}}",
        f"\\newcommand{{\\FormalizationYWSPublishedUnitCount}}{{{yws_cov['published_units']}}}",
        f"\\newcommand{{\\FormalizationYWSExtensionUnitCount}}{{{yws_cov['extension_units']}}}",
        f"\\newcommand{{\\FormalizationDKResultCount}}{{{dk_cov['source_units']}}}",
        f"\\newcommand{{\\FormalizationYWSTheoryCount}}{{{yws_theory_count}}}",
        f"\\newcommand{{\\FormalizationDKTheoryCount}}{{{dk_theory_count}}}",
    ]
    (SNAPSHOTS / "formalization_theory_macros.tex").write_text("\n".join(snapshot_macros) + "\n", encoding="utf-8")
