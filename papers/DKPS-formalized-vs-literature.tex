...
\code{DkpsQuench/Perfect}.  Seven open proof bodies remain after the
polynomial-cover theorem, deterministic covariance-floor transfer,
sample-centered quadratic-form bridge, rectangular spectral transfer, and the
first event-localized spectral-composition theorem were completed.  The count below is the live source count, not an estimate derived
...
\addlinespace
Spectral regularity & 0 & The coordinate weak laws, covariance algebra,
measurability, fixed-dimensional covariance event, entrywise-to-quadratic
transfer, exact sample-centered projection identity, rectangular
scatter--Gram eigenvalue bridge, target-augmentation floor, and the
high-probability spectral certificate are all proved. & Global floor and ceiling assumptions over every reference
...
\\
\addlinespace
Spectral composition & 2 & The central event-localized CMDS theorem is
proved by intersecting the response and spectral subevents with the deterministic
rate gate.  Two response-geometry adapters remain. & Global spectral facts
...
\item discharge the two remaining response-level spectral-composition adapters;
\item assemble the four finite, compact-infinite, and all-query capstones.
\end{enumerate}

The general mathematics behind the former first two frontier items (the
scatter--Gram transport and the augmentation floor) is now fully proved in the
imported \texttt{ForMathlib} layer: the nonzero-eigenspace equivalence between
$T^*T$ and $TT^*$ with equal multiplicities, the rectangular adjoint invariance
\texttt{LinearMap.singularValues\_adjoint}, the two-way transfer between
quadratic floors and codomain-Gram spectral floors, finite analysis/synthesis
and frame/Gram operators with the frame-bound correspondence, and the exact
add-one centered-scatter identity with its L\"owner and quadratic-form
corollaries.  The intrinsic singular system (explicit left singular vectors,
orthonormality on the nonzero subtype, reconstruction, and the codomain
orthonormal-basis extension) is proved as well.  The source
...
The rectangular spectral bridge is now proved from Mathlib's
\texttt{LinearMap.singularValues} API without a competing singular-value
definition and without new paper-facing hypotheses; the vendored snapshots were
used as cross-checks only, and no external code was copied into the build.
