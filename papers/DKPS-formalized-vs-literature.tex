...
positive nondegeneracy certificate by polarization.  The remaining work is to
transport this floor through sample centering, the rectangular scatter--Gram
spectral bridge, target augmentation, and the measurable high-probability event.
...
closed in \code{DkpsQuench.Perfect.CovarianceFloor}; the random covariance event
and the later scatter/Gram eigenvalue transfers remain.  The explicit response
...
\code{DkpsQuench/Perfect}.  Twelve open proof bodies remain after the
polynomial-cover theorem and the deterministic covariance-floor transfer were
completed.  The count below is the live source count, not an estimate derived
...
\addlinespace
Spectral regularity & 4 & The coordinate weak laws, covariance algebra,
measurability, fixed-dimensional covariance event, and
entrywise-to-quadratic transfer are proved.  The remaining proofs are the
reference scatter lower bound, the rectangular scatter--Gram eigenvalue bridge,
target augmentation, and construction of the high-probability spectral
certificate. & Global floor and ceiling assumptions over every reference
...
\begin{enumerate}
\item rewrite the reference empirical covariance quadratic form as the average
      of squared projections of the sample-centered reference configuration;
\item transport that feature-space floor to the first $d$ sorted eigenvalues
