...
\paragraph{Component flow.}
A target enters implementation after its source passage, inherited assumptions,
and intended scope have been recorded.  Once Lean accepts an implementation,
the evidence register maps the source target to the declaration used as its
formal evidence.  The checkers resolve that declaration in the pinned workspace
and regenerate compiler-derived types and consistency checks.  The audit packet
then combines those results with the source context and the claimed
correspondence.  Semantic-review decisions are written back to the register so
later model sessions inherit the target and review state without relying on a
chat transcript.
