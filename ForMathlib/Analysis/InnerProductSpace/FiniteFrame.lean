...
`ForMathlib.Analysis.InnerProductSpace.RectangularSingularValues`, this yields the two-way
correspondence between lower frame bounds and spectral floors of the Gram operator:

* `ForMathlib.le_eigenvalues_finiteGramOperator_of_forall_le_sum_sq`: a lower frame bound
  forces the first `finrank 𝕜 E` sorted Gram eigenvalues to be at least the bound;
* `ForMathlib.sum_sq_floor_of_le_eigenvalues_finiteGramOperator`: conversely, a spectral
