...
\title{Davis--Kahan 1970 in Hilbert space:\\
formalization-oriented proof distillation and finite-specialization roadmap}
\author{GPT 5.6 High}
\date{10 July 2026}
...
\begin{abstract}
This document distills the proof architecture of Davis and Kahan's
\emph{The Rotation of Eigenvectors by a Perturbation. III} (SIAM Journal on
Numerical Analysis 7 (1970), 1--46), together with the finite-dimensional
antecedents in Davis (1963). The source target is the paper's separable
Hilbert-space setting: a bounded Hermitian main body, arbitrary
unitary-invariant norms, and unbounded self-adjoint extensions with domain
conditions. It is written to support the Lean files under
\texttt{DavisKahan/}.
...
The maintained transcription states that the ambient space is separable,
that all four theorems apply in infinite and finite dimensions, and that the
bounds are given for arbitrary unitary-invariant norms. It takes bounded
Hermitian operators as the standing subject and treats unbounded self-adjoint
extensions under domain and bounded-residual conditions. These facts define
the completion target; the finite theory described throughout this note is a
specialization and proof laboratory.
...
The Lean development should follow the same dependency order. Definitions are
made on closed subspaces, projectors, and isometric embeddings rather than on a
chosen pair of coordinate bases. The finite-dimensional branch may reduce
spectral statements to orthonormal eigenbases and is retained as a valuable
weaker-foundation specialization. The canonical source-facing layer must not
...
\texttt{DavisKahan/Sources/Davis1963/RotationEnergy.lean} & total rotation, eigenvalue-motion subtraction, and
