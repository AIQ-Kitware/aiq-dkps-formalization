...
\title{Davis--Kahan 1970 in its full Hilbert-space scope: formalization distillation}
...
ambient Hilbert-space theory that is the default target of the repository.
The finite-dimensional development is a valuable specialization and alternate
proof surface; it is not the parent theory from which this document is an
optional extension. This note records theorem interfaces, hidden analytic
lemmas, proof decompositions, and the boundary between the bounded main body,
unitary-invariant operator ideals, and the unbounded passages. It intentionally
does not reproduce source text.

The checked-in distributable Davis--Kahan source specification records that the paper works on a
separable Hilbert space, takes bounded Hermitian operators as the standing
main-body subject, states all four headline theorems for infinite and finite
dimensions and arbitrary unitary-invariant norms, and extends the results to
unbounded self-adjoint operators under domain and bounded-residual conditions.
...
\section{Shared-core strategy}
The first canonical target is a bounded Hilbert-space core: projection algebra,
two-projection geometry, operator angles, graph subspaces, direct rotation, the
source Sylvester theorems, and the four angle families. These arguments are
basis free and should be specialized back to finite dimensions.

Finite-dimensional singular-value lists, arbitrary finite rectangular
unitarily invariant norms, Frobenius identities, eigenvalue-order
combinatorics, and statistical corollaries remain important. They may be
completed independently and retained as lower-dependency alternatives. They do
not discharge the corresponding Hilbert-space source obligation.

The full-paper roadmap must also include symmetric norm ideals and the
unbounded passages. It is acceptable to stage those after the bounded
operator-norm skeleton, but no intermediate finite or bounded-only milestone
should be reported as completion of the paper.

\section{Staged proof order}
The proposed milestones are:
\begin{enumerate}
\item projection algebra and the gap metric;
\item the identity $\sin\Theta=|P-Q|$ and graph representation of acute pairs;
\item planar sharpness and direct rotation;
\item the ordered semigroup proof of the Sylvester estimate;
\item ordered residual and perturbation $\sin\Theta$ and reflection
      $\sin 2\Theta$;
\item finite specialization of those operator-norm results;
\item clopen-spectrum or Riesz projections and norm-continuous continuation;
\item bounded Riccati equations, off-diagonal gap preservation, and tangent
      theorems;
\item arbitrary separated spectra with the $\pi/2$ constant;
\item symmetric ideals, compact singular-subspace theory, unbounded operators,
      and quadratic forms.
\end{enumerate}

\section{Why Borel calculus is not the first dependency}
Most isolated-gap Davis--Kahan statements need only a projection onto a
spectral component separated by positive distance.  The indicator of such a
component is continuous on the spectrum, so continuous functional calculus
or a Riesz contour is sufficient.  Full Borel functional calculus and strong
countable additivity remain important endpoints, but they should not block the
first operator-norm perturbation theorems.

\section{Finite specialization contract}
A later bridge should convert finite-dimensional linear maps to continuous
linear maps, identify the two projection APIs, and specialize operator-angle,
graph, direct-rotation, ordered $\sin\Theta$, and $\sin 2\Theta$ theorems.  It
should import only the bounded core, never the full unbounded or ideal stack.
...
\section{Unitary-invariant norm domains}
The paper does not globally assume compactness. It discusses compact singular
values, Ky Fan norms, and the noncompact case through spectral multiplicity,
while allowing some norm statements to become vacuous when the relevant norm
is not finite. A faithful formal API must therefore distinguish several
legitimate domains rather than replacing the source by a compact-operator
restriction:
\begin{itemize}
\item the operator norm on every bounded operator;
\item finite Ky Fan or approximation-number quantities where defined for
bounded noncompact operators;
\item compact and symmetric norm ideals with two-sided ideal bounds, pinching,
unitary invariance, direct sums, and singular-value gauges;
\item concrete trace-class, Hilbert--Schmidt, and Schatten instances;
\item any broader spectral-multiplicity formulation needed to match the
source's noncompact convention.
\end{itemize}
The compact/ideal theory is an important implementation layer, but compactness
must not become an unadvertised hypothesis of the source-facing theorem.
...
carrier is Mathlib's \code{LinearPMap}: an unbounded operator is a partial map,
and dense domain, closed graph and self-adjointness are hypotheses on it, not
fields of a record.  There is no bundled closed-operator structure anywhere in
the tree --- \code{TauCeti.DavisKahanExt.ClosedOperator} and its whole
forwarding API were deleted on 2026-08-28 --- and none must be reintroduced.
