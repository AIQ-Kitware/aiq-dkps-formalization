...
\item[Formalization target] \code{DavisKahan/Sources/YuWangSamworth2015.lean}, \code{AlignedBasis.lean}, \code{DavisKahan/Specialized/Statistics.lean}, and \code{DavisKahan/Specialized/SingularSubspace.lean}.
\end{sourceguide}
...
Principal angles & Singular values of $\widehat V^TV$, Frobenius sine distance, cross-complement identity. & \code{PrincipalAngles.lean}, \code{AlignedBasis.lean}, and the canonical subspace API used by \code{DavisKahan/Specialized/Statistics.lean}. \\
Population residual lower bound & $\Delta^2$ times cross-subspace energy is bounded by squared residual-column energy. & \code{DavisKahan/Sources/YuWangSamworth2015.lean}: \code{sq_gap_mul_sum_cross_le_sum_sq_norm_residualColumn} and \code{sq_gap_mul_sum_cross_le_of_population_gap}. Fully proved. \\
Operator-norm numerator & Residual bounded by $2\sqrt d\norm{E}_{\op}$. & \code{DavisKahan/Sources/YuWangSamworth2015.lean}: \code{sq_gap_mul_sum_cross_le_of_population_gap_opNorm}. Fully proved. \\
Sine theorem & Population-only gap bound for Frobenius principal-angle distance. & \code{DavisKahan/Sources/YuWangSamworth2015.lean}: \code{sqrt_sum_cross_le_of_population_gap}; \code{DavisKahan/Specialized/Statistics.lean}: \code{yuWangSamworth_sinTheta_le}, \code{yuWangSamworth_intervalBlock_le}. Fully proved. \\
...
Rectangular extension & Right and left singular subspace sine bounds. & \code{DavisKahan/Specialized/SingularSubspace.lean}: \code{rightSingularSubspace_sinTheta_le}, \code{leftSingularSubspace_sinTheta_le}, and \code{singularSubspace_dilation_sinTheta_le}. Fully proved. \\
...
\item \textbf{This note's result numbering conflicts with the rest of the repository and is not authoritative.} The \code{\textbackslash sourceanchor} lines here restart the counter per environment type, so they name the rank-one corollary \emph{Corollary 1}, the rectangular theorem \emph{Theorem 3}, and the appendix lemma \emph{Appendix Lemma 1}. \code{FinishYuWangSamworth} and every Lean declaration name use a single flat sequence --- Theorem 1, Theorem 2, Corollary 3, Theorem 4, Lemma 5 --- citing arXiv:1405.0680. The two agree only on Theorems 1 and 2. Neither can settle it from inside the repository, since the transcription is not distributed and the published article is not present. Recorded as gap \code{numbering-conflict} in \code{dev/yu-wang-samworth-2015-full-source-census.json}, which keys on the flat sequence because the Lean names do.
