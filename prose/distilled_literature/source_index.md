...
- **`DavisKahan1970` — The Rotation of Eigenvectors by a Perturbation. III**: Keep DavisKahan1970_part_III.tex mathematically faithful and in source order as the single distributable audit specification; keep its DK-CERT passage hashes and source-to-Lean registration synchronized. A private transcription may be used to re-audit the distillation but must never be required by repository checks.
