...
\item[Formalization target] The mathematical meaning of the pole-free tangent results: the coercive strip/exterior hypotheses, the Ritz residual, the directed graph/no-pole conclusion, the arbitrary-unitarily-invariant-norm theorem \code{davisKahan1970_tanTheta0_ritzResidual_le}, its strict-lower-rank generalization, and the lower-dependency per-vector theorem \code{tan_theta_le}.
...
\[
  ...
  \qquad(z\in Z).
\]
This implies $Z\cap V^\perp=\{0\}$ and hence the tangent graph condition.  This elementary operator-norm/per-vector theorem remains useful, but the repository now also proves the source-facing arbitrary-UI-norm Ritz-residual theorem through \code{davisKahan1970_tanTheta0_ritzResidual_le} and its strict-lower-rank generalization.
...
Tangent estimate & $\delta\|\tan\Theta\|\le\|R\|$ & Arbitrary rectangular UI-norm theorem \code{davisKahan1970_tanTheta0_ritzResidual_le}; elementary companion \code{tan_theta_le} \\
Lemma 2.1 & UI-norm triple-product estimate through singular-value majorization & Implemented through the singular-system witness and Ky Fan argument in \code{TanTheta/RitzResidual.lean} \\
Theorem 2 & Unequal-dimensional generalized tangent theorem & \code{davisKahan1970_generalizedTanTheta0_ritzResidual_le}, with the strict-lower-rank alternative and no public transversality premise \\
...
\item Keep the per-vector theorem as a low-dependency companion to the arbitrary-UI-norm Ritz-residual theorem.
...
\item Preserve the singular-system/Ky Fan proof as the source of the full UI-norm statement and document the directed tangent convention explicitly.
\item Keep the no-pole/transversality conclusion separate enough to be reused by generalized trial-map developments.
\end{enumerate}
