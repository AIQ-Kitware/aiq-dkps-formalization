...
\item[Formalization target] The Davis-specific endpoints in \code{ForMathlib/Analysis/InnerProductSpace/DavisKahanTheory/Davis1963.lean}, together with \code{RotationBound.lean}, \code{RotationSharp.lean}, and the unfinished canonical direct-rotation layer in \code{DirectRotation.lean}.
\end{sourceguide}
...
Section I, direct rotation & Canonical unitary from polar factors; intertwining; Frobenius and UI-norm minimality; block principal angles. & \code{DavisKahanTheory/DirectRotation.lean} contains the intended architecture but still has unfinished proofs. Theorem 1.1--1.3 are not yet fully represented by green public endpoints. \\
...
Theorem 3.2 & Total rotation plus matched eigenvalue motion is bounded by perturbation Frobenius energy. & \code{Davis1963.lean}: \code{totalRotation_add_eigenvalueMotion_le}; supporting result \code{rotation_add_displacement_le_hilbertSchmidt_intertwining}. Fully proved. \\
Theorem 4.1 & Eigenvalue motion dominates pinch energy minus off-diagonal energy under a perturbed-gap hypothesis. & \code{Davis1963.lean}: \code{diagonalPerturbation_sub_offDiagonal_le_eigenvalueMotion}. The repository explicitly corrects an earlier wrong off-diagonal hypothesis. Fully proved. \\
Theorem 3.3 & Total rotation bounded by twice off-diagonal perturbation energy. & \code{Davis1963.lean}: \code{totalRotation_le_two_mul_offDiagonal}; \code{RotationBound.lean}: \code{rotation_le_two_mul_offDiag}. Fully proved. \\
Theorem 5.1, sine form & Sharp product inequality equivalent to $\sin2\theta$. & \code{Davis1963.lean}: \code{sinTwoTheta_eigenvector_product_le}; \code{RotationSharp.lean}: \code{sin_two_theta_le_of_mem}, \code{sin_two_theta_le}; sharp model in \code{DavisKahanTheory/Sharpness.lean}. Fully proved. \\
Theorem 5.1, tangent form & Vanishing diagonal blocks imply the stronger $\tan2\theta$ estimate. & \code{Davis1963.lean}: \code{tanTwoTheta_eigenvector_product_le}; \code{RotationSharp.lean}: \code{tan_two_theta_le_of_mem}, \code{tan_two_theta_le}; sharp model in \code{DavisKahanTheory/Sharpness.lean}. Fully proved. \\
