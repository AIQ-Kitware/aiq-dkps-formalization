...
\item[Formalization target] \code{DavisKahan/FiniteDimensional/Sylvester/Internal/ReciprocalMultiplier.lean} and \code{Sylvester.lean}, especially the normalized finite reciprocal-Fourier interpolation seam and the sharp rectangular Ky Fan/UI-norm consequences.
...
The repository now supplies the algebraic part of the error-correction route in
\code{exists\_finite\_fourier\_interpolation}. Given arbitrary complex values
on a finite real set, it rescales the real nodes into an arc shorter than one
full circle, applies finite Lagrange interpolation to their distinct phases,
and reads the resulting polynomial coefficients as a finite Fourier sum. Thus
an approximation error can always be corrected exactly. The strengthened
theorem \code{exists\_finite\_fourier\_interpolation\_with\_mass\_bound}
fixes the interpolation nodes once and bounds the correction coefficient mass
linearly in the finite error vector. Consequently increasingly accurate
Fourier approximations have vanishing exact-correction cost.
...
Finite exact correction after Fourier approximation & Lagrange interpolation on finitely many distinct circle phases & \code{exists_finite_fourier_interpolation} and \code{exists_finite_fourier_interpolation_with_mass_bound}; the latter supplies a fixed linear coefficient-mass bound. \\
Approximation-to-exact reciprocal bridge & Correct the error on the finite difference set and concatenate the two Fourier sums & \code{HasApproximateFiniteReciprocalFourierInterpolation}, \code{hasFiniteReciprocalFourierInterpolation_of_approximate}, and \code{hasFiniteReciprocalFourierInterpolation_pi_div_two_add_eps_of_approximate}. \\
...
\begin{enumerate}[leftmargin=2em]
\item Derive uniformly accurate finite reciprocal Fourier sums, with coefficient masses approaching $\pi/2$, from the Sz.-Nagy--Strausz extremal theorem or a modern equivalent.
\item Apply the proved approximation-to-exact bridge to obtain the normalized exact finite interpolation. The Lagrange correction and its vanishing mass estimate are now formalized, so exact finite moment compression is no longer an independent obligation.
