...
The repository's theorem
\code{partIII_generalizedTanTheta_ritzResidual_uiNorm} is therefore a compiled
finite-dimensional strict-lower-rank specialization, not the full
Hilbert-space source theorem.  The source-faithful remaining hard step is the
finite-Ky-Fan approximation argument of the paper (and its unbounded cutoff
passage), followed by the already-available Fan-dominance promotion to an
arbitrary unitary-invariant ideal gauge.  The false symmetric formulation is
now guarded by a compiled counterexample in
...
\item The finite-dimensional arbitrary-unitarily-invariant-norm \code{tan Theta} Ritz-residual specializations are proved.  They must not be conflated with the still-open Hilbert-space Theorem 6.3 source endpoint, the all-unitarily-invariant-norm \code{tan 2 Theta} extension, or graph-operator APIs whose construction remains experimental.
