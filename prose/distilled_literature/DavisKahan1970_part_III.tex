...
A modern pole-free proof should not simply write $C^{-1}$ before proving that no angle equals $\pi/2$. Nakatsukasa and Motovilov clarify how the required graph property follows from finite-dimensional spectral placement.  The repository now exposes the source-facing arbitrary-unitarily-invariant-norm Ritz-residual theorem through \code{partIII_tanTheta_ritzResidual_uiNorm} and its strict-lower-rank companion, with transversality derived rather than assumed.  The elementary \code{tan_theta_le} theorem remains useful as a coordinate-free operator-norm proof with substantially lighter singular-value dependencies.
...
Section 6, tan theorem & Rayleigh--Ritz residual, source-oriented interval placement, equal-rank and strict-lower-rank alternatives & \code{partIII_tanTheta_ritzResidual_uiNorm}, \code{partIII_generalizedTanTheta_ritzResidual_uiNorm}, and the low-dependency \code{partIII_tanTheta_vector} companion \\
...
\item The arbitrary-unitarily-invariant-norm \code{tan Theta} Ritz-residual theorem is now proved.  This must not be conflated with the still-open all-unitarily-invariant-norm \code{tan 2 Theta} extension or with graph-operator APIs whose construction remains experimental.
