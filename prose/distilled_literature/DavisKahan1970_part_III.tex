...
\item[Current proved specialization] The source-facing finite API exposed through \code{DavisKahanTheory.PartIII}: the ordered and interval Sylvester roots; residual, generalized-trial-map, and perturbation \code{sin Theta}; the arbitrary-unitarily-invariant-norm Ritz-residual \code{tan Theta} theorem in both equal-rank and strict-lower-rank forms; \code{sin 2 Theta} for every unitarily invariant norm; and the sharp operator-norm \code{tan 2 Theta} theorem.  The elementary \code{partIII_tanTheta_vector} theorem remains as a low-dependency companion rather than the canonical source-facing tangent endpoint.
...
\item[Current boundary] The strongest proof-complete package is presently finite-dimensional over an arbitrary \code{RCLike} scalar field, with a partial bounded-operator branch. The finite package is a valuable specialization and weaker-foundation alternative, not the completion criterion for this paper.
