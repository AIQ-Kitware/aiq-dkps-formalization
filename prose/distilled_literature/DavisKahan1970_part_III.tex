...
\item[Atomic completeness denominator] The 49 claim blocks are source-order organizational groups, not a claim that the paper has only 49 mathematical assertions.  \code{dev/davis-kahan-1970-source-atom-inventory.json} enumerates the finer mathematical atoms contained in those groups, including every numbered equation, distinct theorem conclusion, counterexample, sharpness/scope assertion, and established mathematical clause that occurs inside a mixed open-question passage.  Completion is ultimately judged against those atoms, not merely the 49 group labels.
