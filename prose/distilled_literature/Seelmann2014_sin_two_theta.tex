...
In the classical Davis--Kahan ordered or convex-hull separation geometry, the corresponding Sylvester inverse has norm $1/d$, so the factor $\pi/2$ is replaced by $1$. The finite specialization API should preserve both results, while the source-facing Davis--Kahan project retains the Hilbert-space theorem as its canonical target:
