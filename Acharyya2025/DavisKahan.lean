...
theorem inner_eigenvector_map_sub_eq
    ...
    (i j : Fin n) :
    -- Conclusion: ⟪uᵢ, (S − T) ûⱼ⟫ = (λ̂ⱼ − λᵢ) · ⟪uᵢ, ûⱼ⟫ (cross-term identity).
...
/-- Internal helper (total cross-energy bound, Parseval step).
With the operator-closeness hypothesis `∀ x, ‖(S − T) x‖ ≤ ε ‖x‖`, the sum over
all eigenvector pairs of the squared off-diagonal entries of `S − T` is at most
`n ε²`. -/
theorem sum_inner_map_sq_le
    ...
    -- Conclusion: ∑ᵢⱼ ⟪uᵢ, (S − T) ûⱼ⟫² ≤ n·ε² (total off-diagonal energy).
    ∑ i : Fin n, ∑ j : Fin n,
...
This is the eigenvector-perturbation tool the paper invokes (prose: "Davis Kahan
...
theorem sum_cross_inner_sq_le
    (hT : T.IsSymmetric) (hS : S.IsSymmetric)   -- both operators self-adjoint
    (hn : finrank ℝ E = n)                       -- finite-dimensionality (implicit in the paper)
    (d : Nat)                                    -- split index: leading block `< d`, trailing `≥ d`
    {gap : ℝ} (hgap_pos : 0 < gap)               -- eigengap is strictly positive (separation)
    (hgap : ∀ i j : Fin n, (i : Nat) < d → d ≤ (j : Nat) →
      gap ≤ |hT.eigenvalues hn i - hS.eigenvalues hn j|)  -- gap separates leading T- from trailing S-eigenvalues
    {ε : ℝ} (hε : ∀ x : E, ‖(S - T) x‖ ≤ ε * ‖x‖) :        -- operator-norm bound: ‖S − T‖op ≤ ε
    -- Conclusion: total squared overlap (sin²Θ) of leading-T vs trailing-S eigenvectors ≤ n·ε²/gap².
