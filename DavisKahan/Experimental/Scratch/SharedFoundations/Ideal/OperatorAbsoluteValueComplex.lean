...
theorem spectra_absOp_eq_operatorAbs (T : E →L[ℂ] E) :
    absOp T = ForMathlib.operatorAbs T := by
  apply ForMathlib.operatorAbs_unique (T := T)
...
theorem DavisKahanExt.SymmetricNormIdeal.operatorAbs_mem_iff_and_gauge_eq
    ...
    (I.mem (ForMathlib.operatorAbs T) ↔ I.mem T) ∧
      (I.mem T → I.gauge (ForMathlib.operatorAbs T) = I.gauge T) := by
  let U : E →L[ℂ] E := polarIsometry T
  let J : E →L[ℂ] E := ContinuousLinearMap.id ℂ E
  have hTfactor : T = U ∘L ForMathlib.operatorAbs T ∘L J := by
    rw [ContinuousLinearMap.comp_id]
    exact (polarIsometry_comp_operatorAbs T).symm
  have hAbsfactor : ForMathlib.operatorAbs T = U.adjoint ∘L T ∘L J := by
    rw [ContinuousLinearMap.comp_id]
...
theorem RectangularSymmetricIdealFamily.operatorAbs_mem_and_gauge_eq
    ...
    N.Mem (ForMathlib.operatorAbs T) ∧
