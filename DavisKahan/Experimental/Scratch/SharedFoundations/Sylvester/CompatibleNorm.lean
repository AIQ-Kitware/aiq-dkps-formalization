...
theorem sub_le
  ...
  simpa [sub_eq_add_neg, N.map_neg T] using N.triangle S (-T)
...
theorem comp_le_mul
  ...
  have hLn : ‖Ln‖ ≤ 1 := by
    rw [Ln, norm_smul, norm_inv, norm_real, inv_mul_cancel₀ (norm_ne_zero_iff.mpr hL)]
  have hRn : ‖Rn‖ ≤ 1 := by
    rw [Rn, norm_smul, norm_inv, norm_real, inv_mul_cancel₀ (norm_ne_zero_iff.mpr hR)]
  have hcompat := N.compatible Ln T Rn hLn hRn
  have hfactor : L ∘L T ∘L R = ((‖L‖ * ‖R‖ : ℝ) : ℂ) • (Ln ∘L T ∘L Rn) := by
    ext x
    simp [Ln, Rn, hL, hR, mul_assoc, mul_left_comm, mul_comm]
  rw [hfactor, N.smul]
  calc
    ‖((‖L‖ * ‖R‖ : ℝ) : ℂ)‖ * N (Ln ∘L T ∘L Rn)
        ≤ (‖L‖ * ‖R‖) * N T := by
          simpa using mul_le_mul_of_nonneg_left hcompat (mul_nonneg (norm_nonneg L) (norm_nonneg R))
    _ = ‖L‖ * N T * ‖R‖ := by ring
...
theorem comp_left_le_mul
    ...
    (L : Y →L[ℂ] Y) (T : X →L[ℂ] Y) :
    N (L ∘L T) ≤ ‖L‖ * N T := by
  simpa using N.comp_le_mul L T (ContinuousLinearMap.id ℂ X)
...
theorem comp_right_le_mul
    ...
    (T : X →L[ℂ] Y) (R : X →L[ℂ] X) :
    N (T ∘L R) ≤ N T * ‖R‖ := by
  simpa using N.comp_le_mul (ContinuousLinearMap.id ℂ Y) T R
...
theorem theorem5_1_of_boundedLeftInverse
  ...
  have htri : N T ≤ N (L ∘L C) + N (L ∘L T ∘L B) := by
    rw [hLT]
    exact N.triangle _ _
  have hLC : N (L ∘L C) ≤ (gamma + delta)⁻¹ * N C := by
    exact (N.comp_left_le_mul L C).trans
      (mul_le_mul_of_nonneg_right hleft.norm_le (N.nonneg C))
  have hLTB : N (L ∘L T ∘L B) ≤
      (gamma + delta)⁻¹ * N T * gamma := by
    exact (N.comp_le_mul L T B).trans
      (mul_le_mul (mul_le_mul_of_nonneg_right hleft.norm_le (N.nonneg T))
        hB (norm_nonneg B) (mul_nonneg (norm_nonneg L) (N.nonneg T)))
  ...
  rw [mul_add, mul_assoc, mul_inv_cancel₀ hgd.ne', one_mul,
    mul_assoc, mul_inv_cancel₀ hgd.ne', one_mul] at hscaled
  nlinarith [N.nonneg T, N.nonneg C]
