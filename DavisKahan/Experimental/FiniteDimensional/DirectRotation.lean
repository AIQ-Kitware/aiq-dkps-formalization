...
This file develops the finite-dimensional formulas and extremal properties of
the canonical direct rotation.  The construction is the polar factor of

`P_V P_U + P_{Vᗮ} P_{Uᗮ}`.

The proof is organized by the canonical two-projection decomposition.  On the
common and doubly orthogonal summands the rotation is the identity.  On every
nontrivial principal plane it is the shorter planar rotation through the
principal angle.  The operator identities are therefore the functional-calculus
assembly of the corresponding scalar two-dimensional identities.  The
unitarily invariant norm statements follow from weak majorization of singular
...
/-- The partial complex structure on the nontrivial principal planes. -/
...
/-- Reversing the ordered pair reverses the canonical rotation. -/
...
/-- The stable direct rotation is precisely the polar factor of the canonical
intertwiner. -/
...
/-- Uniqueness among square roots of the reflection product with nonnegative
real part. -/
...
/-- Davis--Kahan Proposition 4.3: every unitarily invariant norm is minimized
on the positive displacement square by the direct rotation. -/
...
`π/3`, the direct rotation minimizes the unsquared displacement for every
...
/-- The canonical direct rotation minimizes maximal pointwise displacement. -/
...
/-- Davis--Kahan Proposition 4.2: the direct rotation minimizes the sum of
squared basis-vector displacements. -/
