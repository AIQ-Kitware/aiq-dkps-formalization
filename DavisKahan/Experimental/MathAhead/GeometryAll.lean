...
This aggregate deliberately excludes the Section 8 continuation chain and the
Sylvester finite-step campaign, so the Section 3 candidates can be compiled and
measured independently of the stale `SinTheta/General.lean` dependency.
-/
