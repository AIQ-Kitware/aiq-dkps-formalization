...
noncomputable def closedOperatorKernel
    (A : DavisKahanExt.ClosedOperator (𝕜 := ℂ) (E := H)) : Submodule ℂ H :=
  A.toLinearMap.ker.map A.domain.subtype
...
structure SobolevTraceFoundation where
  ...
  selfAdjoint :
    (DavisKahanExt.ClosedOperator.mk freeDomain freeFourth
      dense_freeDomain closed_freeGraph).IsSelfAdjoint
  ...
  affineKernelEquiv :
    EuclideanSpace ℂ (Fin 2) ≃ₗᵢ[ℂ]
      closedOperatorKernel
        (DavisKahanExt.ClosedOperator.mk freeDomain freeFourth
          dense_freeDomain closed_freeGraph)
  ...
  spectrum_nonnegative :
    (DavisKahanExt.ClosedOperator.mk freeDomain freeFourth
      dense_freeDomain closed_freeGraph).realSpectrum ⊆ Set.Ici 0
  ...
  positive_spectrum_characterization : ∀ lambda : ℝ,
    lambda ∈ (DavisKahanExt.ClosedOperator.mk freeDomain freeFourth
      dense_freeDomain closed_freeGraph).realSpectrum →
    0 < lambda →
...
theorem positive_spectrum_gt_five_hundred
    ...
    (hlambda : lambda ∈ D.operator.realSpectrum) (hpositive : 0 < lambda) :
    500 < lambda := by
...
theorem spectrum_subset_zero_union_Ioi_five_hundred
    (D : SobolevTraceFoundation (H := H)) :
    D.operator.realSpectrum ⊆ ({0} : Set ℝ) ∪ Set.Ioi 500 := by
