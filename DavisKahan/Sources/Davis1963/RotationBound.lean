...
theorem nonDegenerate_ofOrthonormalBasis {b b' : OrthonormalBasis (Fin n) 𝕜 E}
  ...
  simp only [OrthoProjFamily.ofOrthonormalBasis_proj, OrthonormalBasis.spectralProjection_singleton_apply]
    at hz hcontra
...
theorem intertwiningUnitary_apply_ofOrthonormalBasis {b b' : OrthonormalBasis (Fin n) 𝕜 E}
  ...
  have hPb : OrthonormalBasis.spectralProjection b {i} (b i) = b i := by
    rw [OrthonormalBasis.spectralProjection_apply_basis]
    simp
  have hP'b' : OrthonormalBasis.spectralProjection b' {i} (b' i) = b' i := by
    rw [OrthonormalBasis.spectralProjection_apply_basis]
    simp
  have hMb : (OrthonormalBasis.spectralProjection b' {i} ∘ₗ OrthonormalBasis.spectralProjection b {i}) (b i)
      = ⟪b' i, b i⟫_𝕜 • b' i := by
    rw [LinearMap.comp_apply, hPb, OrthonormalBasis.spectralProjection_singleton_apply]
  -- `|Mᵢ|` acts on `b i` as multiplication by the overlap size `‖c‖`
  have habs : abs (OrthonormalBasis.spectralProjection b' {i} ∘ₗ OrthonormalBasis.spectralProjection b {i}) (b i)
    ...
    have h2 := congrArg (fun f : E →ₗ[𝕜] E => f (b i))
      (abs_mul_self (OrthonormalBasis.spectralProjection b' {i} ∘ₗ OrthonormalBasis.spectralProjection b {i}))
    simp only [LinearMap.comp_apply] at h2
    rw [h2, LinearMap.adjoint_comp,
      (OrthonormalBasis.isPositive_spectralProjection b {i}).adjoint_eq,
      (OrthonormalBasis.isPositive_spectralProjection b' {i}).adjoint_eq]
    -- Pᵢ (P'ᵢ (P'ᵢ (Pᵢ (b i)))) = (c * conj c) • b i = ‖c‖² • b i
    rw [hPb, OrthonormalBasis.spectralProjection_singleton_apply, LinearMap.comp_apply, map_smul, hP'b',
      map_smul, OrthonormalBasis.spectralProjection_singleton_apply, smul_smul,
  ...
  · -- the block polar factor sends `b i` to the polar phase of the overlap times `b' i`
    have hinv : abs (OrthonormalBasis.spectralProjection b' {i} ∘ₗ OrthonormalBasis.spectralProjection b {i})
