...
section FixedCosineSubspace

/-! ### The fixed-cosine eigenspace of Proposition 3.5

This block is scalar-generic: it holds over any `RCLike` field, at arbitrary
dimension, with no completeness assumption.  It sits ahead of the complex
`OneSpace` section because nothing in it depends on that section, and because
`inner_starProjection_self_eq` is reused there.

The recorded obstruction to real scalars was `eigen_of_reducing_quadratic`,
whose old proof ran the complex polarization identity.  It is replaced by a
symmetric-operator argument that needs no complex structure; see that
