...
-- supplies the central-band layer -- `gapExterior`, `centralBandSubspace`, the
-- shifted-operator bounds and `opNorm_le_of_abs_re_inner_le` -- extracted from
-- this module.
...
Its companion `opNorm_le_of_abs_re_inner_le` now lives in
...
/-! ## 4. The central band, identified from the source spectral hypotheses
