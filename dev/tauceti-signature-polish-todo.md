...
| **Current file** | ForTauCeti/Analysis/InnerProductSpace/CourantFischer.lean |
| --- | --- |
| **Proposed disposition** | Refactor substantially before upstreaming. Keep generic basis-span lemmas separate from self-adjoint eigenvalue min-max results. |
| **Readiness** | Medium-low in current shape. |
| **Primary review risks** | specSubspace is not intrinsically spectral; misleading quantifier names; no single characteristic Courant-Fischer theorem; LinearMap/ContinuousLinearMap boundary. |
| **Likely PR slice** | PR A3, probably split into BasisSpan and CourantFischer files. |

#### P0  specSubspace

**Disposition: Rename and relocate**

```lean
noncomputable def specSubspace (b : OrthonormalBasis (Fin n) 𝕜 E) (p : Fin n → Prop) : Submodule 𝕜 E
```

**Proposed target shape**

```lean
noncomputable def OrthonormalBasis.spanIndices
    (b : OrthonormalBasis ι 𝕜 E) (s : Set ι) : Submodule 𝕜 E :=
  Submodule.span 𝕜 (b '' s)
```

- Rename: a predicate-selected span of an arbitrary orthonormal basis is not a spectral subspace.

- Generalize Fin n and predicate p to a finite index type and Finset/Set where useful.

- Place next to OrthonormalBasis rather than in a Courant-Fischer-specific namespace.

- Provide membership/repr support iff and orthogonal-complement API.

**Likely adversarial review:** The naming reviewer can correctly reject specSubspace because the definition contains no operator or spectrum.

#### P0  finrank_specSubspace

**Disposition: Rename / generalize**

```lean
theorem finrank_specSubspace (b : OrthonormalBasis (Fin n) 𝕜 E) (p : Fin n → Prop) [DecidablePred p] : finrank 𝕜 (specSubspace b p) = (Finset.univ.filter p).card
```

**Proposed target shape**

```lean
theorem OrthonormalBasis.finrank_spanIndices
    (b : OrthonormalBasis ι 𝕜 E) (s : Finset ι) :
    finrank 𝕜 (b.spanIndices s) = s.card
```

- Use the renamed basis-span object.

- Prefer Finset when cardinality is the conclusion.

- Make decidability explicit through Finset rather than DecidablePred p.

**Likely adversarial review:** The current theorem inherits the misleading spectral name and predicate encoding.

#### P0  re_inner_map_self_eq_sum_eigenvalues_mul_sq

**Disposition: Canonicalize eigenvalue API**

```lean
theorem re_inner_map_self_eq_sum_eigenvalues_mul_sq (hT : T.IsSymmetric) (hn : finrank 𝕜 E = n) (x : E) : RCLike.re ⟪T x, x⟫_𝕜 = ∑ i : Fin n, hT.eigenvalues hn i * ‖(hT.eigenvectorBasis hn).repr x i‖ ^ 2
```

**Proposed target shape**

```lean
theorem IsSymmetric.re_inner_apply_self_eq_sum_eigenvalues_mul_sq
    (hT : T.IsSymmetric) (x : E) :
    re ⟪T x, x⟫ = ∑ i, hT.eigenvalues ... i * ‖...‖²
```

- Move the theorem into the IsSymmetric namespace or use dot notation consistently.

- Hide hn if a FiniteDimensional canonical basis can avoid passing an equality finrank=n through every theorem.

- Check whether Mathlib already has the spectral expansion under a different name.

- Use apply rather than map in names for function application if that matches adjacent API.

**Likely adversarial review:** Reuse and signature reviewers will challenge the explicit hn threading and possible duplication of spectral decomposition.

#### P1  re_inner_map_self_le_of_mem_specSubspace

**Disposition: Rename / abstraction review**

```lean
theorem re_inner_map_self_le_of_mem_specSubspace (hT : T.IsSymmetric) (hn : finrank 𝕜 E = n) {p : Fin n → Prop} {c : ℝ} (hc : ∀ i, p i → hT.eigenvalues hn i ≤ c) {x : E} (hx : x ∈ specSubspace (hT.eigenvectorBasis hn) p) : RCLike.re ⟪T x, x⟫_𝕜 ≤ c * ‖x‖ ^ 2
```
...
theorem re_inner_apply_self_le_of_mem_spanIndices
    (hc : ∀ i ∈ s, eigenvalue T i ≤ c)
    (hx : x ∈ b.spanIndices s) :
    re ⟪T x, x⟫ ≤ c * ‖x‖ ^ 2
```

- Rename after the basis-span refactor.

- Package upper and lower bounds symmetrically.

- Consider a theorem over invariant submodules and spectral bounds rather than an index predicate.

**Likely adversarial review:** The current name treats a helper coordinate span as an intrinsic spectral subspace.

#### P1  le_re_inner_map_self_of_mem_specSubspace

**Disposition: Factor shared theorem**

```lean
theorem le_re_inner_map_self_of_mem_specSubspace (hT : T.IsSymmetric) (hn : finrank 𝕜 E = n) {p : Fin n → Prop} {c : ℝ} (hc : ∀ i, p i → c ≤ hT.eigenvalues hn i) {x : E} (hx : x ∈ specSubspace (hT.eigenvectorBasis hn) p) : c * ‖x‖ ^ 2 ≤ RCLike.re ⟪T x, x⟫_𝕜
```
...
theorem le_re_inner_apply_self_of_mem_spanIndices
    (hc : ∀ i ∈ s, c ≤ eigenvalue T i)
    (hx : x ∈ b.spanIndices s) :
    c * ‖x‖ ^ 2 ≤ re ⟪T x, x⟫
```

- Keep exactly parallel with the upper-bound theorem.

- If an order-bounds API exists for self-adjoint restrictions, derive both from it.

- Avoid duplicate private eigenbasis versions later in the same file.

**Likely adversarial review:** The file currently repeats the same upper/lower pattern for canonical and arbitrary eigenbases.

#### P1  exists_unit_vector_re_inner_le_eigenvalue

**Disposition: Polish signature**

```lean
theorem exists_unit_vector_re_inner_le_eigenvalue (hT : T.IsSymmetric) (hn : finrank 𝕜 E = n) (k : Fin n) (V : Submodule 𝕜 E) (hV : finrank 𝕜 V = (k : ℕ) + 1) : ∃ x ∈ V, ‖x‖ = 1 ∧ RCLike.re ⟪T x, x⟫_𝕜 ≤ hT.eigenvalues hn k
```

**Proposed target shape**

```lean
theorem exists_unit_mem_re_inner_le_eigenvalue
    (hV : finrank 𝕜 V = k + 1) :
    ∃ x : V, ‖x‖ = 1 ∧ re ⟪T x, x⟫ ≤ eigenvalue T k
```

- Use subtype x : V instead of x ∈ V as separate conjunct.

- Rename unit_vector to unit and express membership in the binder.

- Generalize exact dimension to k+1 ≤ finrank V if natural.

**Likely adversarial review:** Generality and API review will prefer a typed subspace witness and weaker dimension premise.

#### P0  forall_unit_vector_eigenvalue_le_re_inner

**Disposition: Rename**

```lean
theorem forall_unit_vector_eigenvalue_le_re_inner (hT : T.IsSymmetric) (hn : finrank 𝕜 E = n) (k : Fin n) : ∃ V : Submodule 𝕜 E, finrank 𝕜 V = (k : ℕ) + 1 ∧ ∀ x ∈ V, ‖x‖ = 1 → hT.eigenvalues hn k ≤ RCLike.re ⟪T x, x⟫_𝕜
```
...
theorem exists_submodule_forall_unit_eigenvalue_le_re_inner
    ...
    ∃ V : Submodule 𝕜 E,
      finrank 𝕜 V = k + 1 ∧
      ∀ x : V, ‖x‖ = 1 → eigenvalue T k ≤ re ⟪T x, x⟫
```

- Fix the name: the outer conclusion is exists, not forall.

- Use x : V to eliminate a separate membership hypothesis.

- This theorem should likely be a component of a max-min equality, not the headline.

**Likely adversarial review:** The current name demonstrably misstates its outer quantifier and is a direct naming-rubric failure.

#### P1  abs_eigenvalues_sub_le

**Disposition: Rename / choose headline**

```lean
theorem abs_eigenvalues_sub_le (hT : T.IsSymmetric) (hS : S.IsSymmetric) (hn : finrank 𝕜 E = n) {ε : ℝ} (hε : ∀ x : E, ‖(T - S) x‖ ≤ ε * ‖x‖) (k : Fin n) : |hT.eigenvalues hn k - hS.eigenvalues hn k| ≤ ε
```
...
theorem abs_eigenvalue_sub_eigenvalue_le
    (hε : ‖T - S‖ ≤ ε) (k : Fin n) :
    |eigenvalue T k - eigenvalue S k| ≤ ε
```

- Use singular eigenvalue in the theorem name.

- Prefer a norm premise over a pointwise bound when the operator norm is the natural object; make the pointwise version a helper.

- Consider the stronger Lipschitz theorem as the main public result.

**Likely adversarial review:** The current theorem takes a pointwise hypothesis and has a plural name for a pointwise conclusion.

#### P0  abs_eigenvalues_sub_le_opNorm

**Disposition: Change operator level**

```lean
theorem abs_eigenvalues_sub_le_opNorm (hT : T.IsSymmetric) (hS : S.IsSymmetric) (hn : finrank 𝕜 E = n) (k : Fin n) : |hT.eigenvalues hn k - hS.eigenvalues hn k| ≤ ‖LinearMap.toContinuousLinearMap (T - S)‖
```
...
theorem abs_eigenvalue_sub_eigenvalue_le_norm
    (hT : T.IsSymmetric) (hS : S.IsSymmetric) (k : Fin n) :
    |eigenvalue T k - eigenvalue S k| ≤ ‖T - S‖
```

- State the theorem over ContinuousLinearMap if the conclusion uses operator norm.

- Avoid exposing LinearMap.toContinuousLinearMap in the conclusion.

- Name suffix norm, not opNorm, if adjacent Mathlib uses norm for operators.

**Likely adversarial review:** The conversion in the public right-hand side is a sign the theorem is in the wrong operator type/namespace.

#### P1  eigenvalues_eq_of_eigenbasis

**Disposition: Polish**

```lean
theorem eigenvalues_eq_of_eigenbasis (hS : S.IsSymmetric) (hn : finrank 𝕜 E = n) (w : OrthonormalBasis (Fin n) 𝕜 E) {μ : Fin n → ℝ} (hμ : Antitone μ) (hw : ∀ i, S (w i) = (μ i : 𝕜) • w i) : hS.eigenvalues hn = μ
```

**Proposed target shape**

```lean
theorem IsSymmetric.eigenvalues_eq_of_hasEigenvectorBasis
    (hμ : Antitone μ)
    (hw : ∀ i, S (w i) = (μ i : 𝕜) • w i) :
    hS.eigenvalues ... = μ
```

- Name is mostly good, but “of_eigenbasis” should match actual local terminology.

- Check whether Antitone should be StrictAnti or sorted order with multiplicities.

- Hide finrank equality if possible.

**Likely adversarial review:** The reviewer will ask whether the basis/eigenvalue ordering hypothesis exactly characterizes the sorted sequence.

#### P0  eigenvalues_le_eigenvalues_of_re_inner_le

**Disposition: Order-based API**

```lean
theorem eigenvalues_le_eigenvalues_of_re_inner_le (hT : T.IsSymmetric) (hS : S.IsSymmetric) (hn : finrank 𝕜 E = n) (h : ∀ x, RCLike.re ⟪T x, x⟫_𝕜 ≤ RCLike.re ⟪S x, x⟫_𝕜) (k : Fin n) : hT.eigenvalues hn k ≤ hS.eigenvalues hn k
```

**Proposed target shape**

```lean
theorem IsSymmetric.eigenvalue_mono
    (h : ∀ x, re ⟪T x, x⟫ ≤ re ⟪S x, x⟫) (k : Fin n) :
    hT.eigenvalues ... k ≤ hS.eigenvalues ... k
```

- Use singular eigenvalue in a pointwise theorem name.

- If an operator order is available, provide eigenvalue_mono as the canonical theorem and derive the quadratic-form version.

- Consider hT ≤ hS rather than a forall-inner premise.

**Likely adversarial review:** The current name is long and encodes the proof hypothesis rather than the natural self-adjoint order.

#### P1  map_mem_specSubspace

**Disposition: Strengthen result shape**

```lean
theorem map_mem_specSubspace (hT : T.IsSymmetric) (hn : finrank 𝕜 E = n) (p : Fin n → Prop) {x : E} (hx : x ∈ specSubspace (hT.eigenvectorBasis hn) p) : T x ∈ specSubspace (hT.eigenvectorBasis hn) p
```

**Proposed target shape**

```lean
theorem invariant_spanIndices_eigenvectorBasis :
    MapsTo T (b.spanIndices s) (b.spanIndices s)
-- or a Reduces/InvariantSubmodule theorem
```

- Return the standard invariant/reducing predicate rather than only a pointwise membership lemma.

- Rename after removing specSubspace.

- Derive the pointwise application lemma from the structured predicate.

**Likely adversarial review:** The current theorem is too low-level for the characteristic API of a spectral span.

#### P1  orthogonal_specSubspace

**Disposition: Move / rename**

```lean
theorem orthogonal_specSubspace (b : OrthonormalBasis (Fin n) 𝕜 E) (p : Fin n → Prop) : (specSubspace b p)ᗮ = specSubspace b (fun i => ¬ p i)
```

**Proposed target shape**

```lean
@[simp] theorem OrthonormalBasis.orthogonal_spanIndices
    (b : OrthonormalBasis ι 𝕜 E) (s : Set ι) :
    (b.spanIndices s)ᗮ = b.spanIndices sᶜ
```

- Move to the basis-span namespace and use set complement.

- Mark simp only after checking orientation and classical decidability behavior.

- This is a characteristic theorem for the renamed definition.

**Likely adversarial review:** The result is valuable but currently placed and named as operator spectral theory.

> **Missing headline theorem**
>
> A file named CourantFischer should expose an actual min-max/max-min equality in a discoverable form. The current file mostly proves coordinate helpers and comparison corollaries. Before upstreaming, decide the canonical finite-dimensional theorem statement (submodule infimum/supremum, unit sphere, and eigenvalue ordering), then make the existing lemmas private or secondary API around it.
