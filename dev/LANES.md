...
| jon (window A) | Frontier/Section3.lean (OneSpace section: proposition3_1_positivity_characterization, proposition3_3/3_4/3_5, fixedCosineSubspace) | Props 3.1/3.3/3.4/3.5 acute direct-rotation cluster | 2026-07-23 | active — starting Prop 3.1. AUDIT FINDING: `IsPaperDirectRotation`'s diagonal-compression fields are numerical (`0 ≤ re⟪x,(PTP)x⟫`), which is strictly weaker than operator positivity and does NOT force uniqueness — finite-dim counterexample: on `U=V=H`, every scalar `exp(iθ)`, `|θ|<π/2`, satisfies all five fields yet ≠ 1. Minimal source-faithful fix = the diagonal compressions must be self-adjoint (the genuine cosine blocks are). Strengthening Prop 3.1 with two `IsSelfAdjoint` hypotheses (rather than touching the shared Core.lean def while window B is active). Do NOT touch Classification section (window B). |
