...
| edward (resumed) | Frontier/Section8.lean (ContinuationBridge section), Frontier/CircleContour.lean (new) | spectralContinuationWitness_of_circle, spectralContinuationWitness_of_circle_endpoints, selectedBranchProjectionLipschitzConstant_of_circle + the circle instantiation of PiecewiseC1ClosedContour/SpectralSeparatingContour (winding via scalar Cauchy). SinTheta touched read-only (imports). | 2026-07-23 | in progress — CircleContour.lean landed sorry-free (circleContour, winding = indicator, compactness margin, circleSeparatingContour). The three Section8 wire-ups are BLOCKED: SpectralContinuationWitness elaborates through ContinuationCore → DoubleAngle → SinTheta/General (broken, jon's queued lane); note ContinuationCore's DoubleAngle import is unused but DirectRotationAPI also routes through DoubleAngle, and DoubleAngle genuinely uses General.sinAngleOperator. Resumes when General elaborates. |
