...
| jon | MathAhead/HiddenFoundations/{Section3Nonacute,PolarIsometryFinal}.lean, Interop/Spectra/DirectRotation.lean (new normality/polar-real-part helpers), Frontier/Section3.lean | nonacute polar campaign: canonicalPolarFactor_sourceCompression_nonnegative, canonicalPolarFactor_crossed_blocks_general, crossedDefectEquivOfPaperDirectRotation, adjoint_polarIsometry, and downstream Proposition 3.2 promotion. Route: C=(1+R_V R_U)/2 is normal with C*C=Re C, so polar factor satisfies W+W*=2|C| ⇒ compressions = ⟪Px,|C|Px⟫≥0 and crossed blocks = (|C| commutes with P). Work in existing DavisKahan/vendor-Spectra architecture; declaration-level provenance for adapted Spectra material | 2026-07-23 | active |
