...
**Parameter range widened 2026-08-15.** The published middle-block model was formalized for `0 < eps < 1`; the restriction was an artifact of the first proof, not of the construction. What the model actually needs is `2 < 2 + eps < 5`, so that `2 + eps` is the SECOND level of the sorted sample spectrum and the perturbed block occupies the same ordered positions as the population block of threes. Every lemma is now proved for the full range `0 < eps < 3`, and that range is maximal: `card_middleSharpnessSample_level_three` shows that at `eps = 3` the level `2 + eps` merges with the leading level `5` and its multiplicity jumps from `d` to `p - d`, so the two blocks are no longer a corresponding eigenblock of dimension `d`; at `eps = 0` it merges downwards with the `2`s instead. Neither endpoint refutes the sharpness claim -- the construction simply stops describing what it claims to.
