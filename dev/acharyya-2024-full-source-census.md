...
### `continuous-mds-lp` — The continuum layer is formalized; Lemma 2 and Theorem 5 remain one measurability step short
...
WHAT IS NOW PROVED. The whole chain, in Acharyya2024.ContinuousMDS unless noted.
  ...
  The limit theorems. ae_tendsto_averaged_pointStress is the strong law for the averaged one-point stress; the null set it produces depends on the position, and tendsto_of_dense_of_equiLipschitz spreads convergence from a countable dense set to the whole space off a SINGLE null set, using abs_sub_pointStress_le, whose Lipschitz constant does not grow with the sample. tendsto_argmin_of_tendsto_of_equiLipschitz then moves the minimizers for a growing reference collection, and tendsto_outOfSampleExtension does the same for a fixed sample whose configuration moves. Together these give ae_tendsto_outOfSampleExtension_of_iid: the out-of-sample position converges almost surely to the population minimizer. That is the content the source attributes to its reference [23] rather than proving.

  The conclusions. tendsto_lpPairDistErr_of_ae_tendsto turns convergence at almost every pair into the L^p(P x P) conclusion by dominated convergence, with lpPairDistErr_eq_integral_prod the Fubini identity underneath. tendsto_lpPairDistErr_frameEmbedding assembles the chain for a fixed reference sample, along the full sequence and uniformly in p -- both stronger than the printed subsequence-and-per-p form. tendsto_measure_lpPairDistErr_gt is the corresponding result against the empirical model measure, proved earlier and independent of all this.
...
WHAT IS MISSING. One step, and it is measurability rather than mathematics. ae_tendsto_outOfSampleExtension_of_iid fixes the target model and yields a null set of sample paths depending on it; the L^p conclusion needs, for almost every sample path, convergence at almost every PAIR of models. TauCeti.ae_ae_of_forall_ae performs that exchange, but requires the failure set to be measurable in the product -- for a non-measurable set the two quantifier orders genuinely disagree. Discharging it for the out-of-sample map, which is defined by choice, means either a measurable selection or, following the convention this repository already applies to every estimator, taking the estimator's measurability as the disclosed implicit assumption the paper leaves unstated. The measurable-selection question is settled, and the answer is that no selection is needed. TauCeti.measurableSet_exists_isMinOn_le makes the one-stage minimizer event measurable -- being a minimizer is a sublevel condition on a measurable infimum, which measurable_iInf_of_isCompact supplies -- and TauCeti.measurableSet_tendsto_isMinOn assembles the convergence event from countably many of those. Since a convergence conclusion about minimizers is a statement about the minimizer SET, it never names a particular minimizer, so the event it describes is measurable outright. eventually_forall_isMinOn_dist_lt is the conclusion in that form: for every tolerance, eventually EVERY minimizer of the stage objective lies within it of the limit, not merely a chosen sequence of them, and ae_eventually_forall_isMinOn_of_iid is its almost-sure version for an iid reference sample. Nothing is selected anywhere in either statement, and the event they describe is the one TauCeti.measurableSet_tendsto_isMinOn shows measurable. What remains is to thread that through TauCeti.ae_ae_of_forall_ae, which exchanges the sample and model quantifiers, and tendsto_lpPairDistErr_of_ae_tendsto, which converts pointwise convergence into the L^p conclusion -- both proved.

CORRECTIONS MADE WHILE WORKING, recorded so the reasoning can be checked rather than trusted. First, an earlier version of this entry named a law of large numbers for V-statistics as the missing foundation. That was wrong: the population form integrates against P x P, not an empirical measure, so Fubini removes the double average entirely and no such law is needed. The V-statistic results that were built (TauCeti.map_evalPair_pi, TauCeti.integral_doubleSum_pi) are correct and reusable but are not on this critical path. Second, the out-of-sample map was first defined by augmenting one model at a time; that is the right object for a single target model, which is what the downstream augmented pipelines have, but it gives each model its own frame and so cannot serve an integral comparing two models. frameEmbedding replaced it.

ATTEMPTED AND BACKED OUT, recorded so the next attempt does not repeat it. The composition was drafted along the expectation route, which reaches the printed convergence-in-probability mode without exchanging quantifiers at all: the expectation of the L^p discrepancy over the sample is, by Tonelli, the double integral of the per-pair expectations; each vanishes by dominated convergence against the uniform bound; and Markov converts the vanishing expectation into convergence in probability. That route needs neither TauCeti.ae_ae_of_forall_ae nor a measurable selection, only joint measurability of the integrand, which is the estimator-measurability condition the repository discloses everywhere.

It did not compile. The obstruction is Lean engineering, not mathematics: the step deriving measurability of the per-pair expectation from joint measurability, via StronglyMeasurable.integral_prod_right' with the sample coordinate integrated out, sends the unifier into a loop that a million heartbeats does not clear. The likely cause is the let-bound abbreviation for the integrand; writing it out, or lifting it to a top-level definition so nothing unfolds through a local binding, is the thing to try next. The attempt was reverted rather than left in the tree.
