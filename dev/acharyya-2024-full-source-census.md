...
THE POPULATION CONCLUSION IS PROVED. tendsto_measure_lpPairDistErr_population gives Lemma 2's conclusion against the population measure, in the convergence-in-probability mode the source states, via the expectation route: The composition was drafted along the expectation route, which reaches the printed convergence-in-probability mode without exchanging quantifiers at all: the expectation of the L^p discrepancy over the sample is, by Tonelli, the double integral of the per-pair expectations; each vanishes by dominated convergence against the uniform bound; and Markov converts the vanishing expectation into convergence in probability. It needs neither TauCeti.ae_ae_of_forall_ae nor a measurable selection, only joint measurability of the integrand -- the estimator-measurability condition the repository discloses everywhere and which the printed statement needs for its own integral to denote anything.

A first attempt did not compile: deriving measurability of the per-pair expectation sent the unifier into a loop that a million heartbeats did not clear, because the integrand was a let-bound local abbreviation. Lifting it to the top-level definition pairDiscrepancy fixed that, and integral_prod_left -- which integrates out the FIRST coordinate -- was the right lemma where integral_prod_right' had been tried.
