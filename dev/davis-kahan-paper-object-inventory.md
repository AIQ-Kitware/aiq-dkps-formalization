...
| `sinAngleOperatorC` | **B** | DK-6.1-prop | The paper's *ambient* sine-angle operator. `sinAngleOperatorC_eq` characterizes it. Distinct from the directed `sinAngleOperatorC`. |
| `tanAngleOperatorC` / `…R` | **B** | S2-tan-theta | The paper's ambient tangent. `directedTanAngleOperatorC_eq_cfc` gives the functional-calculus form under transversality; `directedTanAngleOperatorC_eq_modulus_blockRepresentative` gives the block form. `…R` is defined by transport through complexification. |
| `sinTwoAngleOperatorC` / `…R` | **B** | S2-sin-two-theta, DK-8.2-thm | The paper's ambient `sin 2Θ`. `norm_sinTwoAngleOperatorC_eq_norm_directedSinTwoAngleOperatorC` relates it to the directed operator **in operator norm only**; the approximation-number identification is claimed for the directed operator, not this one. That is a real distinction: an ambient angle object carries each principal angle twice where the directed block carries it once. |
| `absTanTwoAngleOperatorC` / `…R` | **B** | S2-tan-two-theta | The paper's branch-free ambient `|tan 2Θ|`. `absTanTwoAngleOperatorC_eq_directedTanTwoAngleOperatorC` and `…_eq_modulus_tanTwoAngleOperatorC` are the characterizations; the branch-free form is the one definable with no quarter-acute hypothesis. |
| `Theorem61Data`, `RealTheorem61Data` | **C — replacement landed** | (no longer canonical) | Source-shaped data bundles for Theorem 6.1, each wrapping a further `UnboundedSinThetaData` record. Replaced as canonical evidence on 2026-08-31 by `theorem6_1_complex` / `..._real`, which take components. Retained as the implementation and compatibility API. |
| `Theorem62Data`, `RealTheorem62Data` | **C — replacement landed** | (no longer canonical) | As above; replaced by `theorem6_2_complex` / `..._real`. |
| `SymmetricSinThetaProblem`, `RealSymmetricSinThetaProblem` | **C — replacement landed** | (no longer canonical) | As above; replaced by `proposition6_1_complex` / `..._real`. The Challenge wrapper `sinTheta_wholeSpace_paperUINorm`, which had demonstrated the direct-hypothesis API, is now an alias of the production declaration. |
| `SinThetaRepresentativeAcross` | **A** | DK-6.1-thm, DK-6.2-thm | Not source vocabulary: it is the statement that two operators across different spaces have the same singular-value data, which is what lets a unitarily invariant norm be evaluated on either. Reusable as it stands. |
| `ContinuousLinearMap.hilbertSchmidtNorm`, `IsPaperHilbertSchmidt` | **B** | DK-6.2-thm | Theorem 6.2 is printed *for the Hilbert--Schmidt norm specifically*, with the source's lower-frame factor. The named object is what makes that printed specialization visible rather than hidden inside a general gauge. |
| `projectionBlock` | **A** | DK-6.1-lem | A projection block between two subspaces. `projectionBlock_eq_subtypeL_comp` identifies it with the ordinary composite; nothing about it is Davis--Kahan specific. |
| `blockCompression` | **A** | S2-tan-two-theta | Compression of an operator to a block. `blockCompression_complexify_equiv` transports it through complexification. Ordinary mathematics. |
| `diagonalPair` | **B** *(was D)* | DK-6.2-lem | `2·(pair) = R_U + R_Uᗮ` (`two_smul_diagonalPair_eq_add_reflections`), i.e. the pinching `T ↦ P_U T P_U + P_Uᗮ T P_Uᗮ`. Lemma 6.2 **is** the statement that this contracts every Ky Fan gauge, so the object is what the printed lemma is about, not a vehicle for proving something else. Class D said "never appears in a canonical theorem signature", and this one does; the D label was the error, not the signature. |
| `crossSineSum` | **C** *(was D)* | (no longer canonical) | `P_Uᗮ P_V + P_U P_Vᗮ`. Not a proof-only object either: it appeared in the conclusion of the real Proposition 6.1 theorem. It is a **representation adapter** — `crossSineSum_same_projectionDiff` gives it exactly the approximation-singular sequence of `P_V − P_U`, and it is used because there is no real continuous functional calculus here, so no literal real `sin Θ` operator exists to conclude on. As of 2026-08-31 the canonical real Proposition 6.1 concludes on the projector difference instead, through that transport, and this object no longer appears in any canonical signature. |
| `corollary3_2_nonacuteQuarterTurn` | **B** | DK-3.2-cor, DK-3.5-prop | The paper's quarter-turn used in Corollary 3.2's symmetry statement; source vocabulary for a specific printed construction. |
| `IsDirectRotation` | **B** | DK-3.2-prop | Definition 3.1's direct rotation, as printed. Kept deliberately distinct from stronger operator-positivity spellings: `proposition3_4_isDirectRotation_complex` was once accepted against this predicate and had to be repaired, because over `ℂ` its diagonal clauses do not force self-adjointness of the compression while Definition 3.1 asks for genuine positivity. `crossedDefectEquivOfDirectRotation` is the bridge to the crossed-defect data. |
...
| --- | --- |
| `projectionBlock Ω Γ K` | `compressionBlock` — the block of `K` from `Γ` to `Ω` |
| `blockCompression` | `compression` |
| `SinThetaRepresentativeAcross` | `SameSingularDataAcross` — it says nothing about sines |
