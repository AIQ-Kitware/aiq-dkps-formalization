...
- **Next action:** The real BICONDITIONAL.  Over `R` only the implication is landed (`real_directRotation_of_positiveDiagonalBlocks`); the converse needs `(U.starProjection * W * U.starProjection).IsPositive` for `directRotationR`, i.e. descending operator positivity through the complexification.  The complex converse is done (`complex_directRotation_iff_positiveDiagonalBlocks`).
