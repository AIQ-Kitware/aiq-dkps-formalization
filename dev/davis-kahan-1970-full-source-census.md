...
**THE AMBIENT HALF NOW HAS A REAL-SCALAR ENDPOINT, 2026-08-09 (Claude Opus 5).  THE DIRECTED HALF DOES NOT.**  `TauCeti.DavisKahan1970.tanTheta_ambient_bounded_paperUINorm_real_of_transversality` (`DavisKahan/Sources/DavisKahan1970/WholeSpaceReal.lean`, default build, `[propext, Classical.choice, Quot.sound]`) is the second printed conclusion `delta ||tan Theta|| <= ||H||` over a REAL Hilbert space of arbitrary dimension, for every `PaperUnitaryInvariantNorm`, with the membership conclusion supplied rather than assumed and with no constant lost.
...
* `TauCeti.DavisKahan1970.sinTwoTheta_ambient_bounded_paperUINorm_complex` and its Ky Fan core `TauCeti.DavisKahan1970.sinTwoTheta_ambient_bounded_kyFan_complex` (`DavisKahan/Sources/DavisKahan1970/SinTwoThetaWholeSpace.lean`, default build, `[propext, Classical.choice, Quot.sound]`).
...
- `TauCeti.DavisKahan1970.sinTwoTheta_unbounded_perturbation_arbitraryRepresentative_complex`
- `TauCeti.DavisKahan1970.tanTwoTheta_unbounded_residual_opNorm`
...
**SCOPE, AND IT MUST NOT BE OVERSTATED: THESE ARE CONDITIONAL ON `IsDoubleAngleEigenbasis`.  At `n = 1` the result is WEAKER than `tanTwoTheta_unbounded_residual_opNorm`, which needs no eigenvector.  What is new is `n >= 2`, which that theorem does not reach at all.  DO NOT CITE THESE AS THE UNCONDITIONAL ENDPOINTS.**  The honest-scope caveat is also in the module docstring (line 340), coordinator-read.
...
FOR THIS ROW: still open.  Its remaining axis is the REAL-scalar `tanTwoTheta_unbounded_residual_opNorm`; this work is complex-only.  The perturbation form `delta * N(tan 2Theta) <= 2 * N(H)` also remains untouched.
...
NAMED BASE-LAYER MISSION IF THE OWNER WANTS THE PROGRAMME: *conjugation-equivariant cyclic decomposition* -- run the complexification's countable cyclic decomposition with cyclic vectors drawn from the real copy, so each cyclic isometry intertwines `conjugation` on `E_C` with complex conjugation on `Lp C 2 mu`, where `BorelCalculus.map_ofReal_realSpectrumDiagMeasure` already puts mu on the real spectrum; deliverable, a real analogue of `exists_countable_isHilbertSum_lp_diagMeasure`.  **THIS IS AN ESTIMATE, NOT A FINDING**, and section 5 of the failed-directions doc records that estimates on this row have been false in BOTH directions.
...
- `TauCeti.operatorUnitaryEquiv_of_sameSpectralMultiplicity`
- `TauCeti.sameSpectralMultiplicity_of_operatorUnitaryEquiv`
...
**FOUR FRAMING ERRORS, ALL THE COORDINATOR'S.**  (i) "`CyclicDecomposition.lean`, `SeparableCyclic.lean` and `MultiplicityModel.lean` are ALL stated over `spectrum C a`" is FALSE for all three: only FOUR declarations across them genuinely mention the spectrum-indexed `L^2` layer (`exists_isHilbertSum_lp_diagMeasure`, `exists_linearIsometryEquiv_lp_diagMeasure`, `exists_countable_isHilbertSum_lp_diagMeasure`, `mulLp_eq_coordMulLp`); 20 of the other 21 are spectrum-free or binder-bound, and `MultiplicityModel.lean` is stated over `C` and `C x N` -- the subtype is already gone by then.  (ii) Orthogonality carries, but NOT for the stated reason: `TauCeti.isHilbertSum_comp_linearIsometryEquiv` never uses inner-product preservation; what needs the transport to be an EQUIVALENCE is the TOTALITY half.  (iii) INDEXING is not an obstacle at all -- the family is reused verbatim and `ha` is one witness shared across indices.  (iv) `embLpEquiv` is not an obstacle either: it is already fully field- and type-generic.
...
**(D) FOUR STATEMENTS IN THIS ROW ARE FALSE.  Coordinator-verified individually.**  (i) The `notes` "HISTORY" says `theorem3_1_spectralMultiplicity_classification` "remains `sorry`, and is vacuous besides".  BOTH ARE PROVED AND AXIOM-CLEAN; `Frontier/Section3.lean` contains ZERO `sorry` and the single hit in `Frontier/Core.lean` is the WORD inside a docstring at line 58 describing a PAST sorried definition.  (ii) The `notes` "NOTE ON EVIDENCE" says `twoProjection_operator_classification` is "deliberately NOT listed in this row's `lean_declarations`" because it "lives under `DavisKahan.Experimental.Frontier`, which no default target builds".  IT IS LISTED, and the module is `DavisKahan/Frontier/Section3.lean`, reachable from `DavisKahan/All.lean`, which IS in `defaultTargets`.  (iii) `scope_gap` says `OperatorUnitaryEquiv` hardcodes `C`; it is `RCLike`-generic, as this row's own `notes` record from the same day.  (iv) `Core.lean:81` cites "§5" of `dev/section3-multiplicity-plan-2026-08-06.md`; that file has no §5, and is a 57-line record marked "Status: completed" with an explicit warning not to use it as a work queue.
...
- `TauCeti.DavisKahan.Section4.sum_displacementAngleSineSq_ge_of_mem`
- `TauCeti.DavisKahan.Section4.tsum_displacementAngleSineSq_ge_of_mem`
...
- `TauCeti.DavisKahan1970.tanTwoTheta_unbounded_residual_opNorm`
- `TauCeti.DavisKahan1970.tanTwoTheta_unbounded_residual_div`
...
- `TauCeti.DavisKahan1970.Section8.theorem8_2_perturbationHalfGap_source`
- `TauCeti.DavisKahan1970.Section8.theorem8_2_residualHalfGap_source`
...
- `TauCeti.DavisKahan1970.Section8.theorem8_2_sinTwoTheta_perturbation_source`
- `TauCeti.DavisKahan1970.Section8.theorem8_2_sinTwoTheta_residual_source`
...
- `TauCeti.DavisKahan1970.Section8.theorem8_2_source`
...
* `Theta < pi/4` -- `theorem8_2_branch_source_maximalAngle_lt` (C, finite (1.5)) and dimension-free via `theorem8_2_branch_source_directed` composed with `maximalAngle_lt_pi_div_four_of_crossedDefects`; over R the named dimension-free composite `theorem8_2_branch_source_real_maximalAngle_lt_of_crossedDefects` already exists.

**THE THREE QUESTIONS THAT BLOCKED THE PROMOTION ARE ANSWERED.**  (1) The residual endpoints ARE the directed `Theta_0` statements: their conclusion is on `sinTwoThetaIdealBlock Q P = P_Q . P_{J_P(Qperp)}`, the repository's canonical `sin 2Theta_0` block, checked by hand in dimension 2 to be rank <= 1 with single singular value `sin 2theta` -- directed data, not doubled ambient data, constant 2 not 4.  (2) NO `FiniteDimensional` rides on any of the six `sin 2Theta` endpoints; it appears only on the packaged `theorem8_2_source` / `theorem8_2_source_real` and the `(1.5)`-finite `maximalAngle_lt` variants, and it is not a narrowing because the paper's own standing (3.5) (L961) suffices in any dimension via `CrossedDefectsEquivalent`.  **A COORDINATOR PREMISE WAS WRONG HERE: `theorem8_2_source` (COMPLEX) carries `[FiniteDimensional C H]` too, not only the real one -- coordinator-verified at `Section8SourceTheorem82.lean:556`.**  (3) Nothing printed is complex-only; the only asymmetries run the other way.
