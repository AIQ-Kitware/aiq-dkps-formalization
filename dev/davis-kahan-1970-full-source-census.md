...
- **Next action:** One bridge: prove `approximationNumber (genericCosineBlock U Vperp) = approximationNumber (P_U (1 - P_V) P_U)` under trivial elementary Halmos summands, so the realization composes with the classification by `:=`.  Then the remaining items are the `P(I-Q)P` versus `PQP` compact-object question and named real-scalar instantiations of the older complex-only declarations.
