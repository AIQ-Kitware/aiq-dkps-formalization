...
Gates: S1-block-residual (proved_in_build), S2-tan-theta (proved_in_build), S2-sin-two-theta (proved_in_build), S2-tan-two-theta (proved_in_build), S2-unbounded-scope (proved_in_build), DK-3.1-def (proved_in_build), DK-3.2-def (proved_in_build), DK-3.1-prop (proved_in_build), DK-3.4-prop (proved_in_build), DK-3.5-prop (proved_in_build), DK-4.1-prop (proved_in_build), DK-5.2-thm (proved_in_build), DK-5.1-lem (proved_in_build), DK-7-sin2-proof (proved_in_build), DK-7-tan2-proof (proved_in_build)
...
- **Next action:** One follow-up remains, and it buys only the paper's literal phrasing: for Theorem 3.1 stated with *spectral multiplicity functions* rather than with the unitary-equivalence class, build spectral multiplicity (measure class plus cardinal-valued multiplicity function) and prove Hahn--Hellinger, then bridge unitary equivalence to multiplicity equality. `SameSpectralMultiplicity` (Frontier/Core.lean) is still `def _ : Prop := by sorry`, so `theorem3_1_spectralMultiplicity_classification` remains vacuous and is not on this row's evidence path.
...
- **Next action:** Two separate obligations, and they are not the same size. (1) Prove the corrected finite-index frontier statement -- the production theorem `directRotation_minimizes_sum_sq_basis_angles` is the finite-dimensional analogue but uses a basis of the WHOLE space and the displacement-energy cost `norm (W b - b)^2`, not the angle cost `1 - (re <b, W b>)^2`, so it is not a specialisation and cannot simply be cited. (2) Settle the infinite-dimensional summability convention. Do NOT attempt the arbitrary-Finset form -- it is false, see notes.
