...
- **Next action:** Needs the Hahn-Hellinger multiplicity stack; Mathlib has none of it. Layer 1 (the cyclic isometry ||f(a)xi||^2 = int |f|^2 dmu_xi, which makes a cyclic subspace unitarily equivalent to L^2(mu_xi)) is proved in ForTauCeti/.../BorelCalculus/CyclicIsometry.lean. Remaining layers and the two formulations that must be REFUSED are in dev/section3-multiplicity-plan-2026-08-06.md. The mathematical content of Theorem 3.1 is already proved admission-free as twoProjection_operator_classification; what is missing is only the paper's literal multiplicity phrasing.
