...
- **Next action:** The two items in `scope_gap`: tie the four planar residuals to admissible `(A, B)` pairs so the equalities become sharpness of the THEOREMS rather than identities among matrices; and identify the block-sum angle operator with the direct-sum configuration's angle operator at the SUBSPACE level.  BOTH obligations this field used to name are discharged -- see `notes`, and note that one of them was MIS-STATED.
