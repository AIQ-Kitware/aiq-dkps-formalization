...
- **Next action:** Optional, not proof debt: remove the codomain-room hypothesis from `equation1_12` (see `scope_gap`).  The route is to run the attaining argument at index `nu'' = min nu (finrank W')` -- `rectangularKyFanSum_eq_minFinrank_of_minFinrank_le` says the prefix sum has already saturated there -- and then extend the resulting orthonormal `nu''`-tuple to a `nu`-tuple inside the finite-dimensional `W` with `Orthonormal.exists_orthonormalBasis_extension_of_card_eq`.  Nothing else on this row is open: (1.9)--(1.11), the Ky Fan biconditional, (1.12) and both forms of (1.13) are compiled and axiom-clean.
