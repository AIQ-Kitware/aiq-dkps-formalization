...
- **Next action:** Scalar axis only.  Both printed pieces are compiled.  Attempt the NATIVE `RCLike` route on `Section3Nonacute.lean` and the polar modules it names -- measured as containing no complex-specific tokens and no dependency on `DirectRotationSquare.lean` -- rather than descending the complex theorem, which the recorded M33 obstruction refutes.  Do NOT descend an arbitrary complex defect isometry by taking real parts; that route is refuted.
