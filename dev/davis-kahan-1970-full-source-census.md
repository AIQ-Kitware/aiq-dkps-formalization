...
**Items:** 50  
**Unique cited Lean declarations:** 1119
...
- `TauCeti.DavisKahan.ExactSinTheta.PaperTheorem61Data`
...
- `TauCeti.DavisKahan.ExactSinTheta.PaperUnitaryInvariantNorm.prefixGauge_le_of_all_kyFan_le_hetero`
- `TauCeti.DavisKahan.ExactSinTheta.paperKyFanNorm`
- `TauCeti.DavisKahan.ExactSinTheta.paperKyFanNorm_gauge`
- `TauCeti.DavisKahan.ExactSinTheta.paperKyFanNorm_extendedGauge`
- `TauCeti.DavisKahan.ExactSinTheta.all_kyFan_le_of_every_paperNorm_extendedGauge_le`
- `TauCeti.DavisKahan.ExactSinTheta.all_mul_kyFan_le_of_every_paperNorm_gauge_le`
...
- `DavisKahan1970.sinTheta_unbounded_formGap_paperUINorm_complex_ofRCLike_conforms`
- `DavisKahan1970.sinTheta_unbounded_formGap_paperUINorm_real_ofRCLike_conforms`
...
- `TauCeti.DavisKahan1970.paperSecantSquared`
- `TauCeti.DavisKahan1970.paperTanBlockRepresentative`
- `TauCeti.DavisKahan1970.paperTanBlockRepresentative_mul_self`
...
- `TauCeti.DavisKahanExt.paperTanAngleOperatorR`
...
- `TauCeti.DavisKahan1970.tanTheta_directed_bounded_arbitraryDimension_paperUINorm_real`
...
- `TauCeti.DavisKahan1970.tanTheta_directed_bounded_paperUINorm_real`
...
- `TauCeti.DavisKahan1970.tanTheta_directed_bounded_perturbation_paperUINorm_real`
...
**THE AMBIENT HALF NOW HAS A REAL-SCALAR ENDPOINT, 2026-08-09 (Claude Opus 5).  THE DIRECTED HALF DOES NOT.**  `TauCeti.DavisKahan1970.tanTheta_ambient_bounded_paperUINorm_real_of_transversality` (`DavisKahan/Sources/DavisKahan1970/AmbientReal.lean`, default build, `[propext, Classical.choice, Quot.sound]`) is the second printed conclusion `delta ||tan Theta|| <= ||H||` over a REAL Hilbert space of arbitrary dimension, for every `PaperUnitaryInvariantNorm`, with the membership conclusion supplied rather than assumed and with no constant lost.
...
the real-scalar axis and did not record that the ambient half `tanTheta_ambient_bounded_paperUINorm_complex_of_transversality` carries
...
**THE AMBIENT TRANSVERSALITY HYPOTHESIS IS GONE, 2026-08-10 (Claude Opus 5, coordinator-verified).**  `||sin Theta|| < 1` is NO LONGER A HYPOTHESIS of the ambient `tan Theta` endpoints: `tanTheta_ambient_bounded_paperUINorm_complex_of_crossedDefects` and `tanTheta_ambient_bounded_kyFan_complex_of_crossedDefects` derive it, with real twins in `DirectedReal.lean`.  This closes the audit finding at `dev/davis-kahan-1970-final-audit-2026-08-09.md:516-517`.
...
UNEXAMINED AND LIKELY THE SAME: whether the whole-space `tan 2Theta` and `sin 2Theta` endpoints carry the same removable hypothesis.  Candidates measured 2026-08-10: `tanTwoTheta_ambient_bounded_orderedForm_kyFan_complex` and `tanTheta_ambient_bounded_paperUINorm_complex_of_transversality` (`TanTwoThetaWholeSpace.lean:1426, :1495`), `sinTwoTheta_ambient_bounded_kyFan_complex` and `sinTwoTheta_ambient_bounded_paperUINorm_complex` (`SinTwoThetaWholeSpace.lean:350, :372`), and the branch-free pair in `TanTwoThetaAmbientBranchFree.lean:206, :259`.  If the same wiring applies it would close hypotheses on further rows.

**THIS ROW CONTRADICTED ITSELF, AND THE CONTRADICTION COST A MISSION'S FRAMING.  Recorded so the pattern is visible.**  On 2026-08-10 the coordinator scoped a mission from this row's `next_action`, which read `REAL SCALARS, DIRECTED HALF ONLY`.  The same row's `scope_gap` already said, in capitals, `THE REAL-SCALAR AXIS IS NOW CLOSED FOR BOTH HALVES`, and named `tanTheta_directed_bounded_paperUINorm_real` (`DirectedReal.lean:418`) as the closing declaration.  The coordinator had even cited that declaration in the brief as background while framing the endpoint as missing.  LESSON, general: these fields go stale INDEPENDENTLY, nothing cross-checks them, and NO GATE READS ANY OF THEM -- `probe_census_declarations.py` only checks the declarations a row NAMES.  Read `status`, `blocked_by`, `scope_gap`, `next_action` AND the tail of `notes` together before scoping from a row; when they disagree the newest dated statement wins and the others are the bug.

The mission was salvaged rather than wasted: it landed the two orientations that genuinely did NOT exist -- the real SPECTRAL orientation (`tanTheta_directed_bounded_spectralGap_paperUINorm_real`, the real twin of `tanTheta_directed_bounded_spectralGap_paperUINorm_complex`, with the spectral-to-form conversion done NATIVELY over `R` by `SpectralOrder.Real.upperFormBoundOn_top_of_spectrum_subset_Iic` and `lowerFormBoundOn_of_restriction_spectrum_subset_Ici`, so NO complexification was needed) and the real PERTURBATION companion, which existed at no scope.  The conjugation route the `next_action` prescribed was never needed: it had already been done by an earlier session and sits in `DirectedReal.lean:82-182`.
...
**Scope gap.** The COMPLEX endpoint covers only bounded gap intervals; the source and this project's own inventory attach half-infinite gap scope to this result, and only the real endpoint carries it. See the note for the exact missing theorem and the traced blocker.
...
- `TauCeti.DavisKahan1970.sinTwoTheta_directed_unbounded_addBounded_blockRepresentative_paperUINorm_real`
- `TauCeti.DavisKahan1970.sinTwoTheta_directed_unboundedReflectionResidual_blockRepresentative_paperUINorm_real`
...
* `TauCeti.DavisKahan1970.sinTwoTheta_ambient_bounded_paperUINorm_complex` and its Ky Fan core `TauCeti.DavisKahan1970.sinTwoTheta_ambient_bounded_kyFan_complex` (`DavisKahan/Sources/DavisKahan1970/SinTwoThetaAmbient.lean`, default build, `[propext, Classical.choice, Quot.sound]`).
...
* `TauCeti.DavisKahan1970.sinTwoTheta_directed_unbounded_addBounded_blockRepresentative_paperUINorm_real` and `TauCeti.DavisKahan1970.sinTwoTheta_directed_unboundedReflectionResidual_blockRepresentative_paperUINorm_real` -- at `PaperUnitaryInvariantNorm`, the SAME norm class as the real ambient half `sinTwoTheta_ambient_bounded_paperUINorm_real`;
...
- `TauCeti.DavisKahan.sharp_paperUnitaryInvariantNorm`
- `TauCeti.DavisKahan.sharp_paperUnitaryInvariantNorm_selectedBranch`
- `TauCeti.DavisKahan.paperFaithful_tanTwoTheta_uiNorm_real`
- `TauCeti.DavisKahan1970.tanTwoTheta_branchFree_bounded_finiteSubspace_paperUINorm_rclike`
...
- `TauCeti.DavisKahan1970.tanTwoTheta_branchFree_bounded_paperUINorm_complex`
- `TauCeti.DavisKahan1970.tanTwoTheta_branchFree_bounded_paperUINorm_real`
...
- `TauCeti.DavisKahanExt.paperTanTwoAngleOperatorC_nonneg`
- `TauCeti.DavisKahan1970.paperDoubleSecant`
- `TauCeti.DavisKahan1970.paperTanTwoBlockRepresentative`
...
- `TauCeti.DavisKahan1970.tanTwoTheta_ambient_bounded_orderedForm_paperUINorm_complex`
- `TauCeti.DavisKahanExt.paperTanTwoAngleOperatorR`
- `TauCeti.DavisKahanExt.complexify_paperTanTwoAngleOperatorR`
- `TauCeti.DavisKahan1970.tanTwoTheta_ambient_bounded_orderedForm_paperUINorm_real`
- `TauCeti.DavisKahanExt.paperAbsTanTwoAngleOperatorC`
- `TauCeti.DavisKahanExt.paperAbsTanTwoAngleOperatorC_nonneg`
- `TauCeti.DavisKahanExt.paperAbsTanTwoAngleOperatorC_eq_paperTanTwoAngleOperatorC`
- `TauCeti.DavisKahan1970.isUnit_one_sub_two_mul_paperProjectorDifference_sq_of_cos_two_ne_zero`
- `TauCeti.DavisKahan1970.paperAbsTanTwo_sq_mul_cos_two_sq`
...
- `TauCeti.DavisKahan1970.tanTwoTheta_ambient_bounded_branchFree_paperUINorm_complex_of_corner`
- `TauCeti.DavisKahan1970.tanTwoTheta_ambient_bounded_branchFree_orderedForm_kyFan_complex`
- `TauCeti.DavisKahan1970.tanTwoTheta_ambient_bounded_branchFree_orderedForm_paperUINorm_complex_of_poleExclusion`
- `TauCeti.DavisKahan1970.tanTwoTheta_directed_boundedResidual_branchFree_blockRepresentative_kyFan_complex`
- `TauCeti.DavisKahan1970.tanTwoTheta_directed_boundedResidual_branchFree_blockRepresentative_paperUINorm_complex`
- `TauCeti.DavisKahan1970.tanTwoTheta_directed_boundedResidual_blockRepresentative_spectralGap_paperUINorm_complex`
- `TauCeti.DavisKahan1970.paperTanTwoDirectedCornerR`
- `TauCeti.DavisKahan1970.paperProjectionBlock_complexifySubmodule_real`
...
**Notes.** UNBOUNDED SOURCE-SCOPE CLOSURE 2026-08-12: the remaining Section 2 scope advertised for infinite-dimensional unbounded self-adjoint operators is now compiler-verified. `tanTwoTheta_directed_unboundedResidual_blockRepresentative_paperUINorm_complex` and `tanTwoTheta_directed_unboundedResidual_blockRepresentative_paperUINorm_real` provide the complex and real directed residual endpoints; `tanTwoTheta_ambient_unbounded_blockRepresentative_paperUINorm_complex` and `tanTwoTheta_ambient_unbounded_blockRepresentative_paperUINorm_real` provide the complex and real ambient factor-two endpoints. These wrappers use only the printed ordered-gap, off-diagonal, reducing-subspace, and norm-domain data; pole exclusion/canonical cutoffs and the two-corner assembly remain internal. The result-only census therefore promotes S2-tan-two-theta to terminal exact at full printed scalar/dimension/unbounded/UI-norm scope.

SOURCE-FACING CLOSURE 2026-08-12: the Section 2 `tan 2 Theta` theorem now has exact source-facing wrappers for both printed conclusions and for both complex and real Hilbert spaces. `tanTwoTheta_directed_boundedResidual_blockRepresentative_spectralGap_paperUINorm_complex` upgrades the branch-free Section 7 directed Ky Fan estimate to every `PaperUnitaryInvariantNorm` and derives `cos (2 theta) != 0` internally from the printed ordered spectral gap and `H_0 = H_1 = 0`; it states the sharp residual conclusion `delta * N(tan(2 Theta_0)) <= 2 * N(R)` on the canonical directed projection-block representative. `tanTwoTheta_directed_boundedResidual_blockRepresentative_spectralGap_paperUINorm_real` transports this exact source statement to real Hilbert spaces using the same canonical complexification convention already used by `paperSourceDirectedAngleR`, while the residual norm is descended to the real projection block. The ambient half is independently closed by `tanTwoTheta_ambient_bounded_spectralGap_paperUINorm_complex` and `tanTwoTheta_ambient_bounded_spectralGap_paperUINorm_real`, with `paperAbsTanTwoAngleOperatorC_eq_modulus_paperTanTwoAngleOperatorC` connecting the branch-free positive representative to the literal signed functional-calculus tangent. Neither exact endpoint assumes a quarter branch, explicit pole exclusion, nor perturbed-Q-block spectral placement. A semantic search before adding the directed wrapper found no existing source-facing theorem with this exact signature hidden elsewhere; the nearest declarations were the explicit-`hcos` Ky Fan corner specialization, the graph-coordinate real branch-free theorem, and the already-closed ambient theorem.

INDEPENDENT AUDIT CORRECTION 2026-08-12: this row was downgraded from `compiled_exact`. The source theorem does not assume quarter-pole exclusion or perturbed-block spectral placement. The registered `tanTwoTheta_ambient_bounded_branchFree_orderedForm_paperUINorm_complex_of_poleExclusion` still takes `hcos` explicitly, while the graph-coordinate branch-free theorem has extra representation data. Those are useful compiled specializations, not yet the literal ambient Section 2 wrapper.
...
* `TauCeti.DavisKahan1970.tanTwoTheta_branchFree_bounded_paperUINorm_complex` -- ℂ;
* `TauCeti.DavisKahan1970.tanTwoTheta_branchFree_bounded_paperUINorm_real` -- ℝ.
...
during this campaign (`sinTwoTheta_ambient_bounded_paperUINorm_complex` 2026-08-08,
`tanTheta_ambient_bounded_paperUINorm_complex_of_transversality` 2026-08-09).  When a census row covers a
...
* `tanTwoTheta_ambient_bounded_orderedForm_paperUINorm_complex`:
...
`tanTwoTheta_ambient_bounded_orderedForm_paperUINorm_complex`), with the real form at `b967c62f`.  Re-verified here by
elaboration: `tanTwoTheta_ambient_bounded_orderedForm_paperUINorm_complex` concludes
...
* `TauCeti.DavisKahan1970.tanTwoTheta_ambient_bounded_branchFree_kyFan_complex_of_corner` and
  `TauCeti.DavisKahan1970.tanTwoTheta_ambient_bounded_branchFree_paperUINorm_complex_of_corner` -- THE REDUCTION.
...
`tanTwoTheta_ambient_bounded_orderedForm_paperUINorm_complex` are now DERIVED from the reduction, and the private
...
THE TARGET CHAIN is `kyFan k T_0 <= sum_{n<k} tan(arcsin a_n(S_0)) <= (2/delta) kyFan k R_0` on the TYPED directed corners `paperBlockCompression U^perp U`, never the ambient `unboundedReflectionTangent : H ->L[C] H` (which carries both adjoint corners and would risk double-counting the sharp factor 2).
...
- `TauCeti.DavisKahan.IsPaperDirectRotation`
...
- `TauCeti.DavisKahan.ExactSinTheta.paperHilbertSchmidt_sylvester_real_le_of_pairwiseSpectrumGap`
- `TauCeti.DavisKahan.ExactSinTheta.paperHilbertSchmidtEnergy_sylvester_le_of_pairwiseSpectrumGap`
- `TauCeti.DavisKahan.ExactSinTheta.paperHilbertSchmidt_sylvester_le_of_pairwiseSpectrumGap_direct`
...
- `TauCeti.DavisKahan.ExactSinTheta.paperOperatorNorm_sylvester_le_finrank_range`
...
**Scope gap.** NO SCOPE GAP REMAINS.  Closed 2026-08-10 (Claude Opus 5, coordinator-verified).  The trial-dimension axis closed earlier; the NORM-ABSTRACTION question that held this row is now MEASURED AND ANSWERED FAVOURABLY, and the complex `PaperUnitaryInvariantNorm` endpoint exists.

THE MEASUREMENT, three compiled pieces of evidence.  (1) `PaperUnitaryInvariantNorm` is SCALAR-AGNOSTIC DATA and its Fan-dominance law is `RCLike`-generic: `PaperUnitaryInvariantNorm.mul_gauge_le_of_all_mul_kyFan_le` (`SineTheta/Norms/UnitaryInvariantNorm.lean:325`) is quantified over `{K} [RCLike K]` and consumes ONLY `forall k, c * kyFanApproximationGauge k A <= kyFanApproximationGauge k B`.  So a paper norm measures a COMPLEX operator directly and the recorded cross-field transport trap is STRUCTURALLY UNREACHABLE on this route.  (2) The complex Ky Fan core supplies exactly that input at every prefix and every trial dimension: `theorem6_3_all_kyFan_core_infiniteTrial` (`TanTheta/Theorem63InfiniteTrial.lean:468`).  (3) The paper's norm class LITERALLY CONTAINS the Ky Fan norms -- `paperKyFanNorm k hk` (`Ideals/KyFanNorm.lean:210`) IS a `PaperUnitaryInvariantNorm` whose extended gauge is `kyFanApproximationGauge k`, which is how `all_mul_kyFan_le_of_every_paperNorm_gauge_le` recovers the family back.  THE TWO ABSTRACTIONS ARE EQUI-STRONG FOR THIS THEOREM: the ideal-family endpoint was never a narrowing, and the paper-norm endpoint is not one either.
...
- `TauCeti.DavisKahan1970.tanTheta_directed_bounded_paperUINorm_complex`
- `TauCeti.DavisKahan1970.tanTheta_directed_bounded_paperUINorm_real`
...
**CORRECTION TO THIS ROW'S OWN PREVIOUS `scope_gap`, which the coordinator wrote and which was WRONG.**  It named `PaperUnitaryInvariantNorm.prefixGauge_le_of_all_kyFan_le_hetero` as `exactly the bridge the real endpoint uses`.  MEASURED 2026-08-10: the real endpoint `tanTheta_directed_bounded_paperUINorm_real` NEVER INVOKES IT -- it uses `N.mul_gauge_le_of_all_mul_kyFan_le` (`DirectedReal.lean:448`, coordinator-verified by reading the line).  The `_hetero` variant is for operators with DIFFERENT CODOMAINS, whereas here both operands are `Z ->L E`; and it could not have been the bridge anyway, being unscaled (no `delta`) and concluding only a `prefixGauge` inequality rather than membership plus the gauge bound.
...
**PROPOSITION 6.1's COMMON-DOMAIN FORM NOW EXISTS** (item (c)): `PaperCommonDomainSymmetricSinThetaProblem` with `A` and `B` closed operators, self-adjoint in the domain-aware sense, sharing a domain, and a bounded `perturbation` representing `B - A` there.  Its conclusion is the bounded Proposition 6.1's, at every `PaperUnitaryInvariantNorm`.  IT IS A MACHINE-CHECKED RELAXATION, not an asserted one: `ofBounded` maps any `PaperSymmetricSinThetaProblem` into it with `domain_eq := rfl` and `perturbation_eq := rfl`, so no hypothesis is dropped and none added.  `perturbation_isSymmetric` is DERIVED from symmetry of `A` and `B` on the common domain plus density, NOT taken as a field -- making it a field would have strengthened the source hypotheses.
...
NARROWER GAP CREATED RATHER THAN CLOSED: there is no REAL common-domain Proposition 6.1.  The complex one is landed; the real sibling would follow `SymmetricReal.lean`'s pattern, substituting `real_unbounded_sylvester_kyFan` for `davisKahan1970_sylvester_complex` and stating the conclusion on `paperCrossSineSum`.
...
(ii) THE HARDER ONE, AND THE COORDINATOR'S BRIEF OMITTED IT ENTIRELY.  `symmetric_all_kyFan` and `result_every_unitarilyInvariantNorm` state their conclusions on `TauCeti.DavisKahanExt.paperSinAngleOperatorC`, defined at `DavisKahan/Geometry/Angle/PaperOperatorAngle.lean:131` as `cfc Real.sin (paperAngleOperatorC U V)` over `Submodule ℂ E`.  This is NOT binder editing and NOT a proof-body issue: the statement is UNSTATABLE over general 𝕜, because no real continuous functional calculus exists for this angle operator.  Those two statements therefore remain C, byte-for-byte unchanged, and the generic content is carried by new siblings `symmetric_all_kyFan_crossSineSum` / `result_every_unitarilyInvariantNorm_crossSineSum` on `paperCrossSineSum`, with `crossSineSum_paperMem_iff_and_gauge_eq` as the compiled dictionary against `V.starProjection - U.starProjection`.  This is the SAME split `SymmetricReal.lean` already argues for: a unitarily invariant norm sees only the singular-value sequence.

**THREE FALSE PREMISES IN THE COORDINATOR'S BRIEF, ALL RECORDED.**  (1) The brief suggested the census note about substituting `real_unbounded_sylvester_kyFan` might be describing an obsolete route; it was not, and the census note was correct.  (2) The brief said the sibling `CommonDomainTheorems.lean` `is already [RCLike 𝕜]`; only its DATA layer is -- `PaperCommonDomainTheorem61Data` sits inside a `section Complex`, with a separate real packaging.  The sibling had in fact already taken exactly the generic-data / per-field-endpoint split this mission ended up taking.  (3) The brief warned of several `local instance` declarations needing binder treatment; there is exactly one.  The predicted `Complex.I` / `conj = id` hazard did NOT materialize -- every proof body transferred with `inner_C -> inner_𝕜` and nothing else.
...
**ITEM (a) IS DISCHARGED AT COMPLEX SCALARS, 2026-08-10 (Claude Opus 5; coordinator-verified: build 9614 -> 9615 jobs, BUILD_EXIT=0 read from the echoed log line after waiting for it, zero errors, ZERO warnings from the new module, seven checks EXIT=0, all four ratchets byte-identical at 165/51/51/91).**  New module `DavisKahan/TanTheta/Theorem63UnboundedCompression.lean` (826 lines, 53 declarations, no `sorry`), plus an IN-PLACE refactor of `Theorem63TrialData.lean` that keeps `restrict`'s name and is definitionally identical field-by-field, so every downstream `rfl` lemma survives.
  ...
  `UnboundedCompressionTrialData` -- `compression : DKClosedOperator (H := Z)` UNBOUNDED and self-adjoint, `residual : Z ->L[C] H` bounded, mirroring `PaperCommonDomainSinThetaData`'s layout rather than a new shape
...
- `TauCeti.DavisKahan1970.theorem6_3_unbounded_infiniteTrial_ideal_exists_of_reducing_real`
- `TauCeti.DavisKahan1970.theorem6_3_unbounded_infiniteTrial_ideal_exists_real`
- `TauCeti.DavisKahan.ExactSinTheta.PaperCommonDomainSymmetricSinThetaProblem`
- `TauCeti.DavisKahan.ExactSinTheta.PaperCommonDomainSymmetricSinThetaProblem.perturbation_isSymmetric`
- `TauCeti.DavisKahan.ExactSinTheta.PaperCommonDomainSymmetricSinThetaProblem.forward_all_kyFan`
- `TauCeti.DavisKahan.ExactSinTheta.PaperCommonDomainSymmetricSinThetaProblem.reverse_all_kyFan`
- `TauCeti.DavisKahan.ExactSinTheta.PaperCommonDomainSymmetricSinThetaProblem.symmetric_all_kyFan`
- `TauCeti.DavisKahan.ExactSinTheta.PaperCommonDomainSymmetricSinThetaProblem.result_every_unitarilyInvariantNorm`
- `TauCeti.DavisKahan.ExactSinTheta.PaperCommonDomainSymmetricSinThetaProblem.ofBounded`
...
**AMBIENT CONSUMER ADDED 2026-08-12.**  The Appendix unbounded-compression machinery is now consumed by the Section 2 ambient tangent theorem itself, not only by directed Theorem 6.3.  `tanTheta_ambient_unboundedRitz_raw_paperUINorm_complex` and its real sibling preserve the Appendix case in which both `A_0` and `Lambda_1` may be unbounded, while the residual and perturbation norm remain bounded.  This closes the source-scope integration gap; the row's separate hostile atomization issue for equations (6.7)--(6.11) is unchanged.
...
- `TauCeti.DavisKahan1970.Section6Appendix.paperHilbertSchmidtEnergy_domain_projection_add_real`
...
**THE REMAINING AXIS: the RESIDUAL `sin 2Theta` at a general unitarily invariant norm, at the DIRECTED `Theta_0`, with the printed constant 2.**  The obstruction is MATHEMATICAL.  With `C` the `P`-to-`Pperp` block of `A + K`, the singular values of `C + C*` are those of `C` DOUBLED, so `N(C + C*) = 2 N(C)` in general -- the trace norm witnesses it.  Every route through `sinTwoTheta_ambient_bounded_paperUINorm_complex` must supply a comparison operator REDUCED BY `P`, hence block-diagonal, whose displacement from `A + K` is at best exactly `-(C + C*)`; with `N(C) <= N(R)` that gives constant FOUR, not TWO, and it is not improvable within that route.  Recovering the printed constant needs the singular-value identification of `sin 2Theta_0` with `sin 2Theta_1` -- the HALMOS GENERIC DECOMPOSITION.  Contrast the DIMENSION axis of this row, which turned out NOT to need that machinery.
...
**THE UI-NORM RESIDUAL ITEM IS DISCHARGED AT COMPLEX SCALARS ONLY, 2026-08-10 (owner's commits `4230e6e7`/`7400ba44`, coordinator-validated: build 9600 EXIT=0, six gates EXIT=0, baselines flat, all four ratchets flat, no `sorry` added, endpoints axiom-clean).**  `theorem8_2_sinTwoTheta_residual_source_all_kyFan` and `theorem8_2_sinTwoTheta_residual_source_paperUINorm` prove `delta * N(sin 2Theta) <= 2 * N(residual)` at every Ky Fan gauge and at every `PaperUnitaryInvariantNorm`.  Supporting: `reflectionOperator_comp_reflectionDefect`, `sinTwoTheta_directed_boundedResidual_blockRepresentative_kyFan_complex`, `sinTwoTheta_directed_boundedResidual_blockRepresentative_paperUINorm_complex`, `paperDiagonalPair_even_kyFan_eq_two_mul_of_same`.

**THIS ROW IS NOT COMPLETE, AND MUST NOT BE MARKED SO.  The campaign owner flagged this directly: "8.2 is not finished.  We have it on C not R."**  Coordinator-verified by reading the signatures: both new endpoints bind `{A K : H ->L[C] H}`.  There is NO `theorem8_2_sinTwoTheta_residual_source_real_paperUINorm` and no real all-Ky-Fan residual endpoint -- checked by enumerating every `theorem8_2_*` in the tree.  The asymmetry is exact and worth stating: the PERTURBATION half HAS its real arbitrary-UI-norm endpoint (`theorem8_2_sinTwoTheta_perturbation_source_real_paperUINorm`, axiom-clean), while the RESIDUAL half now has the complex one only.  `theorem8_2_sinTwoTheta_residual_source_real` exists but is not the arbitrary-UI-norm statement.
...
**THE REAL RESIDUAL UI-NORM ENDPOINT LANDED, 2026-08-10 (owner's commits `45851cb1` -> `f55b3f4d` -> merge `2225d1f7`; coordinator-validated after rebase: build 9601 EXIT=0, six gates EXIT=0, baselines flat, all four ratchets flat).**  `theorem8_2_sinTwoTheta_residual_source_real_paperUINorm` is axiom-clean and its signature is genuinely real -- coordinator-read: `{A K : E ->L[R] E}`, `{P Q : Submodule R E}`, at an arbitrary `ExactSinTheta.PaperUnitaryInvariantNorm`.

**THIS CLOSES THE ONE AXIS THIS ROW RECORDED, AT BOTH SCALARS.**  Hours earlier this row recorded the residual `sin 2Theta` at a general unitarily invariant norm as the single remaining item, with a MATHEMATICAL obstruction: every route through `sinTwoTheta_ambient_bounded_paperUINorm_complex` gave constant FOUR rather than the printed TWO, "not improvable within that route", and recovering the printed constant was said to need the Halmos generic decomposition.  **That obstruction is resolved and the constant is 2** -- the complex endpoints (`theorem8_2_sinTwoTheta_residual_source_all_kyFan`, `..._paperUINorm`) conclude `delta * N(sin 2Theta) <= 2 * N(residual)`, and the real endpoint above matches.  The coordinator's note from earlier the same day, that the residual half "now has the complex one only", is SUPERSEDED.
...
* `delta N(sin 2Theta) <= 2 N(H)` at every UI norm -- `theorem8_2_sinTwoTheta_perturbation_source_paperUINorm` (C, on `paperSinTwoAngleOperatorC Q P`, AMBIENT and correctly so: the print is ambient here) and `theorem8_2_sinTwoTheta_perturbation_source_real_paperUINorm` (R).
* `delta N(sin 2Theta_0) <= 2 N(R)` at every UI norm -- `theorem8_2_sinTwoTheta_residual_source_paperUINorm` and `..._all_kyFan` (C), `theorem8_2_sinTwoTheta_residual_source_real_paperUINorm` (R).
...
**THIS ROW'S `scope_gap` IS STALE IN THREE PLACES AND IS SUPERSEDED BY THIS ENTRY.**  The axis it calls "MATHEMATICAL ... not improvable within that route" is closed.  Its claim that "the real residual half reaches the operator norm, EXACTLY MATCHING the complex coverage" is wrong in both directions -- both scalars now reach `PaperUnitaryInvariantNorm`.  And its claim that recovering constant 2 "needs the HALMOS GENERIC DECOMPOSITION" is refuted by the landed proof, which uses a Ky Fan even-prefix pinch plus `reflectedDefect_complementaryBlocks_same` and `offDiagonalBlocks_same`.
