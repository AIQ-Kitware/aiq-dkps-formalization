...
- **Next action:** BOTH halves of the Section 2 tangent theorem are now proved at arbitrary complete COMPLEX Hilbert-space scope, arbitrary trial dimension, and every unitarily invariant norm: the directed half through Theorem 6.3 (`theorem6_3_perturbation_infiniteTrial`, `theorem6_3_infiniteTrial_spectral_exists`) and the ambient half through `tanTheta_wholeSpace_paperUINorm`.  The remaining gap on this row is the REAL-SCALAR lift, and it is the same shape for both halves: complexify the configuration, instantiate the complex result at `KyFanDominantIdealFamily.kyFan (C) k hk` for each `k`, and finish with the RCLike-generic `PaperUnitaryInvariantNorm.mul_gauge_le_of_all_mul_kyFan_le` plus `gauge_complexify`, using `complexifySubmoduleEquiv` to move the compression, the residual, and (for the ambient half) the projector difference `P_V - P_U` across.  Do NOT transport a scalar-specific `KyFanDominantIdealFamily`.  The unbounded scope is S2-unbounded-scope's, not this row's.
...
REAL SCALARS on the ambient half: not covered.  Expected to follow from
`PaperUnitaryInvariantNorm.gauge_complexify` plus the `FormTransport` lemmas,
exactly as `paperFaithful_tanTwoTheta_uiNorm_real` does for the directed half.
