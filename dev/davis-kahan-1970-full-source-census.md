...
- **Next action:** STALE TEXT CORRECTED 2026-08-10.  The bridge this field asked for IS PROVED -- `approximationNumber_genericCosineBlock_eq_ambient`, listed on this row, landed in commit 91724d75, and `corollary3_1_prescribedAngleSequence_classification` composes the two halves with no unproved step.  WHAT ACTUALLY REMAINS: (a) the same composition against `corollary3_1_realization_zeroMultiplicity`, where the elementary summands are DELIBERATELY nontrivial so the bridge as stated does not apply -- a multiplicity-aware version needs the ambient list to be the generic list merged with `1`s of multiplicity `dim (U inf Vperp)`; (b) a real-scalar instance of the composed corollary, blocked because `corollary3_1_compact_defectBlock_angleList_classification` is `C`-only while the realization is `RCLike`-generic; (c) the `P(I-Q)P` versus `PQP` compact-object question, untouched.
