...
**Scope gap.** None. The real/complex and directed/ambient scalar-scope obligations are compiled. The final Section 8 sentence asserting the extension to dim X(E_0) < dim X(F_0) is covered by the four strict-rank source-facing directed wrappers. Under that strict inequality the paper's ambient Hermitian angle Theta, whose construction uses the matched-dimension condition (1.5), is not available; accordingly the sentence extends the directed Theta_0 conclusion, as signaled by its comparison with Theorems 6.1 and 6.3, and does not require a Theta_0-to-Theta bridge.
...
- `TauCeti.DavisKahan1970.tanTwoTheta_branchFree_bounded_finiteSubspace_paperUINorm_real`
...
* `TauCeti.DavisKahan1970.tanTwoTheta_branchFree_bounded_finiteSubspace_paperUINorm_real` -- ℝ.
