...
REMAINING FOLLOW-UPS, neither blocking.  (1') Give `HalmosAngleDatum.ofIntertwinedAngles` a source-numbered consumer: a `theorem3_1_realization`-shaped wrapper stated from `(Theta_0, Theta_1, J)` rather than from a datum, which is the printed form of sentence (ii).  It should be a short `:=` on `theorem3_1_realization (HalmosAngleDatum.ofIntertwinedAngles ...)`.  THAT wrapper -- not the constructor -- is where `spectrum R Theta_i subset [0, pi/2]` belongs, so that `ker (sin Theta_i)` reads as the angle-`0` space and `ker (cos Theta_i)` as the angle-`pi/2` space.  (1'') THE REAL-MULTIPLICITY AXIS: execute **Route S**, bottom-up, one module per compiler checkpoint -- restate `BorelCalculus/DiagonalMeasure.lean` over `(ha : IsSelfAdjoint a)` with `spectrum R a` and `Measure R` symbols; `Polarization.lean` then COLLAPSES to the two-term real identity; then `Operator.lean` and `Multiplicative.lean`.  Land the datum change (`base : Measure R`, `level : N -> Set R`) only after `DiagMeasureMulLp` is through.  KEEP THE HILBERT-SPACE SCALAR AT `C` FOR THE WHOLE FIRST PASS -- generalizing it to `RCLike k` is a separate, purely mechanical second pass once the symbol field is real.  See `notes` for why this is net-negative work and why Route C is dead.  (2) Inhabit `SpectralMultiplicityFoundation` from the now-proved uniqueness (canonical Datum as a quotient by `measureClassSetoid`); parked with the promotion bookkeeping.
