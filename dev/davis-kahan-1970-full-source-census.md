...
WHAT IS STILL MISSING, and why this row is not `compiled_exact`: parts (i), (ii) and (iii) -- the compression, ordered-eigenvalue and symmetric-gauge repulsion refinements -- are proved as algebraic cores (`..._of_rotatedBlockData`, and `..._of_targetSplitting` in `DavisKahan/Frontier/Section8.lean`) but are NOT instantiated at the canonical branch just constructed.  Doing so is now a wiring job rather than a mathematical one: `Theorem81Conclusion` supplies the reducing branch and both form bounds that those data records want.
- **Next action:** Instantiate Theorem 8.1 parts (i)/(ii)/(iii) at `canonicalLowBranch`.  Part (i): feed `Theorem81Conclusion.branch_reduces` and the two form bounds to `upperCompressionRepulsionData_of_targetSplitting` / `lower...`.  Part (ii): `LinearMap.IsSymmetric.eigenvalue_mono` (ForTauCeti/Analysis/InnerProductSpace/CourantFischer.lean) on the compression inequalities.  Part (iii): the finite symmetric-gauge / majorization / Ky Fan layer -- do NOT substitute the operator norm.
...
**FIDELITY BUG FOUND AND THE ROW DOWNGRADED 2026-08-07 (Claude Opus 5).**  This row read `compiled_exact` / "Nothing outstanding" while the production source file it points at, `DavisKahan/Sources/DavisKahan1970/Section8/SourceSurface.lean`, says in its own module docstring that the complete historical Theorem 8.1 and Theorem 8.2 are NOT promoted.  Both statements cannot be true.  The source file is the one that is right: `theorem8_1_selectedBranch_and_spectralRepulsion` is an `alias` for `theorem81CoreConclusion`, which takes a `SpectralContinuationWitness` plus `hsmall`, `h0` and `h1` -- the branch selection, the contour smallness, and the spectral orientation are all supplied BY THE CALLER, and they are exactly what the paper proves.  Likewise `theorem8_2_perturbationHalfGap_selectedBranch` requires a `PerturbationHalfGapBridge`, whose field `contour_selects_quarter_branch` is the conclusion.  A declaration compiling says nothing about whether its hypotheses are the printed ones; that is the lesson this row records.
- **Next action:** Prove the perturbation alternative from `||H|| < delta/2` alone.  Recipe, verified against the available infrastructure on 2026-08-07 but NOT implemented: path `B(t) = A + H - t H` on `[0,1]` (t=0 perturbed, t=1 unperturbed); fixed circle centred at `(alpha+beta)/2` of radius `(alpha-beta+delta)/2`, whose real intersections are `beta - delta/2` and `alpha + delta/2`; contour invertibility for every `t` by Neumann perturbation of the INITIAL resolvent (base distance >= delta/2 gives resolvent norm <= 2/delta, and `||tH|| <= ||H|| < delta/2`) -- this replaces the non-source global projection-Lipschitz hypothesis `hquant`.  Then `circleRieszProjection_eq_boundedSelfAdjointSpectralProjection` identifies `Q(t)`, `continuous_circleRieszProjection_path` gives continuity of `f(t) = subspaceGap (Q t) (Q 0)`, and no crossing of `sqrt 2 / 2` follows from `sinTwoTheta_spectrum_operator` (DavisKahan/InfiniteDimensional/DoubleAngleSpectrum.lean): at a crossing `delta <= d * ||sin 2Theta|| <= 2 * t * ||H|| < delta`.  The one genuinely missing lemma is `||sinTwoAngleOperatorC U V|| = 1` when `subspaceGap U V = sqrt 2 / 2`; the graph-coordinate route is `gap = ||X|| / sqrt(1 + ||X||^2)`, so the quarter gap is `||X|| = 1`.  The residual alternative then needs Krein's self-adjoint contractive completion (`exists_selfAdjoint_completion_eq_norm_restriction`, via the Julia operator `L J_A L*` with `Gamma` from `ContinuousLinearMap.exists_contraction_of_gram_eq`), after which `A' := A + H - H'` reduces the residual alternative to the perturbation one.  `PerturbationHalfGapBridge` and `ResidualHalfGapBridge` may survive as internal conveniences but MUST NOT appear in the source-facing statement: their field `contour_selects_quarter_branch` is the conclusion.
