...
**Items:** 50  
**Unique cited Lean declarations:** 1412
...
| Then Theta <= pi/4 if and only if the chosen reducing blocks of A + H satisfy Lambda_1 >= alpha + delta and Lambda_0 <= alpha. | `theorem8_1_maximalAngle_le_iff_orderedFormGap_unbounded` is literally that biconditional, at the paper's inherited UNBOUNDED ambient scope, for an arbitrary reducing subspace `M` of `A + H`: `maximalAngle P M <= pi/4 <-> ((A + H) <= alpha in form on M) /\ ((A + H) >= alpha + delta in form on M-perp)`.  The placements are in the ordered form reading Davis and Kahan print: `Lambda_0` and `Lambda_1` are the blocks of `A + H` and the printed relations are operator inequalities on them.  Both directions are proved from the printed hypotheses; neither is supplied by the caller.  `..._real` is the same statement over a real Hilbert space.  The bounded spectrum-inclusion siblings `theorem8_1_maximalAngle_le_iff_spectrumIn` and `..._real`, canonical until 2026-09-06, are retained as registered specializations; the two readings are equivalent for a self-adjoint reducing block. | claimed_exact |
| For fixed A, P, H there exists a reducing projector Q with these properties (take the spectral projector of A + H on the appropriate side of alpha). | `theorem8_1_canonicalBranchUnbounded_printed` constructs it at unbounded scope: the witness is `canonicalLowBranchUnbounded (A + H) alpha`, which is `specRange (A + H) (Iic alpha)`, the genuine spectral subspace of the perturbed operator for the half-line below alpha, and the theorem concludes that it reduces `A + H`, carries `Lambda_0 <= alpha` and `Lambda_1 >= alpha + delta`, and satisfies the printed `Theta(P, Q) <= pi/4`.  `theorem8_1_eq_canonicalBranchUnbounded_of_maximalAngle_le` is the uniqueness half, so the branch is the ONLY reducing subspace inside the closed quarter turn.  The real siblings are `..._printed_real` and `..._of_maximalAngle_le_real`, with the branch given directly by `realSpecRange` rather than by transport.  The bounded `theorem8_1_canonicalBranch` and `..._real`, canonical until 2026-09-06, are retained as registered specializations. | claimed_exact |
| (i) The upper block obeys A_1 - alpha <= C_1 (Lambda_1 - alpha) C_1, with the analogous lower-block inequality. | `theorem8_1_upperCompressionRepulsion_unbounded` and its lower companion, at unbounded scope, in the form reading: the alpha-shifted energy of a domain vector is at most the alpha-shifted energy of its component in the branch's complement.  They hold on the WHOLE domain; the paper reads them on `P-perp`, where the `A + H` form equals the `A` form because `H` is fully off-diagonal, so the printed `A_1 - alpha <= C_1 (Lambda_1 - alpha) C_1` is the specialization.  Both are dimension-free.  The real siblings are `theorem8_1_upperCompressionRepulsion_unbounded_real` and its lower companion.  The bounded `theorem8_1_{upper,lower}CompressionRepulsion` and their real siblings are retained as registered specializations. | claimed_exact |
...
| The scalar field is real or complex. | Both fields are carried at every part.  The characterization, the existence half and the uniqueness half have real endpoints at unbounded scope (`theorem8_1_maximalAngle_le_iff_orderedFormGap_unbounded_real`, `theorem8_1_canonicalBranchUnbounded_printed_real`, `theorem8_1_eq_canonicalBranchUnbounded_of_maximalAngle_le_real`), proved by complexifying the operator, the perturbation and the subspaces, applying the complex theorem, and descending -- the form bounds by evaluating on the real copy, the angle by `subspaceGap_complexifySubmodule`, the branch identity by injectivity of the complexification of subspaces.  Parts (ii) and (iii) carry their own real endpoints. | claimed_exact |
