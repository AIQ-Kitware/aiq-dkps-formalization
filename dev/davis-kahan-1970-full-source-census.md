...
- **`compiler_pending`** -- Declarations and proofs are present, but this exact revision has not yet been checked by the Lean compiler.
...
Gates: S2-unbounded-scope (proved_in_build), DK-3.1-thm (proved_in_build), DK-3.5-prop (proved_in_build), DK-8.1-thm (proved_in_build), DK-9-model (proved_in_build)
...
**ASSESSMENT OF THE `_unequalDimension` DECLARATIONS THAT ARRIVED IN COMMIT d0cac626 (2026-08-10, Claude Opus 5, coordinator-measured).  THEY DO NOT CLOSE SUB-PROBLEM C, AND THE ROW SHOULD NOT BE READ AS IF THEY DO.**
...
**THE ANGLE-OPERATOR INTERTWINER IS NOW RCLIKE, AND THE DATUM DERIVES ITS INTERTWINING FIELDS, 2026-08-10 (Claude Opus 5, coordinator-verified independently: build 9589 EXIT=0, six gates EXIT=0, baselines flat, ratchets unmoved, and the whole chain elaborated at REAL scalars in a probe from outside the source tree).**

`TauCeti.LinearPMap.cfcHom_intertwines_selfAdjoint` is generalized in place from `C` to `RCLike k`, keeping its name, and gains a `cfc`-level corollary `TauCeti.LinearPMap.cfc_intertwines_selfAdjoint` that spares the caller inventing a common compact set.  Exactly ONE proof step needed changing: the Stone--Weierstrass `const` case, where `simp [Algebra.algebraMap_eq_smul_one]` hits max recursion depth and is replaced by the explicit `IsScalarTower.algebraMap_smul` step -- which is where the `IsScalarTower` hypothesis earns its place.  `symbolRestrict` and `continuous_symbolRestrict` needed NO change: they were already generic and the `C`-only theorem was consuming them at `k = R`.  MEASURED AND LEFT COMPLEX ON PURPOSE: `cayley_intertwines`, `cfcHom_intertwines`, `star_intertwines_of_mem_unitary`, `cfcHom_cayley_intertwines`.  `cayley` is built from the resolvent at `z = -i`, and `cfcHom_intertwines` is the STAR-NORMAL calculus, for which Mathlib supplies `IsStarNormal.instContinuousFunctionalCalculus` only over `C`.  Their live consumer `ForTauCeti/Analysis/InnerProductSpace/Rosenblum.lean:385` is unaffected.

`HalmosAngleDatum.ofIntertwinedAngles` (`DavisKahan/Geometry/Halmos/Realization.lean`) builds the datum from a single pair of self-adjoint angle operators and one intertwiner `J Theta_0 = Theta_1 J`.  TEN of the twelve fields are now PROVED rather than assumed -- the four `isSelfAdjoint_*` by `cfc_predicate`, `commute_0`/`commute_1` and `pythagoras_0`/`pythagoras_1` from `cfc_mul`/`cfc_add`/`cfc_one`, and `map_cos`/`map_sin` from the intertwiner lemma.  Only `isometry_on_sin_0` and `coisometry_on_sin_1` remain hypotheses, correctly: they are MULTIPLICITY statements and are invisible to a one-operator functional calculus.

**A COORDINATOR PREMISE WAS WRONG, AND IT IS THE SAME ERROR THIS ROW ALREADY RECORDED ONCE.**  The brief asked for the constructor to require `spectrum R Theta_i subset [0, pi/2]`.  IT IS NOT NEEDED AND IS NOT IN THE CONSTRUCTOR: none of the six derived axioms uses it -- `cos^2 + sin^2 = 1` and `J f(Theta_0) = f(Theta_1) J` hold on all of `R`, and `HalmosAngleDatum` records no nonnegativity anywhere.  This row had ALREADY recorded the same correction one level down for the realization half ("positivity of `C` and `S` ... is NOT used anywhere in the construction").  The confinement belongs on the source-numbered wrapper, where it makes the statement READ as the printed sentence; see `next_action`.

A SECOND, SMALLER PREMISE GAP: the hypothesis block copied from `OperatorModulus.lean` is correct but INCOMPLETE -- `[CompleteSpace E] [CompleteSpace F]` must be added, because without them there is no `Star (E ->L[k] E)` and `IsSelfAdjoint` does not typecheck.  Both instances were verified SYNTHESIZABLE at `k = R` and at `k = C`, so the generalization is not vacuous.

**THE REAL-MULTIPLICITY AXIS IS SCOPED, AND THE ANSWER IS NEITHER OF THE TWO ROUTES THE COORDINATOR PROPOSED, 2026-08-10 (Claude Opus 5, coordinator-verified).**  `TauCeti.OperatorUnitaryEquiv` and its five companions (`operatorUnitaryEquiv_of_intertwines`, `OperatorUnitaryEquiv.exists_intertwiner`, `.refl`, `.symm`, `.trans`) are now `RCLike`-generic -- pure binder work, zero consumer edits, all six axiom-clean and elaborating at `k = R` and `k = C`.

**THE DECISIVE MEASUREMENT: THE BOREL STACK PAYS FOR NORMALITY AND NOBODY BUYS IT.**  `TauCeti.BorelCalculus.exists_hasMultiplicityModel` takes `(ha : IsStarNormal a)` and has EXACTLY ONE call site in the repository -- `DavisKahan/Frontier/Core.lean:116` -- which reads `exists_hasMultiplicityModel hA.isStarNormal` from a hypothesis `(hA : IsSelfAdjoint A)`; upstream, `Section3.lean:2333` discharges it with `isSelfAdjoint_genericCosineBlock`.  Every consumer is self-adjoint.

**ROUTE S (SELF-ADJOINT REFOUNDATION) IS THE RECOMMENDATION, AND IT IS NET-NEGATIVE WORK.**  Do NOT move the Hilbert space's scalar field first.  Lower the SYMBOL field from `C` to `R` and the predicate from `IsStarNormal` to `IsSelfAdjoint`.  The stack then becomes generic in the Hilbert-space scalar for free, because all it needs is `ContinuousFunctionalCalculus R (H ->L[k] H) IsSelfAdjoint`, which synthesizes on BOTH sides: at `k = R` from this repository's `ContinuousLinearMap.instContinuousFunctionalCalculusRealIsSelfAdjoint`, and at `k = C` from Mathlib's `IsSelfAdjoint.instContinuousFunctionalCalculus`.  Route S DELETES rather than adds: the four-term complex identity `BorelCalculus/Polarization.lean:74 inner_polarization` (self-adjoint operators need only the two-term real one), the `re`/`im` symbol split at `DiagonalMeasure.lean:204-221`, and the `reCoord` push-forward at `PVM.lean:194-223`.  `PVM.lean` already concedes the point in source -- `reCoord (w : spectrum C T) : R := (w : C).re`, docstring "Its spectrum is real, so this is a bijection onto the spectrum and no Cayley detour is needed".  The target datum shape `Measure R` / `Set R` also matches `TauCeti.ProjValMeasure`, which ALREADY carries `diag : H -> Measure R` on a COMPLEX Hilbert space (`ProjValMeasure/Basic.lean:100`).

WHERE THE COMPLEX STEP ACTUALLY IS: NOT up the stack but at its IMPORT ROOT.  `BorelCalculus/DiagonalMeasure.lean:57` declares `attribute [local instance] IsStarNormal.instContinuousFunctionalCalculus` and first uses it at line 89; all 18 modules inherit from there, 12 repeating the attribute and 6 threading `IsStarNormal` as a hypothesis.  There is NO binder-bound prefix.  Only two root causes exist in the whole stack: that CFC (18/18 modules) and the four-term polarization (5 modules, all from `Polarization.lean:74`).  Checked and ABSENT: any use of algebraic closedness or spectrum-nonemptiness (zero hits for `IsAlgClosed`, `spectrum_nonempty`, `Gelfand`).  Stone--Weierstrass enters once, via `ContinuousMap.UniqueHom.eq_of_continuous_of_map_id` (`Restriction.lean:437`), which Mathlib supplies over `R` too.

**ROUTE C (COMPLEXIFY AND DESCEND) IS DEAD, AND `dev/` HAD ALREADY RULED IT OUT.**  The descent direction is CIRCULAR: getting a real unitary out requires building a real multiplication model, which is exactly what Route C was meant to avoid.  The intertwiner-level shortcut fails because an arbitrary complex unitary between complexifications need not commute with the conjugations; averaging `U + conj . U . conj` yields a conjugation-fixed intertwiner that can be NON-INVERTIBLE, and repairing it needs spectral projections in the commutant plus polar decomposition -- comparable in cost to Route S for a strictly weaker result.  **THIS WAS ALREADY ADJUDICATED AND THE COORDINATOR FAILED TO CITE IT**: `dev/davis-kahan-1970-completion-handoff.md` line 582 says "Do not descend an arbitrary complex defect isometry by taking real parts", line 591 gives the reason, and line 108 records the same call for reducing subspaces.  `dev/davis-kahan-1970-full-source-census.md:118` names the same obstruction for `DK-3.2-prop`.  Grep `dev/` for a recorded decision BEFORE scoping any mission on this axis.

MINOR DEFECT FOUND IN PASSING: `sameSpectralMultiplicity_iff_unitarilyEquivalent` (`Core.lean:144`) binds `(hB : IsSelfAdjoint B)` and never uses it -- its proof term is `<unitarilyEquivalent_of_sameSpectralMultiplicity A B, sameSpectralMultiplicity_of_unitarilyEquivalent A B hA>`.  Droppable.  Also, `MultiplicityDatum` has EIGHT fields, not the four usually quoted; `base_supported_level_zero` is load-bearing and its own docstring says uniqueness is false without it.

**ROUTE S AS THE COORDINATOR SPECIFIED IT IS REFUTED BY COUNTEREXAMPLE, 2026-08-10 (Claude Opus 5; the counterexample re-proved independently by the coordinator in Lean, EXIT=0).  NOTHING LANDED; the tree is unchanged.  This entry is the deliverable.**

**THE BRIEF WAS INTERNALLY CONTRADICTORY.**  It mandated "keep the Hilbert-space scalar field at `C` for this entire mission" and simultaneously asserted "for SELF-ADJOINT operators only the two-term REAL polarization identity is needed".  THOSE CANNOT BOTH HOLD.  On a COMPLEX Hilbert space the two-term sum recovers only `Re <psi, T xi>`, for ANY `T`, self-adjoint or not.  Counterexample, on `H = C` with the self-adjoint IDENTITY operator, `xi = 1`, `psi = I`: the two-term sum is `0`, while `<psi, T xi> = -I`.  Both halves machine-checked.  `BorelCalculus/Polarization.lean:79 inner_polarization` is therefore NOT removable machinery -- it is load-bearing for exactly as long as `H` is complex.

**THE TWO AXES ARE NOT SEPARABLE AND THE COORDINATOR HAD THE DIFFICULTY INVERTED.**  The brief called the `RCLike k` pass "a separate, purely mechanical second pass".  It is the opposite: the HILBERT scalar field is the mathematically substantive axis -- it is the ONLY thing that licenses the polarization collapse -- and the SYMBOL axis is the mechanical-but-enormous one.  `Complex.I` is structural in `pairVectors`, `norm_pair_sub_pair_le`, `norm_pair_le_crude` and `pair_smul_left`.

**STEP (1) COMPILES IN ISOLATION, AND THAT IS THE PROBLEM.**  `DiagonalMeasure.lean` was rewritten over `(ha : IsSelfAdjoint a)` with `spectrum R a` and real symbols, deleting the `IsStarNormal` CFC instance, the whole `section OfReal` (`ofRealLM`, `ofRealLM_apply`, `star_ofRealLM`) and the re/im split; it built EXIT=0 at 243 -> 190 lines.  But `integral_diagMeasure` then no longer covers `C`-VALUED SYMBOLS, which is precisely what every layer above it consumes.  Full `lake build`: EXIT=1, 59 errors in `Polarization.lean` (46), `DiagMeasureMulLp.lean` (7), `DiagMeasureNatural.lean` (6).  **BLAST RADIUS MEASURED FROM THE IMPORT GRAPH: 623 transitive dependents** (17 in `BorelCalculus`, 24 elsewhere in `ForTauCeti`, 297 in `DavisKahan`, plus `FinishTanTwoTheta`).  There is NO partial depth at which this is green, so "get through (1) and (2) with a green build" was never attainable.

**AND THE ROUTE DEAD-ENDS FOUR MODULES LATER ANYWAY.**  With `R`-valued symbols, `bddSymbols` (`CyclicModel.lean:111`) is at best an `R`-submodule and `symbolCalculus` (`:129`) at best `R`-linear, so the range of `cyclicIsometry` (`:241`) is a REAL subspace while `cyclicSubspace` (`CyclicIsometry.lean:103`) is a COMPLEX one, and `range_cyclicIsometry` (`:273`) becomes FALSE -- take `H = C`, `a = 1`, `xi = 1`: the cyclic subspace is `C`, the closure of `{f(a) xi : f real bounded}` is `R`.  `Multiplicative.lean:374 IsBddMeasurable.const_smul` and `:79 IsBddMeasurable.conj` fail the same way.

**A THIRD ROOT CAUSE EXISTS AND IT IS THE BINDING ONE.**  The earlier measurement recorded two (the star-normal CFC and the four-term polarization).  There is a third: `H` ITSELF IS COMPLEX, and the cyclic/multiplicity layer is complex-linear BY NECESSITY.  Also corrected: the re/im symbol split is NOT scaffolding inherited from the `IsStarNormal` choice -- RMK produces a measure from a REAL positive functional, so relating it to a complex symbol ALWAYS needs `re + i*im`.  It is intrinsic to any version keeping `C`-valued symbols.

**THE ONE ROUTE THAT SURVIVES, AND IT MUST BE SCOPED AS MULTI-MISSION.**  Lower the symbol DOMAIN to `spectrum R a` while KEEPING THE CODOMAIN `C`.  This needs a transported complex calculus `C(spectrum R a, C) -> (H ->L[C] H)`, built from `ha.spectrumRestricts` and Mathlib's `SpectrumRestricts.homeomorph : spectrum S a ~= spectrum R a` (`Mathlib/Analysis/CStarAlgebra/ContinuousFunctionalCalculus/Restrict.lean:34`, coordinator-verified present).  That deletes `reCoord` and the Cayley-free real-spectrum scaffolding downstream, KEEPS `bddSymbols` and `cyclicIsometry` intact, and is mechanical across the affected modules -- but it is ADDITIVE AT THE BASE and multi-mission in size, so it must not be scoped as a first checkpoint.  `dev/` was grepped: no recorded decision covers it, and the line-582 prohibition is about defect isometries and is untouched by this.

LINE CORRECTIONS TO THIS ROW'S OWN EARLIER ENTRY: `inner_polarization` is at `Polarization.lean:79`, not `:74`; Stone--Weierstrass enters at `Restriction.lean:439`, not `:437`.  RE-VERIFIED AND STILL CORRECT: `DiagonalMeasure.lean:57` is the import root; `exists_hasMultiplicityModel` has exactly one call site repo-wide; zero hits for `IsAlgClosed`, `spectrum_nonempty`, `Gelfand`; and `ContinuousFunctionalCalculus R (H ->L[C] H) IsSelfAdjoint` does synthesize.

**THE BASE LAYER OF THE SURVIVING ROUTE IS BUILT, 2026-08-10 (Claude Opus 5, coordinator-verified: build 9592 EXIT=0, six gates EXIT=0, baselines flat, probe from outside the source tree).**  `ForTauCeti/Analysis/CStarAlgebra/RealSpectrumFunctionalCalculus.lean`, 319 lines, one new module, nothing under `BorelCalculus/` touched.

`TauCeti.realSpectrumCfcHom (ha : IsSelfAdjoint a) : C(spectrum R a, C) -> A` transports the complex calculus to the REAL spectrum while KEEPING complex symbols and complex scalars.  Stated for a unital C*-algebra `A` over `C` with `ContinuousFunctionalCalculus C A IsStarNormal` -- it uses NO inner-product structure, so `H ->L[C] H` is merely an instance; that is why it lives under `CStarAlgebra/` and not `InnerProductSpace/`.  Supporting names, copied from source: `realSpectrumId`, `realSpectrumId_apply`, `realSpectrumHomeomorph`, `realSpectrumHomeomorph_eq`, `realSpectrumHomeomorph_apply_coe`, `coe_realSpectrumHomeomorph`, `realSpectrumHomeomorph_symm_apply_coe`, `realSpectrumCfcHom_realSpectrumId`, `realSpectrumCfcHom_algebraMap`, `continuous_realSpectrumCfcHom`, `realSpectrumCfcHom_injective`, `realSpectrumCfcHom_map_spectrum`, `realSpectrumCfcHom_isStarNormal`, `realSpectrumCfcHom_isSelfAdjoint`, `realSpectrumCfcHom_realSpectrumId_operator`.

**THE BRIDGES ARE THE DELIVERABLE**, since every existing consumer is phrased over `spectrum C a`: `realSpectrumCfcHom_apply` and `cfcHom_eq_realSpectrumCfcHom` give both directions against `cfcHom ha.isStarNormal`, and `realSpectrumCfcHom_ofReal_comp` additionally bridges to the REAL-symbol calculus `cfcHom ha` (needs `[ContinuousMap.UniqueHom R A]`, since it routes through `SpectrumRestricts.cfcHom_eq_restrict`).  Coordinator-probed: all three, plus the operator-instance form, elaborate and are axiom-clean.

FALSE PREMISES IN THE COORDINATOR'S BRIEF, all four confirmed.  (i) `ha.spectrumRestricts.homeomorph` DOES NOT ELABORATE: `IsSelfAdjoint.spectrumRestricts` states `SpectrumRestricts a (Complex.reCLM)` with a FUNCTION coercion while `SpectrumRestricts.homeomorph` takes `{f : C(S, R)}`, so higher-order unification fails; the working form is `SpectrumRestricts.homeomorph (f := (Complex.reCLM : C(C, R))) ha.spectrumRestricts`.  (ii) `IsStarNormal.instContinuousFunctionalCalculus` is a THEOREM, not a global instance -- the globally reachable route is `IsStarNormal.instIsometricContinuousFunctionalCalculus`, so `.../ContinuousFunctionalCalculus/Basic.lean` must be imported and `Instances.lean` does not import it.  (iii) **THE READINESS GATE NEVER READS `dev/tauceti/extraction-manifest.json`** -- module placement comes from the `TOPICS` table in `scripts/check_tauceti_roadmap_topics.py`; the manifest is read only by `export_for_tauceti.py`, `check_dependency_layers.py` and `refresh_tauceti_pr1_consistency.py`.  (iv) The ladder baseline of 100 was UNREACHABLE for any mission barred from `dev/`, because every new `ForTauCeti` module is an `off_ladder` finding until seeded in `RUNGS` and `--sync`ed.

**LADDER BASELINE MOVED, AND DOWNWARD: 100 -> 84.**  The coordinator seeded the module in rung G and ran `derive_tauceti_submission_ladder.py --sync`, which rewrote 22 sections to a tree total of 262.  The old document was stale at "205 modules" against 262, so the counts had been carrying a pre-existing error.  Verified after the sync: ZERO of the 84 residual findings names the new module; all 84 are pre-existing off-ladder modules, including `Analysis.InnerProductSpace.AngleGeometryBlockSum` from the same day's S2-sharpness work.

**NEXT MISSION IN THIS PROGRAMME, scoped from what was learned:** the gap between this base layer and what `BorelCalculus/` consumes is **continuous versus bounded-Borel symbols, not real versus complex**.  `bddSymbols a : Submodule C (spectrum C a -> C)` (`CyclicModel.lean:111`) carries `IsBddMeasurable` predicates on RAW functions, so `realSpectrumCfcHom` is the base of the CONTINUOUS layer the Borel calculus is extended from, not of `bddSymbols` itself.  Scope the next mission to exactly one thing: define `bddSymbols` on `spectrum R a` and prove the `Submodule C`-isomorphism with the existing one by precomposition with `realSpectrumHomeomorph` and its inverse -- the homeomorphism is already a Borel isomorphism, so the only new bricks are preservation of `IsBddMeasurable` in both directions and the resulting linear equivalence.  It must STILL not edit `BorelCalculus/`; add the transported symbol algebra beside it.  **Do not combine that with the `cyclicIsometry` rewrite** -- `range_cyclicIsometry` is where the refuted route died and it deserves its own compile budget.
...
**FOLLOW-UP (1) IS DISCHARGED, 2026-08-10, AND ITS PREMISE WAS WRONG.**  This field used to say "the missing lemma is that an intertwiner of two self-adjoint operators intertwines their continuous functional calculi ... Until that exists, `HalmosAngleDatum` carries `J cos Theta_0 = cos Theta_1 J` and `J sin Theta_0 = sin Theta_1 J` as HYPOTHESES rather than deriving them".  THE LEMMA WAS NOT MISSING -- `TauCeti.LinearPMap.cfcHom_intertwines_selfAdjoint` had existed in `ForTauCeti/Analysis/InnerProductSpace/SeparatedIntertwiner.lean` all along.  It was `C`-only while `HalmosAngleDatum` is `RCLike`-generic, and it was consumed by NOTHING.  It is now `RCLike`-generic and consumed; see `notes`.

REMAINING FOLLOW-UPS, neither blocking.  (1') Give `HalmosAngleDatum.ofIntertwinedAngles` a source-numbered consumer: a `theorem3_1_realization`-shaped wrapper stated from `(Theta_0, Theta_1, J)` rather than from a datum, which is the printed form of sentence (ii).  It should be a short `:=` on `theorem3_1_realization (HalmosAngleDatum.ofIntertwinedAngles ...)`.  THAT wrapper -- not the constructor -- is where `spectrum R Theta_i subset [0, pi/2]` belongs, so that `ker (sin Theta_i)` reads as the angle-`0` space and `ker (cos Theta_i)` as the angle-`pi/2` space.  (1'') THE REAL-MULTIPLICITY AXIS.  **ROUTE S AS FIRST SPECIFIED IS REFUTED BY A MACHINE-CHECKED COUNTEREXAMPLE -- do not re-dispatch it.**  Lowering the symbol CODOMAIN to `R` while `H` stays complex breaks polarization, and dead-ends at `range_cyclicIsometry` regardless.  THE SURVIVING ROUTE lowers the symbol DOMAIN to `spectrum R a` and KEEPS THE CODOMAIN `C`, via a transported calculus built on Mathlib's `SpectrumRestricts.homeomorph`.  It is ADDITIVE at the base, has a 623-module blast radius, and MUST be scoped as a multi-mission programme with its own base-layer mission first -- not as a checkpoint.  Note also that the two scalar axes are NOT separable: the Hilbert-space field is the substantive one.  See `notes` for the counterexample, the measurements and why Route C is dead.  (2) Inhabit `SpectralMultiplicityFoundation` from the now-proved uniqueness (canonical Datum as a quotient by `measureClassSetoid`); parked with the promotion bookkeeping.
...
**THE LAST PRINTED ASSERTION LANDED 2026-08-10 (Claude Opus 5, coordinator-verified), AND THE ANGLE MATCHES THE PAPER'S OWN DEFINITION -- PROVED, NOT ASSERTED.**  New `TauCeti.vectorAngle K x y = arccos (re <y,x> / (||x|| ||y||))`, which is transcription (1.14), the VECTOR angle with a REAL PART.  Two machine-checked bridges settle the trap this row recorded: `vectorAngle_real_eq_angle` and `vectorAngle_eq_angle_rclikeToReal` show Mathlib's `InnerProductGeometry.angle` normalizes with the REAL PART too, so it is (1.14) and NOT (1.15)'s modulus.  The recorded mismatch does not fire.
...
**ITEMS (c) AND (d) DISCHARGED, 2026-08-10 (Claude Opus 5, coordinator-verified).**  Item (d) required no work -- it had been done on 2026-08-09 and both `scope_gap` and `next_action` went on advertising it.  Item (c) is new: `Proposition6_1_commonDomain`.
...
- **Next action:** Items (a), (b) and (e) only -- see `scope_gap`.  ITEMS (c) AND (d) ARE BOTH DISCHARGED and have been struck: (d) was already done on 2026-08-09 and this field went on listing it for two days; (c) landed 2026-08-10 as `Proposition6_1_commonDomain`.  Item (b) should NOT be dispatched as a routine mission -- see the measured obstruction on `S2-unbounded-scope`.  Optionally, a REAL common-domain Proposition 6.1, which does not yet exist.
...
**THE MECHANICAL LEFTOVERS AND THE DOCUMENTATION DEFECT ARE CLOSED, 2026-08-10 (Claude Opus 5, coordinator-verified).**  Three real `sin 2Theta` endpoints under 8.2's own hypotheses -- operator norm perturbation and residual, plus every `PaperUnitaryInvariantNorm` for the perturbation -- and `theorem8_2_source_real`, packaging the whole printed theorem over `R` with the same three conjuncts and hypothesis list as the complex `theorem8_2_source`.  Assembling it required a missing piece: only the perturbation-only finrank form and the crossed-defects branch form existed over `R`, so `theorem8_2_branch_source_real_maximalAngle_lt` (the branch form under (1.5)) was added to mirror the complex packaging.
...
RECORD CORRECTIONS: `theorem8_2_residualHalfGap_selectedBranch` was named in these notes as proved but was ABSENT from `lean_declarations` -- coordinator-verified present at `Frontier/Section8.lean:850`, and now listed.  Unlike its perturbation twin it has no source-facing alias.  These notes also placed both selected-branch theorems in `DavisKahan/Experimental/Frontier/Section8.lean`, A PATH DELETED by the 2026-08-06 promotion recorded later in this same field.  Line references had drifted: the bridges are at `Section8.lean:809,825`, and `subspaceGap_eq_directedGap_of_finrank_eq` at `Section8SourceTheorem82.lean:164`.
...
- **Next action:** One item: the residual `sin 2Theta` at a general unitarily invariant norm at the directed `Theta_0`, per `scope_gap`.  Suggested route: prove `delta * N(sin 2Theta_0) <= N(R)` in the bounded reflected geometry via `paperLemma61_all_kyFan`, as `symmetric_sinTheta_spectrum_all_kyFan` does for the perturbation case, then `N(sin 2Theta) <= 2 N(sin 2Theta_0)` from the generic decomposition.  Everything else on this row is closed.
...
- **Blocked by:** `real-scalar-infinite-dimensional-scope`
- **Current Lean references:** `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.beamOperator`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.beamOperator_isSelfAdjoint`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.realSpectrum_beamOperator_subset_gap`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.realSpectrum_beamOperator_subset_sharp`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.beamTrial_orthonormal`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.inner_centeredAffineLp`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.inner_centeredAffineLp_mul`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.inner_mul_centeredAffineLp_mul`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.beamRitz_matrix`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.beamResidualGram_matrix`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.beamFiniteDataCertificate`, `TauCeti.DavisKahan1970.Section9.CenteredAffine`, `TauCeti.DavisKahan1970.Section9.ritz_matrix_from_affine_moments`, `TauCeti.DavisKahan1970.Section9.FreeBeamFiniteDataCertificate`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.exists_pos_eigenpair_beamOperator`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.exists_five_hundred_lt_mem_realSpectrum_beamOperator`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.exists_mem_realSpectrum_beamOperator_ne_zero`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.beamQuadLp`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.inner_affineLp_beamQuadLp`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.beamQuadLp_mem_beamTrial_orthogonal`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.norm_beamQuadLp_sq`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.beamQuadLp_ne_zero`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.eigenspace_beamResolvent_one_le_beamTrial`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.exists_beamOperator_apply_of_beamResolvent_smul`, `TauCeti.DavisKahan.Experimental.MathAhead.HiddenFoundations.FreeBeam.Model.exists_affine_of_beamResolvent_eq_self`, `TauCeti.LinearPMap.mem_realSpectrum_of_eigenvector`, `TauCeti.exists_hasEigenvalue_eigenspace_not_le`
...
**THE POSITIVE SPECTRUM IS PROVED NONEMPTY, 2026-08-10 (Claude Opus 5, coordinator-verified: build 9589 unchanged, certificate signature read directly, three endpoints axiom-clean).**  The witness is explicit: `beamQuadLp = t^2 - t + 1/6`, with both affine moments vanishing and `||beamQuadLp||^2 = 1/180`, hence nonzero and orthogonal to the trial plane.

**THIS ROW'S `next_action` CARRIED A FALSE AND LOAD-BEARING CLAIM.**  It said `what is needed is that the kernel is not everything, WHICH THE AFFINE-PLANE KERNEL COMPUTATION ALREADY GIVES`.  IT DOES NOT.  That computation says the kernel EQUALS the affine plane; it says NOTHING about the plane being PROPER in `BeamL2`.  Nothing in the repository proved `beamTrial-orthogonal != bot`.  That was the one genuinely missing ingredient and it had to be supplied from scratch.  The same field also claimed `the spectrum is a sequence of eigenvalues`, which is NOT proved anywhere -- the only compact-resolvent-to-spectrum result is a CONTAINMENT, and `PositiveSpectrumIsPointSpectrum` is a hypothesis predicate never discharged for `beamOperator`.

ROW SELF-CONTRADICTION, now resolved: the 2026-08-07 `ON INSTANTIATING THE RECORD` paragraph said `third_eigenvalue` `is supplied by an actual nonzero point of beamOperator.realSpectrum`, while the 2026-08-09 M18 adjudication in the SAME FIELD said `only given a nonzero real spectral point that nothing supplies`.  The later text was correct; the earlier claim was ALSO in the Lean docstring, and both are now TRUE rather than contradicted.

A COORDINATOR PREMISE WAS WRONG: the brief said the compact-resolvent theorem `belongs in the form-method / self-adjoint spectral theory layer of ForTauCeti`.  THERE IS NO FORM-METHOD LAYER IN `ForTauCeti` -- `CoerciveFormData`, `ShiftedBeamFormData`, `SequentiallyCompactEmbedding` and `inverseClosedOperator` all live in `DavisKahan/SpectralTheory/FormMethod/`.  (Inherited from handoff H.1, which says the same.)  What WAS missing was only the EXISTENCE half of the compact spectral theorem, now `TauCeti.exists_hasEigenvalue_eigenspace_not_le`, plus the missing INTRODUCTION rule `TauCeti.LinearPMap.mem_realSpectrum_of_eigenvector`; the beam-specific compact resolvent already existed and was not rebuilt.

DOCUMENTATION DRIFT FIXED: the `BeamSection9` module docstring listed `...FreeBeam.Model.beamTrial_eq_ker` under `Main results`.  NO SUCH DECLARATION EXISTS ANYWHERE.

**THE REAL-SCALAR AXIS IS HALF-CLOSED AND THE ROUTE IS SETTLED, 2026-08-10 (Claude Opus 5, coordinator-verified independently of the mission report).**  The 2026-08-09 entry above recorded axis (3) as "MEASURED AND DELIBERATELY NOT ATTEMPTED" and said this axis "should be the last of the three to be worked, and only after the `Lp` complexification isometry exists for an independent reason".  THE OWNER OVERRODE THAT, and the override was right: the `Lp` isometry turned out NOT to be the prerequisite it was assumed to be.  The abstract form method never contained any complex mathematics, so parameterizing it over `RCLike` cost a mechanical substitution and one genuine proof repair, and it did not require the isometry at all.  See `scope_gap` axis (B) for the measurement, what landed, and what remains.

TWO STANDING JUDGEMENTS IN THIS ROW ARE THEREFORE SUPERSEDED: "(ii) THE COST IS THE WHOLE MODEL" understated how much of the stack was ALREADY scalar-generic; and the conclusion that supplying the isometry is "an `Lp`-complexification contribution in its own right, of Tau Ceti scope, not Davis--Kahan work" is still true of the ISOMETRY but no longer describes what this row needs.  Route (a) also needs a complex-to-real DESCENT theorem that does not exist -- the isometry alone would still leave the beam operator complex.  That correction is recorded in `scope_gap` because it applies to every future appeal to the "just complexify" route, not only to this row.
- **Next action:** Two items, neither deferred -- see `scope_gap`.  (A) identify `beamOperator` with the closure of the classical fourth derivative on the free-end domain; this is now the only axis on this row that needs new mathematics.  (B) real `L^2`: the MEASUREMENT IS DONE and route (b) is chosen and half-executed -- the abstract form method and the unit-interval analytic layer are `RCLike`-generic and instantiate at `R`; what remains is the mechanical scalar substitution in the four concrete beam modules and their five consumers, listed in `scope_gap`.  Plus the sequence residue, which needs only infinite-dimensionality of `BeamL2`.
