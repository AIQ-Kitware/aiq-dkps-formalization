...
- **Next action:** Parts (ii) and (iii) only.  (ii): derive the ordered eigenvalue inequalities from `theorem8_1_upperCompressionRepulsion_canonicalBranch` / `..._lower...` using `LinearMap.IsSymmetric.eigenvalue_mono`.  (iii): the printed inequality is for EVERY symmetric gauge; use the existing FiniteSymmetricGauge / majorization / Ky Fan layer and do NOT replace it with the operator norm.  The paper's infinite-dimensional extension via approximation numbers may be exposed if it is cheap, but must not hold the exact finite theorem hostage.
