...
- **Next action:** One substantive item and two mechanical ones -- see `scope_gap`.  Substantive: the residual `sin 2Theta` at every unitarily invariant norm, which needs the Halmos generic decomposition to recover the printed constant.  Suggested route: prove `delta * N(sin 2Theta_0) <= N(R)` in the bounded reflected geometry via `paperLemma61_all_kyFan`, as `symmetric_sinTheta_spectrum_all_kyFan` does for the perturbation case, then `N(sin 2Theta) <= 2 N(sin 2Theta_0)` from the generic decomposition.  Mechanical: the real `sin 2Theta`, and a real `theorem8_2_source`.  Also fix the mislabelled alias docstring recorded in `scope_gap`.
