...
- **Next action:** Measure whether the complex `KyFanDominantIdealFamily` endpoint already covers the paper's `every unitarily invariant norm` via Fan dominance from `theorem6_3_all_kyFan_core`, as the real endpoint does.  If it does, this row is `compiled_exact` with no further Lean work.  The trial-dimension and real-scalar axes are both closed.
