...
| (ii) In finite dimensions, if lambda_k are the ordered eigenvalues of Lambda_1 and alpha_k those of A_1, then alpha_k - alpha <= \|\|C_1\|\|_1^2 (lambda_k - alpha), with the analogous lower-block statement and natural infinite-dimensional extensions. | Both printed readings carry canonical evidence, on the BLOCKS themselves rather than on the blocks extended by zero to the ambient space.  The finite reading is `theorem8_1_{upper,lower}EigenvalueRepulsion_blockSourceExact{,_real}`: the ordered eigenvalue lists of `A_1` on P-perp and `Lambda_1` on Q-perp, indexed by the block dimension, the two dimensions agreeing by `theorem8_1_finrank_orthogonal_branch_eq`.  The printed extension -- 'and natural infinite-dimensional extensions' -- is `theorem8_1_{upper,lower}ApproximationRepulsion_blockSourceExact{,_real}`: the same inequality with no dimension hypothesis, on the paper's own minimax sequence (1.10), which Section 1 says 'makes sense for general bounded operators'.  The two agree where both are defined: `approximationNumber_eq_eigenvalues_of_isPositive` says the minimax sequence of a positive operator is its sorted eigenvalue list, and every block here is positive.  `norm_cosineBlock_eq_principalCosines_zero` identifies the printed \|\|C_1\|\|_1 with the largest principal cosine in the finite reading; in the extension it is the cosine block's operator norm, which is that same largest singular value. | claimed_exact |
...
| The dimension scope differs across the parts: (a), (b) and (i) carry none, (ii) is printed in finite dimensions with natural infinite-dimensional extensions, and (iii) in finite dimensions. | That is exactly how the declarations are stated, and the two parts are not conflated.  The characterization, the branch construction and part (i) carry no dimension hypothesis.  Part (ii) carries BOTH readings: finite witnesses with `[FiniteDimensional]` on the printed eigenvalue lists, and dimension-free witnesses on the paper's minimax sequence for the printed extension, the latter at the paper's separable ambient scope.  Part (iii) carries `[FiniteDimensional]` and nothing else, which is the printed restriction -- the source attaches no extension clause to it. | claimed_exact |
