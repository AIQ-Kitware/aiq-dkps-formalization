...
Gates: S2-sin-theta (proved_in_build), S2-sin-two-theta (proved_in_build), S2-unbounded-scope (proved_in_build), DK-3.2-prop (proved_in_build), DK-3.1-thm (proved_in_build), DK-3.1-cor (proved_in_build), DK-3.5-prop (proved_in_build), DK-6-appendix (proved_in_build), DK-8.1-thm (proved_in_build), DK-8.2-thm (proved_in_build), DK-9-model (proved_in_build)
...
- **Next action:** Unchanged, and now to be read over both scalar fields: prove the printed third clause, for a unitary/orthogonal `W` with `W P_U = P_V W` and both diagonal blocks accretive, deriving `S_1 = S_0*` WITHOUT assuming `W^2 = (Q - Qperp)(P - Pperp)`.  The paper's route (transcription L887--898) is `C_0^2 S_1 = S_1 C_1^2` from (3.2)+(3.3), then `f(C_0^2) S_1 = S_1 f(C_1^2)` by the continuous functional calculus, then `f = sqrt` gives `C_0 S_1 = S_1 C_1`, and density of `Range(C_1)` in the acute case finishes.  Existence and uniqueness need nothing further, over either scalar field.
...
- **Next action:** One mathematical obligation, over both scalar fields.  Prove `sum_i (1 - ||C b_i||^2) = sum_k sin^2 theta_k` for an orthonormal basis of `U`, i.e. identify the compiled right-hand side with the printed one through `dim U - tr((C|_U)^2)` and the fact that the eigenvalues of `C|_U` are the principal cosines.  Until that lands, the compiled theorem is about an unidentified quantity.  The real-scalar axis is closed (M33).  Unchanged: do NOT restate the right-hand side as `sum_i cost D b_i` -- that form is false (notes).
