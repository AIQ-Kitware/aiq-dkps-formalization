...
Recursive hidden-foundation map for the Davis--Kahan 1970 formalization. Each node names its own production module. **A green report on this ledger does not mean the foundations are complete.** It means the ledger is internally valid and every declaration it names resolves in the built environment. The `interface` nodes are the opposite of completion: each is an explicit contract for a foundational campaign that has not been done, and its fields are not counted as delivered mathematics. Read the node kinds below before reading the counts.
