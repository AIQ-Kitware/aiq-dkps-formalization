...
def classify(rec: dict, txt: str) -> str:
    ...
    if body.startswith("# /") or body.startswith("---\nname:"):
        return "slash_body"
    # Bundled-SKILL body expansions are the same thing in a newer wrapper: the
    # harness injects the skill's markdown as a `user` record prefixed with the
    # unpack path.  Added 2026-08-09 (aivm-2404-jon) after one such record --
    # the `design-sync` skill, 28,011 chars -- was counted as a human prompt and
    # by itself carried 18% of that slice's human characters, which is enough to
    # move any claim stated in characters.  It survives every check in section 4
    # of AGENT-PROMPT.md: not `isSidechain`, not a known synthetic prefix, and
    # its register is documentation rather than instruction.
    if body.startswith("Base directory for this skill:"):
        return "slash_body"
    # A bare slash command typed at the prompt.  The wrapped `<command-name>`
    # form above is already routed to `slash_command`; the unwrapped form was
    # falling through to `human`, so `/compact` was being counted as a prompt.
    if re.fullmatch(r"/[A-Za-z][\w-]*(\s.*)?", body, re.S) and len(body) < 200:
        return "slash_command"
    # The `--continue` resume turn is generated by the harness, not typed.
    if body == "Continue from where you left off.":
