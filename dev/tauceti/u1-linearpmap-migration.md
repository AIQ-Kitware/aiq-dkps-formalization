...
| unbounded sine-theta residual data | production consumer migration in progress | `UnboundedSinThetaDataPMap` now stores the three raw partial maps together with explicit density and closed-graph hypotheses; its domain and residual/intertwining fields, plus `linearPMap_unbounded_adjoint_residual_block_identity`, are stated and proved directly over `LinearPMap` domains and adjoints | the legacy data record and the `toClosed` conversion are a source-facing compatibility boundary only; migrate the generic interval/gauge consumers before contracting it |
