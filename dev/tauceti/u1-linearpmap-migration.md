...
| Riccati block direct sum | generic downstream result | `LinearPMap.directSumDomain`, coordinate maps, component action, `directSum`, density, graph closedness, and `unboundedBlockOperatorCorePMap` (via raw `addBounded`) own the raw construction; `unboundedBlockOperatorCore_toLinearPMap` states the bundled core's canonical raw view, while its remaining package carries closedness | repoint block-core consumers to the raw partial map and contract the remaining closedness-only facade |
