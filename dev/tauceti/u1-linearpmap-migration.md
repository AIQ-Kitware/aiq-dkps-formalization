...
| Riccati block direct sum | generic downstream result, migration started | `LinearPMap.directSumDomain`, coordinate maps, component action, `directSum`, and density transport own the raw construction; `UnboundedCore` names delegate | port graph closedness and bounded off-diagonal perturbation before moving Riccati data records to raw partial maps |
