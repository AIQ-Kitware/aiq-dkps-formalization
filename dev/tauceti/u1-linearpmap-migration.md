...
| Riccati transport pullback | generic downstream result, migration started | `LinearPMap.pullbackDomain`, `pullbackDomainToOriginal`, `pullbackLinearMap`, `pullback`, and dense-domain transport own the raw construction; `UnboundedTransport` uses delegating compatibility names | port graph closedness and unitary-equivalence transport before moving Riccati data records to raw partial maps |
