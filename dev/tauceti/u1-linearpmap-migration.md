...
| shifted-inverse predicates and interval/exterior gauge estimate | generic downstream result | `LinearPMap.{Left,TwoSided}ShiftedInverseBound`, raw `addBounded`, raw Sylvester equations, the raw Neumann theorem, `linearPMap_norm_shift_apply_le_of_form_bounds`, both raw `linearPMap_norm_sylvester_le_of_{intervalExterior,exteriorInterval}` estimates, and `linearPMap_exists_bounded_shift_extension` now own the implementation | `ShiftedInverse` preserves the historical predicate names and interval/exterior estimates as compatibility facades, while the remaining ideal-gauge assembly in `ShiftedInverseGauge` retains bundle-shaped source inputs pending its public signature migration |
