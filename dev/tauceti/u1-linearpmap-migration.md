...
| selected reducing graph handoff | production consumer migrating | `ContractiveReducingGraphSelectionPMap` and its existential handoff store raw `LinearPMap.ReducesSubspace` and derive `StrongSolvesRiccatiPMap`; the historical record remains for existing experimental consumers | migrate source-facing continuation/diagonalization callers to the raw record, then make the historical record an explicit adapter |
