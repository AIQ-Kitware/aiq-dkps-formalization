...
| reducing restrictions and Riccati transport | production consumers still needing migration | `LinearPMap.InvariantSubspace`, `ReducesSubspace`, and `reducingRestriction` now own the complete restriction core: domain/action/map, density/closedness, adjoint-domain, symmetry, and self-adjointness; historical closed-operator theorems delegate to it | migrate the remaining Riccati and sine-theta consumers to raw partial-map inputs; bundled constructions must be replaced rather than wrapped permanently |
