...
| Riccati block direct sum | generic downstream result, migration started | `LinearPMap.directSumDomain`, coordinate maps, component action, and `directSum` own the raw algebraic construction; `UnboundedCore` names delegate | port density/closedness and bounded off-diagonal perturbation before moving Riccati data records to raw partial maps |
