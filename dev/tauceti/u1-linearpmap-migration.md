...
| Riccati graph reduction | production consumer migrating | the legacy invariance theorem is already stated over `LinearPMap.InvariantSubspace`; `unboundedBlockGraphPMapDataDomainVector` and its raw coordinate-action lemmas now work directly over `UnboundedBlockDataPMap` | prove the raw-data invariance equivalence, then migrate selected-graph records and leave the legacy theorem as its adapter |
