...
| Riccati transport pullback | generic downstream result | `LinearPMap.pullbackDomain`, `pullbackDomainToOriginal`, `pullbackLinearMap`, `pullback`, dense-domain transport, and graph-closedness transport own the raw construction; `UnboundedTransport` delegates all construction proofs | port unitary-equivalence transport before moving Riccati data records to raw partial maps |
