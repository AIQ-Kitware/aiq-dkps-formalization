...
| shifted-inverse predicates and interval/exterior gauge estimate | generic downstream result | `LinearPMap.{Left,TwoSided}ShiftedInverseBound`, raw `addBounded`, raw Sylvester equations, and the raw Neumann theorem now own the implementation | `ShiftedInverse` preserves the historical predicate names as reducible facades, while `ShiftedInverseGauge` retains bundle-shaped source inputs pending its public signature migration |
...
| Riccati block direct sum | generic downstream result | `LinearPMap.directSumDomain`, coordinate maps, component action, `directSum`, density, graph closedness, and `unboundedBlockOperatorCorePMap` (via raw `addBounded`) own the raw construction; the bundled block core remains a closedness-carrying facade | repoint block-core consumers to the raw partial map and make the bundle's canonical view a stated facade |
| Riccati graph reduction | production consumer migrated | `unboundedBlockGraph_invariantPMap_iff_strongRiccatiCore` is stated over `LinearPMap.InvariantSubspace` and raw block-domain vectors/actions; the historical theorem delegates definitionally | migrate the remaining Riccati data records and reduction consumers to the raw theorem |
| selected reducing graph handoff | production consumer migrated | `ContractiveReducingGraphSelection` and its existence handoff store raw `LinearPMap.ReducesSubspace` and derive raw invariance | continue through downstream diagonalization/transport records |
