...
| reducing restrictions and Riccati transport | production consumers still needing migration | `LinearPMap.InvariantSubspace` and `LinearPMap.ReducesSubspace` now own the representation-preserving predicates; the historical closed-operator names are reducible facades | port the restricted partial-map construction, its dense/closed/self-adjoint facts, and then consumers; bundled constructions must be replaced rather than wrapped permanently |
