...
| Riccati graph reduction | production consumer, migration started | `UnboundedReduction.unboundedBlockGraphDomainVectorPMap` carries graph vectors in the raw block-core domain | restate the invariant/Riccati equivalence over `LinearPMap.InvariantSubspace`, leaving only a source-compatible wrapper if needed |
