...
| reducing restrictions and Riccati transport | production consumers still needing migration | `LinearPMap.InvariantSubspace`, `ReducesSubspace`, and `reducingRestriction` now own the predicates, restricted domain, action, and resulting partial map; the historical closed-operator names obtain this algebraic core through reducible facades | port the dense/closed/self-adjoint restriction facts and then consumers; bundled constructions must be replaced rather than wrapped permanently |
