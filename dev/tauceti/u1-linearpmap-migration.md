...
| shifted-inverse predicates and interval/exterior gauge estimate | generic downstream result | `LinearPMap.{Left,TwoSided}ShiftedInverseBound`, raw `addBounded`, raw Sylvester equations, the raw Neumann theorem, and `linearPMap_norm_shift_apply_le_of_form_bounds` now own the implementation | `ShiftedInverse` preserves the historical predicate names and shifted-form estimate as compatibility facades, while `ShiftedInverseGauge` retains bundle-shaped source inputs pending its public signature migration |
