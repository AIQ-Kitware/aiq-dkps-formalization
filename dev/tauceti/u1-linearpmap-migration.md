...
| reducing restrictions and Riccati transport | production consumers still needing migration | `LinearPMap.InvariantSubspace`, `ReducesSubspace`, and `reducingRestriction` now own the predicates, restricted domain/action/map, density/closedness transport, adjoint-domain characterization, and symmetry preservation; historical closed-operator theorems delegate to this core | port self-adjoint restriction and then consumers; bundled constructions must be replaced rather than wrapped permanently |
