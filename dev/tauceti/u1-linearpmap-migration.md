...
| Riccati graph reduction | production consumer, migration started | raw graph-domain vectors plus arbitrary-domain and graph-vector coordinate lemmas carry the graph calculation without a bundled action | restate the invariant/Riccati equivalence over `LinearPMap.InvariantSubspace`, leaving only a source-compatible wrapper if needed |
