...
| reducing restrictions and Riccati transport | production consumers still needing migration | `LinearPMap.InvariantSubspace`, `ReducesSubspace`, and `reducingRestriction` now own the predicates, restricted domain, action, partial map, dense-domain transport, and graph-closedness transport; the historical closed-operator names obtain this core through reducible facades | port self-adjoint restriction and then consumers; bundled constructions must be replaced rather than wrapped permanently |
