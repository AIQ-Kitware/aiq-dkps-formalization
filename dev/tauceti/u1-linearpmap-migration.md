...
| unbounded sine-theta residual data | production consumer migration in progress | `UnboundedSinThetaDataPMap` now stores the three raw partial maps together with explicit density and closed-graph hypotheses; its domain and residual/intertwining fields, `linearPMap_unbounded_adjoint_residual_block_identity`, and `linearPMap_sinTheta_unbounded_gauge` are stated over `LinearPMap` domains and adjoints | the legacy data record and the `toClosed` conversion are source-facing boundaries. The raw gauge endpoint temporarily packages its right exterior block for the legacy bounded-left/exterior-right Neumann kernel; migrate that exact kernel before contracting this bridge and then convert the interval/gauge callers |
