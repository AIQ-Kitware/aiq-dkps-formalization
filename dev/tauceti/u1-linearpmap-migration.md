...
| Riccati transport pullback | generic downstream result, migration started | `LinearPMap.pullbackDomain`, `pullbackDomainToOriginal`, `pullbackLinearMap`, and `pullback` own the raw algebraic construction; `UnboundedTransport` uses delegating compatibility names | port density/closedness and unitary-equivalence transport before moving Riccati data records to raw partial maps |
