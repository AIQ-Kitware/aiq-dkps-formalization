...
| Riccati block direct sum | generic downstream result | `LinearPMap.directSumDomain`, coordinate maps, component action, `directSum`, density, and graph closedness own the raw construction; `UnboundedCore` names delegate | port the bounded off-diagonal perturbation before moving Riccati data records to raw partial maps |
