...
| one-unbounded/one-bounded Neumann estimate | generic downstream result | `linearPMapSylvester_mem_and_gauge_le_of_unbounded_bound_inverse` takes raw `LinearPMap` and `LinearPMap.SylvesterEquation` | the historical theorem now delegates to that raw proof; migrate its `GenuineSpectrum` and `ShiftedInverseGauge` callers before deleting the bundle-shaped entry point |
