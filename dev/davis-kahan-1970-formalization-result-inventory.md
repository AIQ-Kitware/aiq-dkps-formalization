...
lists of `upperBlockShift`/`lowerBlockShift` — the blocks extended by zero to the
ambient space — indexed by `Fin (finrank H)`, with part (iii)'s symmetric gauge
on `finrank H`, where the source reads the blocks' own lists at the block
dimension. It is closed by putting the statements on the blocks:
`upperBlockCompression` and `lowerBlockCompression` are `A₁ − α` on `Pᗮ` and
`(α + δ) − A₀` on `P`; extending by zero moves no approximation number, so the
ambient estimates transfer verbatim; `finrank_orthogonal_eq_of_isAcute` supplies
the block-dimension equality out of Theorem 8.1's own quarter-acute conclusion
(over `ℝ` through the complexification, whose gap is unchanged); and
`weaklyMajorized_comp_castLE` restricts the ambient majorization to the block
indices, which is what lets part (iii)'s arbitrary symmetric gauge be quantified
