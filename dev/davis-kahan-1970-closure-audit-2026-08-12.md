...
`S2-tan-theta` is classified paper-faithful under a documented nonlocal source interpretation: the Section 2 display omits (3.5), but the source later makes (3.5) standing before the Section 6 proof, and Section 1 has already announced the vacuity convention for nonexistent norms. A later hostile Appendix-scope audit found a separate mathematical narrowing: `tanTheta_unbounded_ambient_*_exact` allowed an unbounded ambient operator but still represented `A_0` by a bounded compression. The Appendix explicitly permits both `A_0` and `Lambda_1` to be unbounded. **CORRECTED 2026-08-12** by `TauCeti.DavisKahan1970.tanTheta_unboundedCompression_ambient_paperUINorm_exact` and its real sibling, which consume the genuine unbounded-compression Ky Fan core and feed the existing ambient two-corner assembly. See the result inventory and generated audit packet.
