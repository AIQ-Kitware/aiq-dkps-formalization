...
Current maintained result denominator: **29**. Exact printed-result boundary reviews: **29/29 accepted**. Current result-level semantic terminality: **28/29**; the remaining **1** is the mathematics/review queue. The old 46-row/32-reopened counts below are retained only as historical evidence from the broader pre-pivot hostile audit and are not the current completion metric.
...
| `S2-tan-theta` | `compiled_exact` | `proved_in_build` | `reopened_math` | RESOLVED 2026-08-12 for the mapping half: `tanTheta_wholeSpace_paperUINorm_real_of_crossedDefectsEquivalent` is now registered as primary evidence, in the Lean semantic audit surface, and in the ambient audit clause, so the real ambient conclusion no longer rests on a transversality premise stronger than the source. The remaining hole is mathematical: the unbounded ambient conclusion exists only over complex scalars. |
