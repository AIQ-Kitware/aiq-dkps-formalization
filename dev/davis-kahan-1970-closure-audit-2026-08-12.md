...
`DK-3.4-prop` is now result-level terminal: the existing complex full-scope theorem is paired with `TauCeti.DavisKahan1970.proposition3_4_full_real`, which transports the printed nonacute hypotheses through canonical complexification and descends the exact real direct-rotation clauses.


`DK-3.1-prop` is now result-level terminal: `TauCeti.DavisKahan1970.proposition3_1` packages the already-proved generic acute direct-rotation existence, uniqueness, positive diagonal blocks, crossed-block identity, and property-(i)-alone characterization at the paper's printed `TauCeti.IsAcute` hypothesis.
...
`S2-tan-theta` is classified paper-faithful under a documented nonlocal source interpretation: the Section 2 display omits (3.5), but the source later makes (3.5) standing before the Section 6 proof, and Section 1 has already announced the vacuity convention for nonexistent norms. A later hostile Appendix-scope audit found a separate mathematical narrowing: `tanTheta_unbounded_ambient_*_exact` allowed an unbounded ambient operator but still represented `A_0` by a bounded compression. The Appendix explicitly permits both `A_0` and `Lambda_1` to be unbounded. **CORRECTED 2026-08-12** by `TauCeti.DavisKahan1970.tanTheta_ambient_unboundedRitz_raw_paperUINorm_complex` and its real sibling, which consume the genuine unbounded-compression Ky Fan core and feed the existing ambient two-corner assembly. See the result inventory and generated audit packet.
...
| `S2-tan-theta` | `compiled_exact` | `proved_in_build` | `reopened_math` | RESOLVED after two hostile corrections: the nonlocal (3.5) semantics are explicit, and the Appendix-complete complex/real endpoints `tanTheta_ambient_unboundedRitz_raw_paperUINorm_complex` / `_real_exact` now cover the case where the Ritz compression `A_0` itself is unbounded. The older `tanTheta_unbounded_ambient_*_exact` declarations are bounded-compression specializations. |
...
| `DK-3.4-prop` | `compiled_exact` | `proved_in_build` | `accepted` | Full nonacute complex theorem plus compiler-validated real source-facing counterpart `proposition3_4_full_real`; exact printed scalar scope is now covered. |
