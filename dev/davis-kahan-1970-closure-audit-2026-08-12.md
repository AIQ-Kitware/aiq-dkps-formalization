...
`S2-tan-theta` has a compiler-validated sharp unbounded ambient theorem under `CrossedDefectsEquivalent` (3.5): `TauCeti.DavisKahan1970.tanTheta_unbounded_ambient_paperUINorm_exact`. **Superseded 2026-08-12 by the nonlocal-semantics audit.** This audit had recorded that the row would be closed by refuting the literal infinite-dimensional Section 2 ambient clause. That plan is withdrawn. The Section 2 display omits (3.5), but the source introduces (3.5) in Section 3, assumes it as standing for the remainder of the paper, and proves this theorem in Section 6 inside that scope, having already announced in Section 1 that results are vacuous when a displayed norm fails to exist. The row is therefore classified paper-faithful under a documented nonlocal source interpretation, the bilateral-shift construction is a vacuity/nonvacuity witness rather than a counterexample, and its one remaining blocker was the missing real-scalar unbounded ambient endpoint. **CLOSED 2026-08-13** by `TauCeti.DavisKahan1970.tanTheta_unbounded_ambient_paperUINorm_real_exact`, which proves that endpoint at the printed hypotheses, the printed constant, and every paper unitarily invariant norm over a real Hilbert space. See the `nonlocal_source_interpretation` record in the result inventory and the corresponding section of the generated audit packet.
...
| `S2-tan-theta` | `compiled_exact` | `proved_in_build` | `reopened_math` | RESOLVED 2026-08-12 for the mapping half: `tanTheta_wholeSpace_paperUINorm_real_of_crossedDefectsEquivalent` is now registered as primary evidence, in the Lean semantic audit surface, and in the ambient audit clause, so the real ambient conclusion no longer rests on a transversality premise stronger than the source. The mathematical half is RESOLVED 2026-08-13: `tanTheta_unbounded_ambient_paperUINorm_real_exact` supplies the real unbounded ambient conclusion, so the row is `accepted` with no completion holes. |
...
| `DK-3.4-prop` | `compiled_exact` | `proved_in_build` | `accepted` | Full nonacute complex theorem plus compiler-validated real source-facing counterpart `proposition3_4_source_full_real`; exact printed scalar scope is now covered. |
