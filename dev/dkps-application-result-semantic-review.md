...
| `acharyya-2024` | `A24-T5` | Theorem 5 | PASS source repair | The source's 'phi_i ~iid P' names only the latent vectors, while the response laws F_ij are model-specific. The printed conclusion integrates a quantity depending on the estimate, hence on the response data, so the model must be drawn as a complete object for the display to denote anything. This is the same repair recorded on A24-L2 and it is the only one this theorem adds. |
