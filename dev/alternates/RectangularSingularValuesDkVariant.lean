...
> comparison of proof strategy. It is intentionally **not** imported by `TauCeti.lean`.
...
> `GramSpectrumBridge` reference `TauCeti.eigenvalues_congr'`), and it uses `calc`/compact
