...
| 9 | The audit corpus is generated by a script that no longer exists | 23 documents | remove |
...
## 9. The audit corpus is generated by a script that no longer exists

`dev/audit/FILE-CHECKLIST.md` opens with

> Generated by `scripts/audit_checklist.py`.  Marks are preserved on
> regeneration ... Do not hand-maintain this list.

`scripts/audit_checklist.py` does not exist.  Neither does `scripts/run_gates.py`,
which `dev/audit/README.md` names as "current automated repository checks".  So
the corpus's two apparently-live documents -- 137 KB and 13 KB, carrying the
progress line "795/1396 files reviewed" -- are generated by a deleted tool, are
explicitly not to be hand-maintained, and report a number nobody can move.  The
other 21 files in the directory are a July 2026 review sweep that the directory's
own README calls "a historical review corpus, not a current work queue".

Nothing outside `dev/audit/` reads any of it.  The whole directory is removed.
...
50 documents and 5 Lean modules, 683 KiB.  Each was either a forwarding
tombstone whose entire content was "this is not current, see elsewhere", a
finished campaign still named as a plan, a survey of code that is no longer
vendored, or the audit corpus above.  Git history is the archive; every
reference to a removed document was rewritten to stand on its own rather than
left dangling.

Kept deliberately: `dev/journals/**` (the lessons corpus `AGENTS.md` points
agents at), `dev/posthoc-prompt-analysis/**` (a separate research study, not
repository history), every source census, statement map, semantic review and
coverage inventory, and `dev/davis-kahan-1970-full-sine-theta-proof-manuscript-2026-07-19.md`,
which `AGENTS.md` now names directly instead of through a stub.
